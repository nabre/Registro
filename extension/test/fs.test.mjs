// `workspace.fs` dello shim: le tre cose che, sbagliate, rompono il registro
// in silenzio.
//
// Il codice dell'errore, perché `archivio.ts` distingue «file mai scritto» da
...
// `percorsi.ts` ci separa gli anni dai file.
...
import { FileSystemError, FileType, Uri, workspace } from '../dist-prove/ambiente.mjs'
...
describe('gli errori arrivano con il codice che il registro guarda', () => {
  it('un file mai scritto è FileNotFound, non un ENOENT grezzo', async () => {
    ...
    assert.ok(errore instanceof FileSystemError)
...
describe('readDirectory dice di che genere è ogni voce', () => {
  it('separa i file dalle cartelle', async () => {
    ...
    assert.equal(mappa.get('registro.json'), FileType.File)
    assert.equal(mappa.get('2026-2027'), FileType.Directory)
  })
...
describe('le altre operazioni fanno quel che dicono', () => {
  ...
  it('stat dice il tipo e la dimensione', async () => {
    ...
    assert.equal(dati.type, FileType.File)

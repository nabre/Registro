...
// `archivio.ts`, i PDF della cassetta in `smistatore.ts` — ma il primo dei due
// regge la difesa più delicata di tutta la migrazione, e va capita prima di
// leggere il resto del file.
//
// `archivio.ts` scrive i propri file e poi si sente annunciare le proprie
...
import { dentro } from './contesto.js'
import { Disposable, EventEmitter, type Event } from './eventi.js'
import { RelativePattern, Uri } from './uri.js'
...
export function createFileSystemWatcher (dove: RelativePattern): FileSystemWatcher {
  ...
  const pronto = (async () => {
    ...
    const vivo = watch(base.fsPath, {
      // Come in VS Code: quel che c'era già non è una novità. `smistatore.ts`
      // ha `recuperaArretrati()` apposta per i PDF rimasti nella cassetta.
      ignoreInitial: true,
  ...
  const smaltibile = new Disposable(() => {
    chiuso = true

...
describe('una data scritta a mano', () => {
  // Il riferimento è la data che il campo aveva: completa quel che non si è
  // scritto («12»).
  ...
  it('un giorno che non esiste si rifiuta invece di scivolare al mese dopo', () => {
    // Il 31 aprile non scivola al 1° maggio: si dice di no.
...
describe('una data dentro un nome di file', () => {
  ...
  it('e si rilegge: è la stessa regola nei due versi', () => {
    assert.equal(dataDalNome('DIC4a_Fascicolo_260904.pdf'), '2026-09-04')
    assert.equal(dataDalNome('DIC4a_Fascicolo_260904'), '2026-09-04')
    assert.equal(dataNelNome(dataDalNome('x_251231.pdf')), '251231')
  })

  it('l’ora entra nel nome con il punto: i due punti nei file non ci stanno', () => {
    assert.equal(oraNelNome('08:20'), '08.20')
    assert.equal(oraNelNome('23:59'), '23.59')
  })

  it('quel che ora non è resta fuori dal nome, come per le date', () => {
    assert.equal(oraNelNome('24:00'), '')
    assert.equal(oraNelNome('8:20'), '')
    assert.equal(oraNelNome(null), '')
    assert.equal(oraNelNome(''), '')
  })

  // Il prefisso dei file in quarantena distingue due PDF con lo stesso nome e
  // dice quando sono arrivati.
  it('un istante si scrive come si legge, sull’orologio di chi guarda', () => {
    assert.equal(istanteNelNome(new Date(2026, 8, 14, 9, 42, 3)), '260914 09.42.03')
    assert.equal(istanteNelNome(new Date(2026, 11, 31, 23, 59, 59)), '261231 23.59.59')
  })

  it('un nome senza data, o con una data che non esiste, non ne inventa una', () => {
    assert.equal(dataDalNome('DIC4a_Presenze_1° semestre.pdf'), null)
    assert.equal(dataDalNome('DIC4a_Fascicolo_261340.pdf'), null)
    // Il numero sta in fondo, dietro un trattino basso: il resto del nome è pieno
    // di cifre.
    assert.equal(dataDalNome('260904_DIC4a.pdf'), null)
  })
})

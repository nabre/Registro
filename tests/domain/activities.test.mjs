// I parametri di un'attività, e la prova che una tappa prevede.
//
// La regola è che ogni tipo chiede quel che serve a lui: un lavoro di gruppo la
// grandezza dei gruppi, una verifica la durata e i punti. Quel che si prova qui
// è che il riassunto mostri solo quel che è stato riempito — un elenco di campi
// vuoti in una scaletta è rumore — e che un valore scritto sotto un tipo di
// prima non venga buttato quando il tipo cambia.
...
describe('i parametri dipendono dal tipo', () => {
  ...
  it('quel che è stato scritto sotto un altro tipo resta, e tace', () => {
    // Cambiare tipo per sbaglio non deve cancellare niente: il valore resta nel
    // file, e semplicemente smette di comparire.
...
describe('la tappa che si valuta', () => {
  it('lo dice la valutazione prevista, non il tipo', () => {
    // Una verifica senza valutazione prevista è una prova che non fa media:
    // capita, e il registro non deve inventarne una.
...
describe('la valutazione che stava sul piano', () => {
  ...
  // attività, e le due dicevano la stessa cosa in disaccordo. Adesso la prova è
  // una tappa e basta: i piani scritti prima vanno riletti così, e niente di
  // quel che qualcuno aveva scritto si perde per strada.
  ...
  it('se nella scaletta non c’è nessuna verifica, la tappa si aggiunge in fondo', () => {
    // Una prova che si faceva davvero non deve sparire dal piano solo perché
    // nessuno le aveva dato una riga nella scaletta.
    const piano = normalizzaPiano({
  ...
  it('non tocca niente se la scaletta dichiara già le sue prove', () => {
    // Chi ha scritto la scaletta sapeva quale tappa fosse la verifica meglio di
    // quanto lo sapesse una casella al livello del piano.
    const piano = normalizzaPiano({
      attivita: [
        {

// Che cosa finisce sullo schermo della classe. Un blocco spento non produce
// dati (non solo «non li disegna»): il proiettore sta davanti a tutta la
// classe. E quel che è acceso arriva davvero.
...
function aula () {
  ...
  // Due presenti e un assente: l'appello ha tre nomi, e nessun altro blocco li
  // deve sapere.
...
describe('la scaletta', () => {
  ...
  it('ogni tappa porta la tinta del suo tipo, già scelta dalla lista', () => {
    // Lo schermo in aula non conosce le liste: il colore arriva nella tappa.
    const { registro, mira, piano } = aula()
    piano.attivita[0].tipo = 'docenza-di-classe'
    piano.attivita[1].tipo = 'esercizio'
    registro.impostazioni = {
      ...registro.impostazioni,
      liste: {
        tipoAttivita: vociDiLista(null, 'tipoAttivita').map((voce) =>
    ...
    assert.equal(contenuto.scaletta[1].colore, '#123456')
    // La docenza di classe ha la sua tinta.
    assert.equal(
      contenuto.scaletta[0].colore,
      coloreDiVoce(null, 'tipoAttivita', 'docenza-di-classe'),
    )
    assert.notEqual(contenuto.scaletta[0].colore, COLORE_DI_RIPIEGO)
  })
})
...
describe('una scheda alla volta', () => {
  it('accese due, ne esce una sola', () => {
    // Un blocco acceso ma non in vista non produce dati: negli strumenti di
    // sviluppo dello schermo grande c'è solo quel che la classe guarda.
    const { registro, mira } = aula()
  ...
  it('in pausa non dice nemmeno i nomi delle schede', () => {
    // In pausa la striscia sparisce con il resto.
    const { registro, mira } = aula()
  ...
  it('le schede stanno nell’ordine dichiarato, non in quello dei clic', () => {
    // L'ordine di scorrimento con le frecce non dipende dai clic.
  ...
  it('spegnendo la scheda aperta si scivola sulla prima che resta', () => {
    // Spento il blocco che si guardava, lo schermo non resta vuoto.
...
describe('le misure della pagina', () => {
  it('sono strette di partenza', () => {
    // Il foglio non scorre: davanti alla classe nessuno può scorrere.
...
describe('la distribuzione di una prova proiettata', () => {
  it('è la stessa che finisce sul PDF, non una cugina', () => {
    // Il grafico di una prova è lo stesso su proiettore, portatile e foglio.
    const { registro, mira } = aula()
...
/*
 * Il calendario proiettato, nelle quattro viste del registro. La vista scelta
 * esce piena e le altre tre `null`; e quel che si vede è il calendario del
 * registro (giorni spenti, vacanze, confine di semestre), sul giorno aperto
 * nel portatile.
 */
describe('il calendario, nelle quattro viste del registro', () => {
  ...
  it('una vista sola esce piena: le altre tre non partono nemmeno', () => {
    // Quel che non si mostra non arriva all'altro pannello.
    const { registro, mira } = aula()
  ...
  it('la fascia oraria si stringe sulle lezioni che ci sono', () => {
    // La griglia si stringe sulle ore che ci sono: la giornata configurata è il
    // ripiego, non il riferimento.
    const { registro, mira } = aula()
  ...
  it('il mese porta le settimane del mese guardato, con le code segnate', () => {
    ...
    // Le code di settembre ci sono, segnate: la prima settimana non comincia a
    // mezz'aria.
  ...
  it('il giorno di riferimento è quello che il registro sta guardando', () => {
    // Si scorre la settimana sul portatile e la classe la vede scorrere.
    const { registro, mira } = aula()
  ...
  it('le vacanze e il confine di semestre arrivano allo schermo', () => {
    // Le due domande della classe davanti al calendario: si vedono come nel
    // registro.
    const { registro, mira } = aula()
  ...
  it('una vista che non esiste non lascia lo schermo senza niente', () => {
    // Un messaggio vecchio o un pannello ricaricato tornano all'agenda.
    const { registro, mira } = aula()

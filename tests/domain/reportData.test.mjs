// Che cosa il registro mette dentro un rapporto: il foglio dice di quale
// periodo parla, e i numeri di sopra sono quelli di sotto. Un rapporto va in
// mano ad altri, e lì un numero sbagliato non lo corregge nessuno.
...
import { componiFile, FILE_GENERATO } from '../../tools/templates.mjs'
...
describe('il conto delle presenze', () => {
  it('guarda un semestre alla volta, e lo scrive sul foglio', () => {
    // Presente in autunno, assente in primavera: sommando l'anno verrebbe «metà»,
    // che non è vero di nessun semestre.
  ...
  it('dice due percentuali, una per denominatore', () => {
    // Due ore da due UD: quattro previste, e sulla seconda niente appello.
    // «% assenza» guarda le previste (le ore dovute restano tali anche senza
    // appello); «% presenza» è cento meno quella; «% appello» guarda solo le UD
    // segnate e dice quanto le prime due sono affidabili.
    const registro = registroCon([
      ora('2026-10-06', [{ allievoId: 'al-1', stati: ['presente', 'presente'] }]),
      ora('2026-10-13', []),
    ])
  ...
  it('l’assenza è sulle ore previste, anche dove l’appello manca', () => {
    // Anna manca alla prima ora, la seconda è senza appello: sulle UD con appello
    // è assente al 100%, sulle previste ha perso metà, la cifra del rapporto.
    const registro = registroCon([
  ...
  it('chiude con la riga della classe: il numero che si legge per primo', () => {
    ...
    const righe =
      datiPresenze(registro, corso(registro), primoSemestre(registro)).tabelle.presenze.righe
...
describe('la scheda dell’allievo', () => {
  it('porta le presenze in cifre, non solo l’elenco delle assenze', () => {
    // La prima cosa che si chiede a un colloquio.
    const registro = registroCon([
  ...
  it('raccoglie le annotazioni sparse in un posto solo', () => {
    // Osservazioni dei verbali e note dei voti stanno insieme sulla scheda, in
    // ordine di data.
    const registro = registroCon([
      ora('2026-10-06', [{ allievoId: 'al-1', stati: ['presente', 'presente'] }]),
    ])
    ...
    // Niente colonna del corso: la scheda è di quel corso.
  ...
  it('la griglia delle presenze dice tutte le ore, non solo quelle mancate', () => {
    // Anche le ore andate lisce: un'ora regolare e una senza appello non si
    // confondono.
    const registro = registroCon([
      ora('2026-10-06', [{ allievoId: 'al-1', stati: ['presente', 'presente'] }]),
  ...
  it('la scheda di tutta la classe la colonna del corso ce l’ha', () => {
    // Senza corso la scheda unisce più ore: la colonna dice di quale materia è la
    // riga.
    const registro = registroCon([
      ora('2026-10-06', [{ allievoId: 'al-1', stati: ['presente', 'presente'] }]),
    ])
  ...
  it('senza appello non inventa una percentuale', () => {
    ...
    // La presenza resta una cifra vera (zero UD su quelle previste); quanto ci si
    // possa contare lo dice l'altra.
...
describe('i modelli di serie', () => {
  it('sono in accordo con templates/', () => {
    // Servono al primo avvio, quando `templates/` non c'è ancora. Si generano con
    // `npm run templates`: se cade, un modello è cambiato senza rigenerarli.
    assert.equal(
      readFileSync(FILE_GENERATO, 'utf8').replace(/\r\n/g, '\n'),
      componiFile(),
      'da rifare: npm run templates',
    )
  })
})
...
describe('il cento per cento sono le ore che l’orario prevede', () => {
  ...
  it('conta il monte ore dell’orario, non le ore già a calendario', () => {
    // Il semestre ha una ventina di martedì, e a calendario c'è una lezione sola:
    // chi la salta ha perso due UD su tutte le previste, non il 100%.
  ...
  it('con ore ancora da fare la frequenza resta piena', () => {
    // Le ore di maggio non ci sono ancora state: non sono perse. La frequenza è
    // cento meno l'assenza, che è zero.
    const registro = conOrario([
  ...
  it('un corso senza orario ripiega sulle ore a calendario', () => {
    // Senza monte ore l'unico conosciuto sono le ore messe a mano.
    const registro = registroCon([
      ora('2026-10-06', [{ allievoId: 'al-1', stati: ['presente', 'assente'] }]),
    ])
...
describe('i recuperi sul foglio che si consegna', () => {
  // Una casella vuota in conferenza è una domanda («e questo?»): il foglio porta
  // il recupero, così la risposta non si cerca a memoria.
  ...
  it('il voto rifatto porta la R: non è un voto preso con la classe', () => {
    ...
    // Valutato ma non ancora ridato: il momento non è chiuso.
  ...
  it('senza una riga, l’assente compare lo stesso: è un recupero da fissare', () => {
    // Il recupero viene dall'appello: la casella vuota porta la sua domanda anche
  ...
  it('le prove finiscono con il totale, e il profitto con la nota', () => {
    ...
    // Quante prove fanno media e la media, in fondo alla colonna dei voti.
    assert.deepEqual(dati.tabelle.prove.totale, ['Totale', '1 prova', '', '4.00', '', ''])
    // La nota sta a sé: il modello la mette in un riquadro.
  ...
  it('chi ha rifatto la prova non compare fra quelli a cui ridarla', () => {
    // Il suo foglio è quello del recupero e la riconsegna è segnata là: niente
    // seconda riga.
    const registro = conRecupero({ previstoIl: '2026-10-20', aggiornatoIl: 'x' })
    registro.valutazioni[0].voti[1] = { allievoId: 'al-2', valore: 4, assente: false }
    const dati = datiValutazioni(registro, corso(registro), primoSemestre(registro))
...
describe('la matrice di esecuzione e riconsegna', () => {
  // Le due date che si contestano («la verifica l'ho fatta», «non me l'hanno
  // ridata») stanno in una casella sola della griglia dei voti.
  ...
  it('un foglio non ancora ridato lo dice, invece di fermarsi alla data della prova', () => {
    // Il trattino dice che la riconsegna è ancora in ballo.
    const registro = conRecupero(null)
    const dati = datiValutazioni(registro, corso(registro), primoSemestre(registro))
  ...
  it('chi ha recuperato porta il giorno del recupero, non quello degli altri', () => {
    ...
    registro.valutazioni[0].voti[1] = { allievoId: 'al-2', valore: 4, assente: false }
    // Il giorno in cui gli altri l'hanno riavuta non è il suo: lui non l'aveva
    // ancora rifatta.
  ...
  it('chi non recupera e chi aspetta una data lo dicono in chiaro', () => {
    ...
    // Senza righe resta quel che sa l'appello: quel giorno non c'era. La stessa
    // parola della griglia dei voti.
...
describe('le due facce dello stesso denominatore', () => {
  // Presenza e assenza si contano sulle ore previste e sommano a cento solo
  // quando le ore sono tutte passate.
...
describe('le presenze sulla scheda dell’allievo', () => {
  // L'appello come a schermo: una riga per ora, una colonna per UD. Così si
  // vedono l'ora senza appello e la differenza fra un'ora intera e metà.
  ...
  it('porta una colonna per UD e le stesse sigle della griglia', () => {
    ...
    // Il corso non c'è: la scheda è di un corso solo.
  ...
  it('l’ora di cui nessuno ha fatto l’appello resta scritta, col trattino', () => {
    // Un'ora senza appello compare, e non si legge come un'ora andata liscia.
    const dati = schedaCon([
  ...
  it('le colonne sono quelle dell’ora più lunga, e le altre finiscono prima', () => {
    // Una casella vuota non è un trattino: quella UD non esiste.
...
describe('la scheda di una prova sola', () => {
  ...
  it('porta i conti della prova, non quelli del corso', () => {
    ...
    // Nessuna data della prova: la riconsegna è per allievo, qui si dice a che
    // punto è il giro.
  ...
  it('un punto per voto, al valore esatto e non per fascia', () => {
    // Ogni voto al suo valore: 3.75 e 3 non finiscono nella stessa colonna. Chi ha
    // lo stesso voto si impila.
    const registro = conProva([
      { allievoId: 'al-1', valore: 3, assente: false },
    ...
    // Il numero ogni mezzo punto, la lineetta ogni quarto (il passo dei voti).
  ...
  it('sull’asse segna la media, e la sufficienza no', () => {
    // La media c'è; la sufficienza la dice il colore dei punti, senza riga sua.
    const registro = conProva([
      { allievoId: 'al-1', valore: 3, assente: false },
  ...
  it('nell’elenco dei voti c’è anche chi non ne ha uno', () => {
    // La casella vuota è quel che si viene a chiedere: un elenco dei soli valutati
    // la farebbe sparire.
    const registro = conProva([
  ...
  it('senza nemmeno un voto non inventa una media', () => {
    ...
    // Il grafico senza punti lo toglie chi compone il foglio.
...
describe('l’assenza oltre la soglia', () => {
  // Due ore da due UD: chi manca a una intera è al 50%, oltre il 20% di serie.
  const conAssenze = () =>
    registroCon([
      ora('2026-10-06', [
        { allievoId: 'al-1', stati: ['presente', 'presente'] },
  ...
  it('e sulla scheda del singolo, con una frase da mettere in evidenza', () => {
    ...
    // Chi sta sotto non ha avviso: il riquadro sparisce da sé.
    assert.equal(scheda(registro.classi[0].allievi[0]).valori.avvisoAssenza, '')
  })

  it('appena oltre, un decimale: non «del 33%, oltre il 33%»', () => {
    // Tre ore da due UD, una persa: il 33,3%. All'intero sarebbe 33, e il foglio
    // si contraddirebbe.
    const registro = registroCon([
...
describe('la parete di ritratti della classe', () => {
  it('mette una casella per allievo, con foto o senza', () => {
    ...
    // Quante facce ci sono: dice se le foto sono ancora da mettere.
...
describe('la matrice del comportamento sui fogli stampati', () => {
  /** Un'ora con due caselle segnate: una con la nota, una senza. */
  function oraSegnata () {
    const lezione = ora('2026-10-06', [])
    lezione.matrice = [
      { allievoId: 'al-2', aspetto: 'partecipazione', segno: 'negativo', nota: 'disturba' },
      { allievoId: 'al-1', aspetto: 'partecipazione', segno: 'positivo' },
    ]
    return lezione
  }

  it('il verbale riporta le caselle segnate, per persona', () => {
    // Il verbale porta la matrice del comportamento: sei caselle segnate non
    // escono come «Osservazioni: nessuna».
    const registro = registroCon([oraSegnata()])

    const dati = datiLezione(registro, registro.lezioni[0], [])

    assert.deepEqual(dati.tabelle.comportamento.righe, [
      ['Bianchi Luca', 'Partecipazione', 'Da migliorare', 'disturba'],
      ['Rossi Anna', 'Partecipazione', 'Molto bene', ''],
    ])
  })

  it('un’ora senza caselle non porta nessuna riga', () => {
    // La tabella resta vuota e il modello la fa sparire.
    const registro = registroCon([ora('2026-10-06', [])])

    assert.deepEqual(datiLezione(registro, registro.lezioni[0], []).tabelle.comportamento.righe, [])
  })

  it('la scheda della persona le rilegge in ordine di giorno, dentro il periodo', () => {
    const prima = oraSegnata()
    const dopo = ora('2027-03-02', [])
    dopo.id = 'lez-2'
    dopo.matrice = [{ allievoId: 'al-1', aspetto: 'impegno', segno: 'negativo' }]
    const registro = registroCon([prima, dopo])

    const classe = registro.classi[0]
    const anno = datiAllievo(registro, classe, classe.allievi[0], null, corso(registro))
    // Senza la colonna del corso: il nome sta già in testata.
    assert.deepEqual(anno.tabelle.comportamento.righe, [
      ['06.10.2026', 'Partecipazione', 'Molto bene', ''],
      ['02.03.2027', 'Impegno', 'Da migliorare', ''],
    ])

    // Il periodo comanda qui come su ogni altra tabella del foglio.
    const primo = datiAllievo(
      registro, classe, classe.allievi[0], primoSemestre(registro), corso(registro),
    )
    assert.deepEqual(primo.tabelle.comportamento.righe.map((r) => r[0]), ['06.10.2026'])
  })

  it('la scheda dice se la persona si è ritirata', () => {
    // La scheda si stampa spesso quando qualcuno se ne va: la parola lo dice.
    const registro = registroCon([])
    const classe = registro.classi[0]
    const scheda = () => datiAllievo(registro, classe, classe.allievi[0], null, corso(registro))
    assert.equal(scheda().valori.stato, '')

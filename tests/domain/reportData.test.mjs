...
describe('i modelli di serie', () => {
  it('sono in accordo con templates/', () => {
    ...
    // da quella cartella con `npm run templates`: se questo test cade, qualcuno
...
describe('la matrice del comportamento sui fogli stampati', () => {
  ...
  it('il verbale riporta le caselle segnate, per persona', () => {
    // Prima il verbale taceva la matrice: un'ora con sei caselle segnate usciva
    // con «Osservazioni: nessuna», ed è il modo in cui un foglio dice meno di
    // quel che il registro sa senza che si veda.
  ...
  it('un’ora senza caselle non porta nessuna riga', () => {
    // La tabella resta vuota e il modello la fa sparire da sé: un titolo
    // seguito dal nulla si legge come un dato che manca.
    const registro = registroCon([ora('2026-10-06', [])])
  ...
  it('la scheda della persona le rilegge in ordine di giorno, dentro il periodo', () => {
    ...
    // Senza la colonna del corso: la scheda è di un corso solo, e il suo nome
    // sta gia in testata.
  ...
  it('la scheda dice se la persona si è ritirata', () => {
    // Una scheda si stampa proprio quando qualcuno se ne va, e senza questa
    // parola si legge come quella di chi frequenta ancora.
    const registro = registroCon([])
    const classe = registro.classi[0]

// Le consegne: quando scadono, chi le deve fare, in quali ore ricompaiono. La
// prova che conta è l'ultima: una consegna data il 12 si ripresenta il 19 e il
// 26 finché non è chiusa.
...
describe('le date di una consegna', () => {
  ...
  it('una consegna a una classe ancora vuota resta aperta', () => {
    ...
    // A settembre la classe c'è e gli allievi non ancora: la consegna non conta
    // come fatta.
  ...
  it('senza nessuno resta aperta: non c’è una chiusura che la zittisca', () => {
    // Si chiude spuntando i nomi: senza nessuno da spuntare non si chiude, si
    // cancella.
    const { registro, corso, ore } = scuola()
    const consegna = creaConsegna(corso.id, 'per una classe vuota', ore[0].data, ore[0].id)
    registro.consegne.push(consegna)
...
describe('chi deve spuntare', () => {
  it('alla classe tocca a tutti quelli che frequentano', () => {
    ...
    // Chi si ritira esce dai destinatari: una consegna che lo aspetta resterebbe
    // incompleta per sempre.
    verdi.attivo = false
  ...
  it('spuntarla per tutti la chiude, e resta scritto chi', () => {
    // Il gesto rapido (un clic per riga) scrive una spunta per nome: non dice
    // «fatta» di chi non ha portato niente.
    const { registro, classe, corso, ore } = scuola()
...
describe('la vecchia chiusura di una consegna', () => {
  it('alla rilettura diventa una spunta per ciascuno', () => {
    // Una consegna chiusa col vecchio «Chiudi comunque» si converte in spunte, così
    // resta chiusa.
    const { registro, classe, corso, ore, rossi } = scuola()
    const consegna = creaConsegna(corso.id, 'certificato', ore[0].data, ore[0].id)
    // Chi l'aveva già portata tiene la sua spunta con la data vera.

// Dove finisce ogni documento stampato e con che nome. Il posto è uno: lo
// compone chi scrive il file e lo ricompone la pagina Documenti per sapere se
// c'è già. PDF e CSV dello stesso documento stanno uno accanto all'altro con
// lo stesso nome.
...
describe('i documenti datati', () => {
  ...
  // Il prefisso riconosce la copia di ieri: col nome esatto, un fascicolo di una
  // settimana fa risulterebbe da fare.
...
it('due lezioni nello stesso giorno non si sovrascrivono', () => {
  ...
  // A distinguerle è l'ora, che è quel che chi cerca il foglio ha in mente.
  assert.match(uno, /_Verbali_260914 08\.00\.pdf$/)
  assert.match(due, /_Verbali_260914 10\.00\.pdf$/)
})

// Un nome di file si legge, si cerca, si detta: niente identificatori interni
// (`lez-m3k9x2-a7f1`).

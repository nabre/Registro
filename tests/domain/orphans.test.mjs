// I momenti di valutazione rimasti senza la tappa che li ha fatti nascere. Il
// motivo dev'essere giusto («il piano non c'è più» o «la tappa non è più una
// prova»): chi guarda decide in base a quello che cosa fare dei voti.
...
describe('i momenti sganciati dalla scaletta', () => {
  ...
  it('un piano sparito comanda sulla tappa sparita', () => {
    // Il piano non c'è più: «la tappa non c'è» sarebbe vero e inutile.
    const { registro, momento } = conUnaTappaValutata()

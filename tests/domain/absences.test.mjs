...
function periodo (righe = []) {
  // Il nome non si passa più alla creazione: lo ricava chi salva dal semestre in
  // cui le date cadono. Qui lo si mette a mano perché è quel che il registro
  // avrebbe scritto per un periodo che copre il primo semestre.
...
describe('un periodo che sta in piedi', () => {
  it('vuole due date e una lettera da spedire, e nessun nome', () => {
    ...
    // Il nome non si chiede a chi crea il periodo — un periodo *sono* le sue
    // due date — e quindi non c'è niente da convalidare: fuori dai semestri
    // resta vuoto, ed è un periodo buono come gli altri.
  ...
  it('e sta dentro il calendario dell’anno', () => {
    ...
    // Un periodo che sborda è quasi sempre l'anno sbagliato nella data, e
    // salvato com'è finirebbe su un anno in cui nessuno lo va a cercare.
    assert.deepEqual(

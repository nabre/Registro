// Le assenze da far firmare: le tre fasi di un periodo. Un foglio vergine apre
// la riga, la mail la fa avanzare, e «firmato» vale solo quando per ogni foglio
// partito ne è tornato uno.
...
function periodo (righe = []) {
  // Il nome lo ricava chi salva dal semestre delle date: qui lo si mette a mano
  // come lo scriverebbe il registro.
  return { ...creaBloccoAssenze('2025-09-01', '2026-01-31', undefined, '1° semestre'), righe }
}
...
describe('il testo della richiesta', () => {
  ...
  it('il periodo si legge con etichetta ed estremi', () => {
    ...
    // Senza nome restano gli estremi, che sono quel che il periodo è.
    assert.equal(
      periodoDetto({ ...periodo(), etichetta: '' }),
  ...
  it('il nome del periodo lo dicono le date, non un campo', () => {
    ...
    // copre due mesi soli: è in quel semestre che la scuola lo chiede.
    assert.equal(etichettaPeriodo(semestri, '2025-09-01', '2026-01-31'), '1° semestre')
    assert.equal(etichettaPeriodo(semestri, '2025-10-01', '2025-12-31'), '1° semestre')
    assert.equal(etichettaPeriodo(semestri, '2026-02-10', '2026-06-01'), '2° semestre')
    // A cavallo di gennaio non è né l'uno né l'altro, e chiamarlo con il nome
    // di uno dei due sarebbe dire una cosa falsa: resta senza nome.
    assert.equal(etichettaPeriodo(semestri, '2025-12-01', '2026-03-01'), '')
    assert.equal(etichettaPeriodo([], '2025-09-01', '2026-01-31'), '')
  })

  it('in breve: il nome se c’è, se no le due date', () => {
    assert.equal(nomePeriodo(periodo()), '1° semestre')
    assert.equal(nomePeriodo({ ...periodo(), etichetta: '' }), '01.09–31.01')
  })

  it('nomina i rapporti che sono davvero in allegato', () => {
    // La lettera parte per allievo: a chi ha solo ritardi non si parla di
    // assenze.
    ...
    // L'ordine è quello dei tipi, non del caricamento: la stessa lettera per tutti.
    assert.equal(
  ...
  it('la lettera scritta prima si aggiorna da sé quando il file si legge', () => {
    // Un periodo con la lettera di serie vecchia (assenze e ritardi sempre) passa
    // alla nuova da solo.
...
describe('i file dei fogli', () => {
  it('il nome su disco dice di chi è e che foglio è', () => {
    // Tipo e periodo: il nome dell'allievo lo aggiunge chi compone il nome del
    // file, una volta sola.
...
describe('un periodo che sta in piedi', () => {
  it('vuole due date e una lettera da spedire, e nessun nome', () => {
    const buono = periodo()
    assert.equal(validaBloccoAssenze(buono).valido, true)

    // Il nome non si chiede a chi crea il periodo: fuori dai semestri resta vuoto,
    // ed è un periodo buono.
    assert.equal(validaBloccoAssenze({ ...buono, etichetta: '' }).valido, true)
  ...
  it('e sta dentro il calendario dell’anno', () => {
    const anno = { inizio: '2025-09-01', fine: '2026-06-30', etichetta: '2025/2026' }
    const buono = periodo()

    assert.equal(validaBloccoAssenze(buono, anno).valido, true)
    // Un periodo che sborda dall'anno è quasi sempre una data con l'anno
    // sbagliato.
    assert.deepEqual(
      validaBloccoAssenze({ ...buono, dal: '2024-09-01' }, anno).errori,
      ['Il periodo esce dall’anno 2025/2026: va da 01.09.2025 a 30.06.2026.'],
    )
    assert.deepEqual(
      validaBloccoAssenze({ ...buono, al: '2026-08-31' }, anno).errori,
      ['Il periodo esce dall’anno 2025/2026: va da 01.09.2025 a 30.06.2026.'],
    )
    // Senza calendario non si controlla niente: è il caso di chi convalida un
    // periodo senza avere l'anno sotto mano.
    assert.equal(validaBloccoAssenze({ ...buono, dal: '2024-09-01' }).valido, true)
  })
...
describe('le richieste di firma nel todo', () => {
  ...
  it('un foglio caricato apre da sé una richiesta da spedire', () => {
    // Nasce dal caricamento: appena i rapporti entrano, la pratica è aperta.

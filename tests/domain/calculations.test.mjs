...
describe('orari della lezione', () => {
  ...
  it('vede anche gli accavallamenti non adiacenti', () => {
    ...
    // vicino al primo. Confrontando le sole coppie adiacenti A↔C sfuggiva, e
    // un orario sovrapposto si salvava senza che niente lo dicesse.
    const dentro = [creaSlot('08:00', 240), creaSlot('08:30', 30), creaSlot('09:30', 30)]
    // B e C fra loro non si toccano: i conflitti sono A↔B e A↔C, e il secondo
    // è proprio quello che il confronto a coppie vicine non vedeva.
    assert.equal(slotInConflitto(dentro).length, 2)

    // Chi comincia dopo la fine del primo non conta: l'uscita anticipata dal
    // ciclo non deve nascondere niente, ma nemmeno inventare conflitti.
...
describe('presenze', () => {
  ...
  // I conti per persona li fa `matriceCorso`, e sono gli stessi che finiscono
  // nella matrice a schermo, nel PDF, nel CSV e nelle segnalazioni. Stavano
  // anche in una `statisticheAllievo` scritta a parte, con un denominatore
  // diverso: le regole qui sotto sono quelle, provate dove adesso vivono.

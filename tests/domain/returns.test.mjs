// Le riconsegne: le prove svolte non ancora tornate agli allievi. La prima
// prova è quella che conta: una verifica svolta compare nel todo *senza che
// nessuno scriva niente*.
//
// Poi il confine fra i due lavori: finché una casella è vuota la prova è da
// correggere, e passa a «da riconsegnare» mettendo i voti. Solo l'ultimo passo
// si scrive: che la classe l'abbia rivista non lo dice nessun dato.
...
describe('da dove nasce una riconsegna', () => {
  it('dalla prova svolta, senza che nessuno scriva niente', () => {
    // Una verifica fatta ieri chiede qualcosa oggi, senza che nessuno la segni.
  ...
  it('una prova che deve ancora svolgersi non chiede niente', () => {
    // Una prova futura non ha riconsegna.
    const { classe, prova } = scuola()
...
describe('i due lavori, e il confine fra loro', () => {
  ...
  it('chi non c’era non tiene aperta la correzione', () => {
    // La casella dell'assente la guarda il todo dei recuperi: contarla qui
    // lascerebbe la prova mai corretta.
    const { classe, prova, rossi, bianchi, verdi } = scuola()
    correggiTutto(prova, [rossi, bianchi])
...
describe('l’unica cosa che si scrive', () => {
  it('la riconsegna chiude la riga, e nient’altro lo fa', () => {
    // I voti dicono che la prova è corretta, non che gli allievi l'hanno vista.
    const { classe, prova, rossi, bianchi, verdi } = scuola()
    correggiTutto(prova, [rossi, bianchi, verdi])

    assert.equal(riconsegnaDelMomento(prova, classe, DOMANI).stato, 'da-riconsegnare')

    // Non c'è una data della prova: ce n'è una per riga.
  ...
  it('si chiude quando è tornato l’ultimo foglio, non prima', () => {
    // Riconsegnata in più volte: la data è quella dell'ultimo foglio.
    const { classe, prova, rossi, bianchi, verdi } = scuola()
    correggiTutto(prova, [rossi, bianchi, verdi])
  ...
  it('una prova a cui erano tutti assenti non ha fogli da ridare', () => {
    // Il debito dell'assente è il recupero, nel suo elenco: fra le riconsegne
    // chiederebbe un gesto che non esiste.
    const { classe, prova, rossi, bianchi, verdi } = scuola()
  ...
  it('la vecchia data della prova scende su ogni voto alla rilettura', () => {
    // I documenti vecchi hanno una data sola per la prova: la si porta sulle righe,
    // così quel che era chiuso resta chiuso.
    const { classe, prova, rossi, bianchi, verdi } = scuola()
    correggiTutto(prova, [rossi, bianchi, verdi])
    // Una data sua ce l'ha già: è più precisa, e non si tocca.
  ...
  it('una prova ferma da più di due settimane si fa notare', () => {
    ...
    // In ritardo resta dov'è: il ritardo dice solo quale guardare per prima.
    assert.equal(riconsegna.stato, 'da-correggere')
  })

  it('sopravvive al giro su disco', () => {
    // Un campo che il caricamento lascia cadere si perde alla prima riapertura.
    ...
    // Chi non ha una data non ne prende una: vale «non riconsegnata».

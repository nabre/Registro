// La mappa: la geometria, la rubrica degli indirizzi, e chi sta dove sta un
// altro.
//
//   - La **proiezione** è Web Mercator come i tasselli: andata e ritorno si
//     chiudono, il meridiano zero cade a metà del piano.
//   - La **chiave** delle coordinate è l'indirizzo ridotto a forma
//     confrontabile: una domanda al geocodificatore vale per tutti quelli che
//     ci stanno, e due scritture della stessa via sono un portone.
//   - I **legami** (casa, ditta) compaiono solo dove ci sono: chi abita dove
//     lavora non ha un legame con nessuno.
...
describe('i punti della mappa', () => {
  ...
  it('gli strati spengono gli usi, non gli indirizzi', () => {
    const { classe } = classeConIndirizzi()
    const soloCase = segniDellaMappa(
      [classe],
      { domicili: true, lavori: false, sede: false },
      rubricaPiena(),
    )
  ...
  it('ogni tragitto ha un nome suo, e dice a quali punti è attaccato', () => {
    ...
    // Due fratelli che abitano e lavorano insieme fanno due righe sovrapposte: due
    // tragitti distinti.
...
describe('scomporre un indirizzo', () => {
  // Righe vere dall'anagrafica di una classe, come le detta un'azienda.

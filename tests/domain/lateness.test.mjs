// Il ritardo è una presenza, in tutti i conti.
//
// È una regola di una riga — `contaComeAssenza` accetta solo `assente` — ma
// vale in cinque posti diversi, e sono i cinque che finiscono su un foglio
// consegnato: il quadro della persona, il riepilogo dell'ora, la matrice del
// corso, le percentuali di fine semestre e la soglia che genera le
// segnalazioni. Cambiata in uno solo, gli altri quattro direbbero un'altra cosa
// senza che nessuno se ne accorga finché non li si mette uno accanto all'altro.
//
// Il perché sta nelle unità didattiche. Chi entra alla terza UD di un blocco di
// quattro ha le prime due segnate `assente`: quelle sono le ore perse, e si
// contano da sé. La terza — quella in cui è arrivato — è un'ora in cui c'era.
// Contarla come assenza vorrebbe dire toglierla due volte.
//
// Le prove qui sotto sono scritte con il ritardo **da solo**, senza assenze
// intorno: è la forma in cui la regola si vede, perché qualunque cosa il conto
// dica è colpa del ritardo e di nient'altro.
...
describe('il ritardo nei conti delle assenze', () => {
  it('la regola: solo «assente» toglie un’ora', () => {
    ...
    // L'esonero è fuori per un'altra ragione: c'è un permesso dietro, e
    // abbassare la frequenza di chi ce l'ha vorrebbe dire penalizzarlo.

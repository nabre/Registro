// Lo smistamento di un PDF unico di classe. Contano chi si riconosce su una
// pagina e come si tengono insieme le pagine di un documento: il nome in testa
// apre il blocco, le pagine mute che seguono gli appartengono.
...
/**
 * Le due metà della decisione, separate: `bozzaSmistamento` (la via del
 * pannello) mette tutto in un elenco con il motivo «da confermare», e qui si
 * rimettono in due elenchi con quel che la funzione pubblica già dice.
 */
...
describe('riconoscere chi è nominato in una pagina', () => {
  ...
  it('due fratelli in classe: il cognome da solo non decide niente', () => {
    ...
    // Nessuna delle due chiavi «rossi» entra nell'indice: sarebbe tirare a sorte
    // fra due fratelli.
...
describe('dividere un PDF di classe', () => {
  ...
  it('chi ha già consegnato non viene sovrascritto', () => {
    ...
    // Il file sta fra i documenti della consegna, la spunta dice che è successo:
    // per lo smistamento conta il file.
    const consegna = {
      ...raccolta([rossi.id]),
      documenti: [
        {
...
describe('la quarantena', () => {
  ...
  it('un PDF che non si è saputo aprire resta in vista, anche senza blocchi', () => {
    // Senza blocchi non è finito: resta nel pannello col suo file.
...
describe('tutto quel che aspetta, classe per classe', () => {
  ...
  it('i PDF di nessuna classe finiscono in coda, e non si perdono', () => {
    // Entrato dalla cartella osservata, senza consegna né classe: compare lo
    // stesso.
    const orfano = pdf('c', {})
  ...
  it('un PDF sta in un mucchio solo, anche quando due classi se lo contendono', () => {
    // Classe del PDF e classe della consegna diverse: si conta una volta sola.
...
describe('dove cadono le forbici, quando lo dice chi carica', () => {
  ...
  it('dentro un blocco il nome resta una proposta, non una condizione', () => {
    // Il taglio lo decide il passo; se c'è un nome leggibile, la riga arriva già
    // compilata.
    const classe = classeCon(['Rossi', 'Mario'], ['Bianchi', 'Luca'])
    const [rossi, bianchi] = classe.allievi
    const consegna = raccolta([rossi.id, bianchi.id])
  ...
  it('i buchi delle pagine già assegnate non si scavalcano', () => {
    // La 3 e la 7 non sono lo stesso documento solo perché vicine in un elenco
    // accorciato.
    const classe = classeCon(['Rossi', 'Mario'])
    const consegna = raccolta([classe.allievi[0].id])
  ...
  it('a mano non taglia niente: un pezzo solo per ogni tratto continuo', () => {
    ...
    // Il primo tratto nomina qualcuno ed è una proposta; il secondo resta da
    // ritagliare a mano.
  ...
  it('senza modo dichiarato si taglia come si è sempre fatto', () => {
    ...
    // Un passo a zero non diventa un taglio infinito.
...
describe('le pagine scelte con il mouse', () => {
  // Si prendono pagine sparse, e il registro si segna intervalli: queste due
  // funzioni fanno da ponte.
...
describe('dove, sulla pagina, il nome è stato letto', () => {
  // Un nome proposto porta il punto in cui compare, o va ricercato a mano sul
  // foglio.
  const pezzi = [

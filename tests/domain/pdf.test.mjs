...
// I caratteri standard stanno accanto ai bundle di prova: senza, pdfjs avverte
// a ogni documento aperto e calcola le larghezze del testo a occhio — e sono
// proprio le larghezze che dicono dove sta un nome sulla pagina.
impostaCaratteri(fileURLToPath(new URL('../../dist-tests/pdf-fonts', import.meta.url)))

/**
 * Il testo di ogni pagina, che è quel che quasi ogni asserzione qui guarda.
 *
 * Stava in `pdf.ts` come funzione esportata, e la produzione non la chiamava:
 * legge le posizioni e poi le butta, mentre lo smistamento ha bisogno proprio
 * di quelle. Qui invece il posto sulla pagina non interessa a nessuno, e una
 * riga sola è più leggibile di `(await ...).map(...)` ripetuto dieci volte.
 */
...
describe('ritagliare delle pagine sparse', () => {
  // È il caso per cui esiste: le due facciate di una persona finite lontane
  // nella scansione. Un intervallo qui non basta, e due ritagli separati
  // farebbero due documenti dove ce n'è uno.
...
describe('unire dei PDF in un fascicolo', () => {
  ...
  it('un foglio illeggibile salta, lo dice nel conto, e gli altri si uniscono', async () => {
    ...
    // Due su tre sono rimasti fuori: è quel che l'azione dice a chi consegna.
  ...
  it('un PDF protetto da password resta fuori invece di entrare illeggibile', async () => {
    // Il documento si segna cifrato a mano, che è esattamente quel che il
    // lettore guarda — un `/Encrypt` nel trailer. Il fork avrebbe
    // `documento.encrypt()`, che cifrerebbe sul serio; qui basta il marchio,
    // perché è su quello che `unisciPdf` decide.
    const buono = await pagelle(['Rossi Mario'])
...
describe('dove sta il testo sulla pagina', () => {
  // Serve a una cosa sola: segnare, sulla miniatura, il punto in cui il nome è
  // stato letto. Le coordinate escono in frazioni del foglio, con l'origine in
  // alto a sinistra — cioè come si disegna su un'immagine, non come le conta un
  // PDF.
  it('torna i pezzi con il loro posto, in frazioni del foglio', async () => {
    ...
    // Il nome sta in testa al foglio, non a metà: la prova serve a dire che
    // l'origine è in alto e non in basso come la conta il PDF.

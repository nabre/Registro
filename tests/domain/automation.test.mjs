...
describe('di quale giorno parla una modifica', () => {
  ...
  it('una lezione e una valutazione portano la loro data', () => {
    // È il giorno che sceglie il periodo dei fogli da rifare: correggendo a
    // marzo un voto di ottobre va rifatta la griglia del primo semestre, non
    // quella del semestre in cui si sta lavorando.
  ...
  it('quel che non ha una data non ne inventa una', () => {
    // Un cognome cambiato vale per l'anno intero, e chi chiama ripiega su
    // oggi: dire una data a caso vorrebbe dire rifare i fogli di un periodo
    // che quella modifica non ha toccato.

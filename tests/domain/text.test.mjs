...
// Cercare è un gesto di fretta per definizione: lo si fa per **non** dover
// ricordare come si scriveva. Quindi è la ricerca che si adatta a chi scrive,
// e mai il contrario — maiuscole, accenti e apostrofi non devono poter
// nascondere una persona a chi la sta cercando.
describe('la ricerca di un nome', () => {
  ...
  it('non guarda gli accenti, nei due versi', () => {
    // La segreteria incolla «Muller» dove il registro scrive «Müller», e chi
    // cerca ha sotto le mani una tastiera che l'accento non ce l'ha.
  ...
  it('non guarda l’apostrofo, scritto o dimenticato', () => {
    ...
    assert.equal(trova("Dell'Acqua Renata", 'dell acqua'), true)
    assert.equal(trova('Dell Acqua Renata', "dell'acqua"), true)
    // Anche l'apostrofo curvo, che è quello che le tastiere mettono da sé.
    assert.equal(trova('Dell’Acqua Renata', 'dellacqua'), true)
  })

// I modelli dei rapporti letti da `templates/`: una riga storta rovina quella
// riga e basta (li modifica chi non programma), e le sezioni vuote spariscono.
...
describe('i modelli dei rapporti', () => {
  ...
  it('le sezioni senza niente sotto spariscono', () => {
    ...
    // «Consuntivo» con un paragrafo vuoto se ne va con lui.
    assert.deepEqual(
      corpo.map((b) => b.tipo),
  ...
  it('un grafico senza punti non è un grafico', () => {
    // Una prova che nessuno ha fatto non ha grafico: niente asse spoglio.
  ...
  it('un testo scritto a mano resta anche senza segnaposto', () => {
    // Una riga senza graffe è quel che qualcuno ha scritto: resta.
 ...
 * grassetto un decimo di più. Si prova la spartizione, e i conti restano
 * leggibili («Rossi» a corpo 10 sono 50 punti).
 */
...
describe('lo stile dei rapporti', () => {
  it('legge i formati per nome e per misure, e rifiuta quel che non è un foglio', () => {
    ...
    // Un refuso non cambia il formato: chi chiama tiene quel che aveva.
  ...
  it('legge le misure del foglio, e una riga storta non cambia niente', () => {
    ...
    // Gli altri corpi restano quelli di serie.
  ...
  it('un modello che non dichiara niente non sovrascrive con i predefiniti', () => {
    // «margini» e «orientamento» non dichiarati dal figlio non coprono quelli della
    // base.
...
describe('la larghezza delle colonne di una tabella', () => {
  ...
  it('dà a ognuna quel che le serve, e l’avanzo a chi pesa di più', () => {
    ...
    // «Allievo» pesa tre volte «Data»: l'avanzo va quasi tutto a lei.
    assert.ok(misure[1] > misure[0] * 2, `${misure[0]} contro ${misure[1]}`)
  })

  it('scrive più piccolo prima di troncare', () => {
    // Dodici colonne di nomi lunghi in una larghezza da otto: fra «Ros...» e mezzo
    // punto in meno si sceglie il punto.
...
describe('i comandi del corpo di un modello', () => {
  ...
  it('«se» guarda anche dentro tabelle, elenchi e gruppi', () => {
    // «C'è?» vale anche per una tabella, non solo per un valore.
    const modello = leggiModello('v', '[corpo]\nse: {{recuperi}}\ntesto: ce ne sono\nfine:\n')
  ...
  it('«ripeti» rifà quel che sta dentro per ogni voce, con i dati di quella voce', () => {
    ...
    // La tabella viaggia col blocco: cercarla per nome a giro finito darebbe
    // sempre la stessa.
  ...
  it('una sezione vuota sparisce anche dentro un «ripeti»', () => {
    // La potatura non guarda oltre la sezione: «Annotazioni» vuota non resta
    // sopra il titolo dell'allievo dopo.
    const modello = leggiModello('v', [
      '[corpo]',
      'ripeti: allievi',
      'titolo: {{allievo}}',
  ...
  it('il salto pagina resta in coda, gli spazi no', () => {
    // Una pagina bianca in fondo chiesta apposta resta: il rapporto per allievo
    // finisce su un foglio staccabile.
    const modello = leggiModello('v', '[corpo]\ntesto: uno\npagina-nuova:\nspazio: 8\n')
...
describe('le frasi comuni', () => {
  ...
  it('una frase che ne cita un’altra non gira in tondo', () => {
    // Un giro solo: due frasi che si rimandano non girano per sempre.
...
describe('le colonne di una tabella scelte dal modello', () => {
  ...
  it('«*» sta per tutte le altre, nell’ordine loro', () => {
    // Una colonna per prova senza sapere quante saranno.
  ...
  it('si ribattezzano dopo la scelta, non prima', () => {
    // La scelta usa i nomi di serie: rinominare una colonna non rompe i modelli.
...
describe('le immagini di un modello', () => {
  ...
  it('rifiuta le risalite e i formati che non sono immagini', () => {
    // Il valore viene da file modificabili a mano: dalle due cartelle non si esce.
  ...
  it('i campi si chiedono su una colonna o su due', () => {
    ...
    // Una barra dentro un valore resta nel valore.
  ...
  it('una parete di ritratti si chiede a colonne e a millimetri', () => {
    ...
    // Senza dirlo, quattro caselle per riga su un A4.
    assert.deepEqual(leggiRichiestaGalleria('allievi'), {
      nome: 'allievi',
      colonne: 4,
      altezza: 30,
    })
    // Fuori dai limiti si torna al valore di serie.
...
`)
    const vuota = {
      valori: {}, elenchi: {}, tabelle: {}, grafici: {}, gallerie: { allievi: { celle: [] } },
    }
    const piena = {
      ...vuota,
    ...
    // Con la parete se ne va anche la sezione che la annunciava.
  ...
  it('una foto di allievo è un percorso dentro l’anno, e passa', () => {
    ...
    // dell'anno (il ritratto di un allievo).
  ...
  it('la testata si eredita tutta insieme, righe e immagini', () => {
    ...
    // Chi dichiara una testata sua la vuole sua per intero, senza il logo della
    // base.
...
describe('i blocchi riusabili', () => {
  ...
  it('legge le sezioni «[blocco: nome]» e non quel che sta fuori', () => {
    ...
    // Una parentesi che non è un blocco chiude quello aperto: `[frasi]` messo per
    // sbaglio non diventa contenuto.
  ...
  it('un parametro non passato lascia sparire la sua riga', () => {
    ...
    assert.deepEqual(
      ...
      // Lo `spazio: 6` in coda al blocco cade con la potatura.
  ...
  it('«usa» funziona anche dentro un «ripeti», con i dati del giro', () => {
    // I dati del giro contano anche i blocchi: ogni «usa» dentro resta.
    const modello = leggiModello('v', [
      '[corpo]',
      'ripeti: allievi',

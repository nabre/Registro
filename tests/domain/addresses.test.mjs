...
//
// La prova che conta è una sola, e sta in fondo: una riga vecchia letta e
// riscritta deve tornare identica. Non è pignoleria — le coordinate della
// mappa stanno in una raccolta a sé, con la riga dell'indirizzo come chiave, e
// una riga che si ricompone diversa è un segnaposto che sparisce dalla mappa
// senza che nessuno abbia toccato niente.
//
// Le righe qui sotto sono le forme che arrivano davvero dai registri veri: la
// via con il NAP in fondo, il nome di uno studio davanti, la casella postale
// in mezzo, il paese fra parentesi in coda.
...
describe('leggere una riga d’indirizzo', () => {
  ...
  it('un NAP estero si legge lo stesso, sigla del paese compresa', () => {
    // La sigla resta attaccata al NAP: è così che si scrive su una busta
    // diretta all'estero, e toglierla vorrebbe dire riscrivere l'indirizzo di
    // qualcuno a modo nostro.
  ...
  it('senza NAP non si indovina niente: la riga resta intera nella via', () => {
    // Meglio una casella con dentro tutto che quattro caselle riempite a caso:
    // chi apre la scheda vede che cos'era e la sistema.
...
describe('il giro completo', () => {
  // È la prova che tiene in piedi la mappa: la chiave delle coordinate è
  // questa riga, e se il giro non torna il punto si perde.
  const righe = [
    'Via Campagna 2, 6512 Giubiasco',

// I documenti da raccogliere sono consegne come le altre, con la categoria del
// documento: la spunta si porta dietro il file e la matrice del docente di
// classe è solo una vista. L'ultima prova: i documenti del formato vecchio
// arrivano come consegne.
...
describe('l’archivio di una classe', () => {
  ...
  it('elenca i fogli nell’ordine della pagina, non in quello di arrivo', () => {
    // Le frecce dell'anteprima seguono l'ordine della matrice, non quello delle
    // scansioni.
  ...
  it('i fogli di colonna vengono prima dei singoli, e si sa che cosa sono', () => {
    // Foglio firme e circolare riguardano la richiesta intera: non stanno fra i
    // nomi.
    const { uno, due, consegna } = conFile()
...
describe('i documenti scritti prima diventano consegne', () => {
  ...
  it('raggruppa per titolo i documenti dei singoli e tiene i file già raccolti', () => {
    ...
    // Chi aveva già portato il foglio risulta spuntato, e il file sta fra i
    // documenti della consegna.
...
describe('la firma in fondo ai messaggi', () => {
  ...
  it('una firma a righe tiene il messaggio in testo semplice', () => {
    // Con «-- » davanti: la convenzione della firma nella posta.
  ...
  it('una firma con dei tag porta tutto il messaggio in HTML', () => {
    // Un corpo è testo o HTML: decide la firma, e il testo si adegua.
  ...
  it('quel che in HTML vorrebbe dire qualcosa si neutralizza', () => {
    // «<3 ore» non apre un tag, e una & non si mangia la parola dopo.

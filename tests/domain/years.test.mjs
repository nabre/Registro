...
//
// La regola provata qui è una sola, e vale la funzionalità: l'anno non ha date
// sue — comincia col primo semestre e finisce con l'ultimo — e fra un semestre
// e il successivo non resta un giorno di nessuno. Prima le date erano quattro e
// indipendenti, e una lezione poteva capitare dentro l'anno e fuori da entrambi
// i semestri: spariva dalle medie e dal cruscotto senza che niente lo dicesse.
...
describe('i semestri sono attigui', () => {
  it('chiude il buco fra l’uno e l’altro', () => {
    // Il secondo comincia una settimana dopo: i giorni in mezzo non erano di
    // nessuno, ed è il caso per cui questa regola esiste.
      ...
      anni: [
        {
          id: 'a1',
          etichetta: '2026/27',
          // Le date dell'anno dicono una cosa, i semestri un'altra: vincono i
          // semestri, e il buco di febbraio si chiude.
          inizio: '2026-08-01',
          fine: '2027-07-15',
...
describe('dividere un registro per anno', () => {
  /**
   * Due anni con una classe ciascuno, e sotto ognuna un corso, un'ora, un
   * voto, una consegna e un fascicolo: la catena intera, che è quel che la
   * divisione deve saper risalire.
   */
  ...
  it('gli sciolti vanno in un anno solo, e non spariscono', () => {
    // Una classe che dichiara un anno che non c'è: senza il ripiego finirebbe
    // in nessuna cartella, ed è la perdita che una migrazione non può fare.
    const registro = registroDiDueAnni({
      classi: [
        { id: 'c1', annoId: 'a1', nome: 'DIC3a', allievi: [] },
        { id: 'c2', annoId: 'a2', nome: 'DIC4a', allievi: [] },
  ...
  it('ogni voce del registro finisce in esattamente una cartella', () => {
    ...
    // 'a2' è l'anno in uso: raccoglie gli sciolti. La somma delle due fette
    // deve fare il registro intero, senza doppioni e senza buchi.
...
describe('le settimane A e B', () => {
  ...
  it('la lettera si mette sulla settimana, non sul giorno', () => {
    // Si clicca stando su un mercoledì, e a essere marcata è la sua settimana:
    // il lunedì è la chiave, e da qualunque giorno la si chieda si trova.
  ...
  it('una settimana senza lettera non compare nel file', () => {
    // Non «vuota»: proprio assente. Un anno intero di stringhe vuote sarebbe
    // quaranta righe di JSON per dire che non c'è niente da dire.
    const messa = conLetteraSettimana(anno, '2026-09-02', 'A')
  ...
  it('l’anno letto dal file butta le chiavi che non sono date e le lettere sconosciute', () => {
    const registro = normalizzaRegistro({
      anni: [
        {
          ...anno,

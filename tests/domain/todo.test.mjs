// Il todo raccolto per classe. Le quattro famiglie (assenze da firmare, prove
// da correggere e ridare, documenti, attività assegnate) restano separate, e
// il lavoro di una classe è quello dei suoi corsi.
...
describe('il todo di una classe', () => {
  it('divide le consegne per tipologia: chi deve fare che cosa', () => {
    // Quattro lavori, quattro momenti: un certificato si sollecita, una pagella è
    // un file per ciascuno, «esercizi 4–7» si spunta, le fotocopie le fa chi
    // insegna.
  ...
  it('un documento chiesto al docente è suo da consegnare, non da raccogliere', () => {
    // `a: 'docente'` con un documento: il foglio lo maneggia chi insegna.
    const { registro, prima, corsi } = registroBase()
    registro.consegne = [
  ...
  it('tiene fuori il lavoro delle altre classi', () => {
    // Una prova della II B non è lavoro della I A anche se la materia è la stessa.
  ...
  it('chi passa la soglia di assenza diventa una pendenza', () => {
    // La soglia produce una riga nella sua famiglia, e si spegne a zero.
    const { registro, prima, corsi } = registroBase()
    registro.impostazioni.sogliaAssenza = 20
    // Un'ora fissa il mercoledì: nel semestre di OGGI si conta la percentuale su
    // quelle previste.
    ...
    // L'appello è fatto su tutte le ore esistenti: la segnalazione preme.
  ...
  it('il filtro delle consegne vale solo per le consegne', () => {
    ...
    // Il filtro cambia che cosa entra, non la tipologia.
...
describe('il riepilogo di tutte le classi', () => {
  ...
  it('le classi senza niente restano nel riepilogo ma non nella pagina', () => {
    // Il registro di una classe risponde anche «niente»; la pagina Todo mostra solo
    // chi ha da fare.
    const { registro, prima, seconda, corsi } = registroBase()
  ...
  it('senza niente da fare i conti sono a zero, non mancanti', () => {
    // Le quattro famiglie esistono sempre.
    const { registro, prima, corsi } = registroBase()

// Le riparazioni: che cosa il registro sa rimettere a posto da solo sotto i
// file modificati a mano, e che cosa si rifiuta di fare (perdere un voto,
// indovinare un corso).
...
describe('i titoli dei corsi', () => {
  it('rimette nell’ordine di adesso i titoli scritti al contrario', () => {
    // I titoli 'Materia — Classe' finiscono lontani dagli altri corsi della classe
    // negli elenchi ordinati.
  ...
  it('recupera la materia sparita anche da un titolo dell’ordine vecchio', () => {
    // La materia si legge togliendo la classe, non prendendo una metà per
    // posizione.
    const { registro, corso, classe, materia } = registroBase()
    corso.titolo = `${materia.nome} — ${classe.nome}`
    registro.materie = registro.materie.filter((m) => m.id !== materia.id)

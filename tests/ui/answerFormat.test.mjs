// Come si legge quel che il modello ha risposto.
//
// La prova che conta è una sola, ed è quella che non si vede guardando lo
// schermo: **una tabella conservata riga per riga e cella per cella**. Una
// colonna contata male non rompe niente — disegna una cella in meno — e chi
// guarda dà la colpa al modello, che quella volta aveva scritto giusto.
//
// L'altra metà è il contrario: che quel che **non** è una tabella non diventi
// una griglia storta. Un percorso con delle barre dentro una frase è il caso
// normale, e una riga di dati inventata dove c'era una frase è peggio di una
// frase lasciata com'era.
...
describe('i pezzi di una riga', () => {
  ...
  // Un asterisco battuto per sbaglio non deve ingoiare la frase che segue: chi
  // lo vede scritto capisce che è suo, chi non vede più il resto no.
...
describe('i blocchi di una risposta', () => {
  ...
  // Le cifre si confrontano con l'occhio solo se cominciano nello stesso
  // punto: «4» e «12» allineati a sinistra si leggono contando le cifre.
  ...
  // Quel che arriva dal modello non è codice del registro: qui si controlla
  // che il parser non ne ricavi niente di diverso da testo. Chi lo disegna
  // costruisce nodi, e un tag resta le parole che sono.

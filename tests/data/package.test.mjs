...
describe('il documento dell’anno', () => {
  ...
  it('conta le riscritture di una voce, e non quelle che non cambiano niente', async () => {
    ...
    // Lo stesso rapporto rifatto identico non è una copia nuova: chi lo sta
    // guardando non deve vedersi ricaricare la pagina sotto gli occhi.
    pacchetto.deposita('esportazioni/DIC4a_Presenze.pdf', new Uint8Array([1, 2, 3]))
    assert.equal(pacchetto.revisioneDi('esportazioni/DIC4a_Presenze.pdf'), prima)

    // Un voto corretto con un altro lungo uguale: stessa misura, contenuto
    // diverso — ed è proprio il caso che la misura da sola non saprebbe dire.

...
// `mtmd.ts` le cancella nel `finally`, che non gira se Windows si spegne col
// registro aperto. All'avvio si tolgono le vecchie, solo coi due prefissi, e
// senza mai sollevare.
...
const { ripulisciTemporaneiVecchi } = await importaSorgente('src/data/temporaryFiles.ts')
...
describe('ripulisciTemporaneiVecchi', () => {
  ...
  it('non solleva quando la cartella non c\'è', async () => {
    await assert.doesNotReject(
      ripulisciTemporaneiVecchi(join(tmpdir(), 'registro-cartella-che-non-esiste-mai')),
    )
  })
})

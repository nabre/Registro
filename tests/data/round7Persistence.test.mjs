// Il settimo giro: tre modi in cui il registro perdeva dati senza dirlo.
//
// Ognuna di queste prove era rossa prima della correzione che la accompagna:
//
//   - due registri sullo stesso documento — o un documento compattato altrove
//     e poi accodato da chi l'aveva letto prima — lasciavano un file che non
//     si riapriva, o che aveva perso la modifica dell'altro in un modo che
//     nessuno vedeva (`Pacchetto.accoda` scriveva agli offset di quando aveva
//     letto il file, senza guardare se sul disco c'era ancora quel file);
//   - una modifica arrivata mentre una ricarica leggeva il documento spariva:
//     la ricarica sostituiva lo stato dopo tre attese, e la modifica era
//     finita nello stato di prima;
//   - i due traslochi (`impacchettaAnni`, `migraAnni`) scrivevano il documento
//     senza la collezione che non si era riusciti a leggere, e poi mandavano
//     nel cestino l'unica copia — che su una chiavetta è una cancellazione.
//
// Le cartelle sono temporanee e le impostazioni pure: si scrivono prima di
// importare lo strato dati, perché lo shim le legge una volta sola.
...
function documento () {
  ...
  return Uri.file(percorso.join(cartella, `anno-${contatore}.registro`))
}
...
describe('accodare solo sul file che si è letto', () => {
  it('due registri sullo stesso documento: il file resta leggibile, e chi salva per ultimo copre', async () => {
    ...
    // Prima B accodava all'offset in cui aveva letto il file, cioè sopra le
    // voci appena accodate da A; e A, al salvataggio dopo, rinominava nel suo
    // indice quelle voci agli offset di prima — dove adesso c'erano i byte di
    // B. Il documento non si riapriva più: «classi.json» rovinata.
  ...
  it('un documento compattato altrove e poi accodato da chi l’aveva letto prima si riapre', async () => {
    ...
    // Prima: «classi.json non si trova dove l'indice dice», o un CRC che non
    // torna — gli offset del vecchio nominavano byte rimessi in fila da altri.
    const voci = vociSulDisco(file)
...
describe('una modifica durante una ricarica', () => {
  it('non si perde', async () => {
    ...
    const file = Uri.file(percorso.join(dati, `${proposto.etichetta.replace(/[/]+/g, '-')}.registro`))
    ...
    // La ricarica parte — è quel che fa l'osservatore quando il file cambia —
    // e mentre legge il documento arriva una modifica. Prima la ricarica
    // sostituiva lo stato con quel che aveva letto, e «II B» spariva.
...
describe('i traslochi non buttano quel che non hanno letto', () => {
   ...
   * esiste. È quel che vede il registro davanti a un segnaposto di OneDrive
   * che non si scarica — la voce è elencata, la lettura fallisce.
   */
  ...
  it('impacchettaAnni: un JSON che non si legge ferma il documento e tiene la cartella', async () => {
    ...
    // Prima: documento scritto senza lezioni, `dati/` nel cestino — e al
    // prossimo avvio il documento c'era già, e non si riprovava più.
    assert.deepEqual(await impacchettaAnni(Uri.file(dati)), [])
    assert.equal(existsSync(percorso.join(dati, '2030-2031.registro')), false)
  ...
  it('impacchettaAnni: anche una copia dello storico che non si legge', async () => {
    ...
    assert.deepEqual(await impacchettaAnni(Uri.file(dati)), [])
    assert.equal(existsSync(percorso.join(dati, '2031-2032.registro')), false)
  ...
  it('migraAnni: un JSON che c’è e non si legge non va nel cestino', async () => {
    ...
    // Prima: gli anni scritti senza lezioni, e `lezioni.json` nel cestino.
...
describe('i reperti medi dello stesso giro', () => {
  ...
  async function annoScritto (voci, versione = 3) {
    const file = documento()
  ...
  it('una collezione con il blocco rovinato non ferma l’apertura, e alla modifica si mette da parte intera', async () => {
    ...
    // Si guasta il CRC di `classi.json` nell'indice: il blocco non passa più
    // il controllo, com'è per un settore rovinato o una sincronizzazione a metà.
  ...
  it('lo storico a gradini tiene le ultime dieci, una per giorno del mese e una per settimana oltre', async () => {
    ...
    // Venti copie di oggi, a un minuto l'una dall'altra, e una al giorno per
    // i cento giorni prima.
    ...
    // Oltre il mese, una per settimana: fra i giorni da 40 a 100 ne restano
    // una decina, non sessanta.

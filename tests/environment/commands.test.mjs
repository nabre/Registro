...
// registrati.
//
// La parte che conta è la sequenza di `panels/projection.ts`: rivelare la
...
// posto, misura, schermo intero — e una cartella condivisa con le altre prove
// vorrebbe dire aprire un pannello già a schermo intero perché un'altra prova,
// in un altro file, ce l'aveva messo.
...
describe('la mappa dei comandi', () => {
  ...
  it('un comando sconosciuto non fa cadere l’azione che lo stava usando', async () => {
    // `startup.ts` lo invoca *dopo* aver esportato il CSV: far fallire

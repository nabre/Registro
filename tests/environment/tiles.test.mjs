// La cache dei tasselli della mappa.
//
// Due guasti che si vedevano solo dopo. Un proxy di scuola che risponde 200 con
// la sua pagina di accesso in HTML finiva in cache come se fosse un PNG, e la
// cache non scadeva mai: un tassello rotto per sempre, anche dopo che il proxy
// si era tolto di mezzo. Adesso in cache va solo un'immagine, e un tassello più
// vecchio di una settimana si richiede — servendo il vecchio se la rete non c'è.
...
import { importaSorgente } from '../data/bundleDiProva.mjs'
...
const { tassello } = await importaSorgente('shell/protocol/tiles.ts', {
  electron: fileURLToPath(new URL('./fakeElectronNet.mjs', import.meta.url)),
})

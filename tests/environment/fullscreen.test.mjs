...
// finestra.
//
// `panels/projection.ts` rivela la proiezione e subito dopo chiede lo schermo
// intero. Su Windows il fuoco arriva quando il sistema ha tempo: se è ancora sul
// registro, e il comando si fida del fuoco, sul proiettore finiscono i voti e le
// note. Con l'id dei `webContents` il comando va sulla finestra nominata e su
// nessun'altra.
...
describe('apparato.schermoIntero con la finestra nominata', () => {
  ...
  it('va sulla proiezione anche se il fuoco è ancora sul registro', async () => {
    ...
    // Il fuoco non si è ancora spostato: è il caso che prima mandava il
    // registro sul proiettore.

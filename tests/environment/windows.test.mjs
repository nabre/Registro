// Le finestre dello shim, usate come le usa `panels/panel.ts`: costruire con
// `localResourceRoots`, assegnare `webview.html`, comporre gli indirizzi con
// `asWebviewUri`, ascoltare i messaggi, rispingere lo stato, aggiornare le
...
describe('la pagina viene dal protocollo, non da un data: URL', () => {
  ...
  it('dà alla policy un’origine che combacia', () => {
    ...
    // `panels/panel.ts` scrive `img-src ${cspSource} data:`: deve coprire gli
    // indirizzi che compone, o la finestra resta bianca.
...
describe('gli indirizzi delle risorse', () => {
  ...
  it('le radici concesse seguono l’anno quando cambia', () => {
    ...
    // Quel che fa `PannelloRegistro.aggiornaRisorse()`.
  ...
  it('anche i disegni di resources/, prima di qualunque pannello', () => {
    // Splash e benvenuto mostrano logo e fascia senza pannelli aperti: la radice
    // serve, o il protocollo risponde 403.
    const concesse = radiciConcesse().map((radice) => radice.fsPath)
    assert.ok(concesse.includes(percorso.join(RADICE, 'resources')), concesse.join(' · '))
  })
})
...
describe('i messaggi', () => {
  it('aspetta che la pagina sia arrivata prima di consegnare', async () => {
    ...
    // `panels/projection.ts` spinge il primo contenuto nel costruttore: non si
    // butta, o lo schermo della classe resta vuoto.

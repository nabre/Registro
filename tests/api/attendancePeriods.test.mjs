// `corso.presenze` raggruppata per periodo: il semestre come forma normale.
//
// Il guasto in una riga: una busta che dice «il 23% di assenza» sull'anno
// nasconde che al primo semestre era l'11% e al secondo il 50% — e il
// peggioramento è l'unica cosa per cui quella cifra si guarda. Il registro
// raggruppa le valutazioni per semestre, le pagelle si compilano per semestre,
// e questa è la lettura da cui una pagella si compila: un numero solo spalmato
// su dieci mesi è giusto e risponde a una domanda che nessuno ha fatto.
//
// Il corredo è costruito apposta perché i due semestri **non si assomiglino**:
//
//   - durano diverso (settembre-ottobre contro novembre), quindi i due
//     `udPreviste` sono due numeri diversi e la loro somma è il totale;
//   - le assenze di Rossi stanno quasi tutte nel secondo, quindi la quota di
//     periodo e quella d'anno non possono coincidere per caso;
//   - i voti sono uno nel primo e due nel secondo, quindi la media d'anno
//     **non** è la media delle due medie — che è la «semplificazione» che
//     qualcuno farebbe domani credendo di ripulire.
//
// Se un giorno i conti per periodo venissero ricavati dividendo il totale,
// queste prove cadrebbero invece di passare per caso. È il punto.
...
/**
 * Il periodo che si chiede quasi sempre qui: due mesi nel primo semestre e uno
 * nel secondo.
 *
 * Tagliato così perché il secondo semestre vero arriva a giugno, e chiederlo
 * intero vorrebbe dire contare a mano trentacinque martedì per sapere che cosa
 * aspettarsi. Qui i martedì sono nove nel primo pezzo e quattro nel secondo:
 * due denominatori diversi che si sommano, che è esattamente il caso da
 * provare.
 */
...
before(async () => {
  ...
  // Il confine passato per esteso: senza, `creaAnno` ripiega su fine gennaio e
  // i due semestri verrebbero quasi uguali — cioè proprio il caso in cui una
  // divisione per due del totale passerebbe inosservata.
  await archivio.creaAnno(
    creaAnno(ANNO_DAL, ANNO_AL, undefined, CONFINE),
  ...
  // Due ore nel primo semestre e due nel secondo: le stesse quantità, così che
  // la differenza fra i due periodi venga solo da dove stanno le assenze e da
  // quanto l'orario prevedeva.
  ...
  // Rossi: una sola assenza nel primo semestre, tutte e due le ore del secondo.
  // Due UD su diciotto previste contro quattro su otto — l'11% contro il 50%.
  ...
  // Bianchi: un ritardo e basta. Il ritardo si conta **per ora e non per UD**,
  // e serve a provare che anche quel conto si spezza fra i periodi invece di
  // restare appiccicato al totale.
  await segna(settembreOtto, bianchi, 'ritardo')

  // Una prova nel primo semestre e due nel secondo, con voti che tirano in due
  // direzioni: la media d'anno pesa ogni prova una volta, la media delle due
  // medie peserebbe ogni semestre una volta, e i due numeri qui non coincidono.
...
describe('corso.presenze: i periodi in cima', () => {
  it('senza semestreId guarda tutti i semestri che l’intervallo tocca', async () => {
    ...
    // Gli estremi **veri**: il primo periodo finisce al confine, il secondo
    // comincia il giorno dopo e finisce dove finisce quel che si è chiesto —
    // non a giugno, che è dove finisce il semestre.
    assert.deepEqual(
  ...
  // La prova che tiene in piedi tutto il resto: se i conti per periodo venissero
  // ricavati dal totale invece che contati a parte, questa passerebbe lo stesso
  // — ma quelle sui denominatori e sulla media no. Qui si guarda il verso
  // opposto: che i pezzi contati a parte rifacciano il totale.
...
describe('corso.presenze: i denominatori per periodo', () => {
  // Il conto che non si può ricavare dividendo: `udPrevisteDaOrario` si chiama
  // una volta per periodo, con gli estremi di quel periodo. È il denominatore
  // che finisce sui rapporti stampati, e due semestri di lunghezza diversa
  // danno due numeri diversi — non due metà.
  it('due periodi di lunghezza diversa danno due udPreviste diversi, che si sommano', async () => {
    ...
    // E la metà del totale non è nessuno dei due: una divisione per due
    // passerebbe di qui e sbaglierebbe tutte e due le quote.
    assert.notEqual(primo.udPreviste, esito.dati.udPreviste / 2)
  })

  // Il caso per cui la modifica esiste. Le quote si ricalcolano sul
  // denominatore del loro periodo, e non si mediano fra periodi.
  it('la quota di un periodo non è quella dell’anno, e la differenza è tutto', async () => {
    ...
    // Quattro su otto: il 50%. Sul conto d'anno questo diventa il 23%, e chi
    // legge quella cifra sola non ha modo di sapere che è tutta di qui.
    ...
    // E non è nemmeno la media delle due: una media di quote pesa un semestre
    // corto come uno lungo.
  ...
  it('anche la presenza si ricalcola sul suo denominatore', async () => {
    ...
    // Presenza: due UD su quattro con l'appello fatto nel primo, zero su
    // quattro nel secondo. Sono due domande sul periodo, non due metà.
  ...
  it('la media d’anno non è la media delle medie di semestre', async () => {
    ...
    // Tre prove d'anno, ciascuna con il suo peso: (5 + 3 + 3) / 3.
    assert.equal(riga.prove, 3)
    assert.equal(riga.media, 3.67)
    // La media delle due medie farebbe 4: mezzo punto di differenza, e nessuna
    // delle due cifre sarebbe segnalata come sbagliata da niente.
  ...
  it('chi non ha voti in un periodo ha media nulla lì, non zero', async () => {
    ...
    // Nulla e non zero: una media di zero voti non è zero, e uno zero scritto
    // qui finirebbe in pagella come un'insufficienza.
...
describe('corso.presenze: un periodo solo', () => {
  it('con semestreId risponde su quel semestre e basta', async () => {
    ...
    // Le stesse cifre che la busta senza filtro dava per quel periodo: il
    // filtro restringe, non ricalcola in un altro modo.
  ...
  it('un semestreId inventato è un errore, non una busta vuota', async () => {
    // Una busta buona con zero assenze si rilegge come «non ne ha», che è
    // falso e detto con sicurezza: un id che non è di quest'anno va detto, con
    // il posto dove si trovano quelli veri.
...
describe('corso.presenze: il periodo tagliato e l’anno senza semestri', () => {
  it('un dal a metà semestre taglia il periodo, e la busta dichiara il taglio', async () => {
    ...
    const [primo, secondo] = esito.dati.periodi
    // Il `dal` chiesto e non quello del semestre. Dichiarare l'inizio del
    // semestre metterebbe accanto a sei settimane di assenze un denominatore
    // di due mesi, e la quota uscirebbe più bassa del vero senza che nessuno
    // che la legge possa accorgersene.
    ...
    // Sette martedì da metà settembre a fine ottobre, contro i nove interi.
    ...
    // E l'assenza del primo settembre è fuori dal taglio: il periodo che la
    // conteneva adesso non la contiene, e la quota va a zero invece di
    // restare quella di prima su un denominatore più corto.
    const riga = rigaDi(esito.dati, rossi.id)
  ...
  it('un anno senza semestri dà comunque un periodo, con le cifre del totale', async () => {
    // Il raggruppamento non sparisce mai: resta un array di uno, così chi legge
    // la busta non deve scrivere due strade. Un anno senza semestri non è un
    // registro valido — `validation.ts` lo rifiuta — ma arriva da un file
    // riparato a mano o da una migrazione a metà, e allora si risponde.

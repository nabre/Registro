...
name: sciame
description: >
  Come si conduce un «giro di sciame» sul Registro docenti — il modo di lavorare
  ...
  registra tutto nel cantiere. Da usare quando si chiede di cercare difetti su
...
Il progetto ha già fatto tre giri, e sono scritti in `docs/CANTIERE.md` § 3.
...
1. **Il contesto del progetto in quattro righe** — Electron, TypeScript, nomi
   italiani, i cinque strati, il documento `.registro`. Un agente che non sa
...
2. `docs/CANTIERE.md`: il giro nel registro dei giri — quante dimensioni, quanti
   reperti, quanti alti, che cosa **non** era un difetto — e le pendenze nei

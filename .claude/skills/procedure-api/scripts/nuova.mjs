...
const righeImport = [
  ...
  ...(genere === 'scrittura'
    // Una scrittura nasce con `scrittura({…})`, che mette da sé genere, uscita
    // e versione. `inoltra` solo quando c'è un gestore a cui passare la palla:
    // importarlo e non usarlo è un errore di stile, e un file nuovo che non
    // passa `lint` insegna a dare quel comando più tardi.
    ? [`import { ${azione ? 'inoltra, ' : ''}scrittura } from '${versoApi}core.js'`]
...
const corpoScrittura = azione
  ? [
      '  // L’ingresso è l’azione senza il suo `tipo`: finché lo schema qui sopra non',
      '  // ne dichiara i campi, questa riga non compila. Se un campo va travasato —',
      '  // rinominato, calcolato — si torna a `daGestore` e lo si compone a mano.',
      `  esegui: inoltra(${cartelle[0]}, '${azione}'),`,
    ]
  : [
...
const testo = [
  ...
  '',
  ...(genere === 'scrittura'
    ? ['export const procedura = scrittura({', `  nome: '${nome}',`]
    : [

...
description: >
  ...
  (`src/api/procedures/`), toccando ogni punto che la riguarda: lo
  ...
  JSON-RPC, attrezzi per l'assistente, `regi catalogo`, o si aggiunge,
...
più di due — il pannello, il condotto, il menu nativo, la riga di comando, il
modello locale — e un tipo TypeScript sparisce quando il programma
gira.

Prima di toccare qualcosa, leggi la procedura più vicina a quella che ti serve:
ce ne sono quasi duecento, e una che somiglia alla tua esiste quasi sempre.
...
```
src/api/procedures/
├── common/              quel che più aree si dividono, per file d'origine
│   └── register.ts      esigiAnno, esigiMateria, esigiCorso, esigiClasse
├── ore/
│   ├── common.ts        quel che le procedure di `ore` si dividono
│   ├── index.ts         procedureOre: i file suoi e gli indici sotto
│   ├── salva.ts         ore.salva
│   └── appello/
│       ├── index.ts     procedureOreAppello
...
- **`common.ts` per quel che si divide.** Una guardia usata da una procedura sola
...
| `tests/api/coverage.test.mjs` | il conto delle azioni | azione nuova o tolta |
| `docs/API.md` · `docs/INDICE.md` · `README.md` | i conti e le tabelle — `tests/counts.test.mjs` li confronta con quelli veri | sempre |
...
5. **Passa la palla.** Se un gestore esiste, `inoltra(gestori, 'tipo')`: il
   lavoro sta in un posto solo, come prima. Non riscriverlo qui. Se un campo
   va travasato prima di arrivare al gestore, `daGestore` — vedi «Le due forme»
   qui sotto.
6. **Scrivi la prova.** Una scrittura in `tests/api/writes.test.mjs`, una
   lettura in `reads.test.mjs`: una lettura nuova va aggiunta anche a
   `leTutte()`, e va alzato il conto delle letture scritto nella stessa prova.
   Che cosa deve provare sta in [references/prove.md](references/prove.md).
...
Una procedura si dichiara in uno di due modi, e `npm run procedures` li
riconosce tutti e due:

- **`scrittura({…})`** — quasi tutte le scritture. È `definisci` con
  `genere: 'scrittura'`, `uscita: SCRITTURA` e `versione: 1` già messi
  (`src/api/core.ts`); `versione` si può ancora scrivere, e vince.
- **`definisci({…})`** — le letture, e le scritture che tornano altro che la
  busta di `SCRITTURA`. Qui genere, versione e uscita si scrivono per esteso.

E due modi di passare la palla al gestore:

- **`esegui: inoltra(registro, 'anno.crea')`** — l'ingresso è l'azione senza il
  suo `tipo`. Il controllo è quello di prima: se lo schema non dichiara un campo
  obbligatorio dell'azione, o lo dichiara di un altro tipo, non compila. Dopo
  le guardie: `return inoltra(g, 'tipo')(ambito, ingresso)`.
- **`daGestore(g['tipo'], (i) => ({ tipo: 'tipo' as const, … }))`** — quando
  l'azione va composta a mano: un campo rinominato, un valore convertito
  (`smistamento.pdf.*` e `comeDivisione`).

Se prende in carico un'azione che **non esiste ancora**, prima va dichiarata in
`src/protocol.ts`, poi il gestore in `src/actions/<area>.ts`, e **poi** il
...
- [references/prove.md](references/prove.md) — che cosa deve provare la prova di
  una procedura, e i file di `tests/api/` che la riguardano.
- [references/llm.md](references/llm.md) — `resources/tools.json`, il comando
  `regi catalogo`, e perché al modello si danno solo le letture.

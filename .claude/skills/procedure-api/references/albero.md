...
| `src/api/core.ts` | `chiama()`, l'elenco, `registra`, `osserva`, `SCRITTURA`, `scrittura`, `inoltra`, `daGestore`, `aEsitoAzione`, `descrivi` | quasi mai |
...
| `src/api/procedures/<cartella>/index.ts` | `procedure<Cartella>`: i file suoi e le cartelle sotto | sempre |
| `src/api/procedures/<area>/common.ts` | guardie, elenchi di valori, pezzi di schema che più procedure dell'area si dividono | quando una cosa serve a due |
...
Un aiuto che serve a **due aree** va in `procedures/common/`, non nell'area di una
...
| `README.md`, `docs/GUIDA.md` | solo se cambia com'è fatto: file, dati, build, condotto. Quel che si vede lo racconta la guida in-app, `src/ui/views/help/` |

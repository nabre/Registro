...
| `scelte` | le risposte sono poche e note | dogana **e** elenco della tendina. L'`aiuto` di ogni scelta è anche la sua etichetta: va scritto con il nome della scelta davanti |
...
| `nascosta` | è **stato scritto dal programma**, non una scelta da offrire | fuori da tutte e due le superfici, ma `valoreAccettabile` la accetta ancora — è di lì che il widget la scrive |
...
**`nascosta` è l'unica uscita dichiarata** dalla garanzia «ogni chiave compare da
qualche parte»: usarla per far sparire una scelta vera è esattamente il guasto
che quella garanzia esiste per impedire. Se una chiave nascosta ha bisogno di un
gesto, il gesto va offerto — l'agenda ha «Rimetti l'agenda dov'è nata», che
ritira le sue cinque chiavi prese da `chiaviNascoste()`, non da un elenco
riscritto a mano.
...
Chi rifiuta deve **dirlo**: `valoreConMotivo()` torna anche la ragione, e tutte e
due le superfici la mostrano. Un valore rifiutato in silenzio lascia il campo

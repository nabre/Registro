...
description: >
  ...
  `procedures`) — che cosa guarda ognuno, come si legge la sua uscita, che cosa
...
| `npm run collections` | che ogni `modifica()` dichiari i JSON che riscrive | una collezione toccata non è dichiarata |
...
4. **È un artefatto generato?** Tre file non si scrivono a mano:
   ...
   `tests/samples/2026-2027.registro` (`npm run sample`).
...
```sh
npx tsc --noEmit && npx eslint . && npm test && npm run layers && npm run collections && npm run forms && npm run buttons && npm run procedures && npm run census
```

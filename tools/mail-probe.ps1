...
.DESCRIPTION
  ...
  La 3 è quella che conta: stesso client id e stesso permesso del registro.
  Un sì alla 3: il tenant lascia agli utenti il consenso per le applicazioni
  di Microsoft. Un sì alla 5: si può registrare un'applicazione propria. Un NO
  dappertutto con «SmtpClientAuthentication is disabled»: la consegna SMTP è
  spenta sulla casella, e la riaccende solo un amministratore.

  Qui si entra con il *device code*, mentre il registro apre il browser: la
  sonda dice se il permesso c'è, e un tenant che blocca il device code la fa
...
# Quel che il registro chiede con la consegna «graph»: anche Mail.ReadWrite,
# perché la bozza nasce nella casella. Un tenant può dare Mail.Send e negare
# Mail.ReadWrite, quindi si prova così com'è.

...
.DESCRIPTION
  ...
  Le vie provate:
    1  VS Code (client di Microsoft)      → SMTP.Send
    3  Microsoft Graph PowerShell         → SMTP.Send            [quella del registro]
    5  ID applicazione proprio (-ClientId) → SMTP.Send
    6  Password (-ProvaPassword)          → AUTH LOGIN su smtp.office365.com

  La 3 è quella che conta: è esattamente il client id e il permesso con cui il
  registro si collega adesso. Un sì lì e non c'è altro da fare.

  Un sì alla 3 vuol dire che il tenant lascia il consenso agli utenti per le
  applicazioni pubblicate da Microsoft. Un sì alla 5 vuol dire che si può
  registrare un'applicazione propria. Un NO dappertutto con
  «SmtpClientAuthentication is disabled» vuol dire che la consegna SMTP è
  spenta sulla casella, e la riaccende solo un amministratore.

  Attenzione: qui si entra con il *device code*, che il registro non usa più —
  lui apre il browser. Serve a sapere se il permesso c'è, non a riprodurre il
  giro dell'applicazione: un tenant che blocca il device code e lascia passare
  il browser fa fallire questa sonda e funzionare il registro.
...
# Quel che il registro chiede davvero con la consegna «graph»: Mail.ReadWrite in più,
# perche' la bozza deve nascere dentro la casella e non solo partire. Va provato cosi'
# com'e': un tenant puo' dare Mail.Send e negare Mail.ReadWrite, e saperlo dopo vuol
# dire scoprirlo al primo giro di comunicazioni.
$SCOPO_REGISTRO = 'https://graph.microsoft.com/Mail.Send https://graph.microsoft.com/Mail.ReadWrite offline_access openid profile'

/**
 * Verifica che l'albero delle procedure stia in piedi: che il percorso di ogni
 * file sia il nome della procedura che contiene, che ogni file sia nominato
 * nell'indice della sua cartella, e che ogni cartella arrivi fino a
 * `src/api/index.ts`.
 *
 * È il controllo che tiene insieme «il file c'è» e «la procedura esiste». Una
 * procedura che non arriva all'indice non si registra, e non si registra **in
 * silenzio**: il nucleo non la conosce, il ponte non la innesta, il condotto
 * risponde «procedura sconosciuta» — e il file è lì, scritto giusto, con dentro
 * il lavoro fatto. Fra tutti i modi di sbagliare una procedura nuova questo è
 * l'unico che non lasci traccia da nessuna parte.
 *
 * Il percorso *è* l'indirizzo: `ore.appello.casella` sta in
 * `src/api/procedures/ore/appello/casella.ts`. Con questa regola una procedura
 * si trova senza cercarla e si cancella senza dimenticarne un pezzo; senza,
 * l'albero torna a essere un posto in cui si mettono i file.
 *
 * Legge il testo e non compila: serve anche — e soprattutto — quando `tsc` non
 * passa, che è il momento in cui si ha più bisogno di sapere dove si è
 * sbagliato. I casi che non sa leggere li dice invece di tacerli.
 *
...
/** Il `nome:` dichiarato dentro `definisci({...})`, se c'è. */
function nomeDichiarato (testo) {
  const dentro = /export const procedura = definisci\(\{([\s\S]*?)\n\}\)/.exec(testo)
...
for (const f of file) {
  ...
  if (NON_PROCEDURE.has(ultimo) || dentroComuni) continue
  ...
  if (!nome) {
    male(
      ...
      '`export const procedura = definisci({…})` — o si chiama `index.ts` o `common.ts`.',
    )
    continue
  }

  // Il percorso è l'indirizzo: `a.b.c` sta in `a/b/c.ts`, e non altrove. Senza
  // questa regola trovare una procedura torna a essere una ricerca nel testo.
  ...
  // Più di una procedura in un file: l'indice ne vedrebbe una sola, e l'altra
  // sparirebbe senza che niente lo dica.
  const quante = (f.testo.match(/^export const procedura = definisci\(/gm) ?? []).length
...
// Il confronto vero lo fa `tests/api/tools.test.mjs`, che il catalogo lo
// ricostruisce davvero. Qui basta il conto: è quel che si sbaglia — si aggiunge
// una procedura e si dimentica `npm run tools` — e si vede senza compilare.
try {

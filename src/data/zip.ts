 ...
 * recuperare a mano il giorno che serve. Vale anche per quel che è già
 * compresso di suo — un PDF fatto di immagini, un'immagine — dove `deflate`
 * lavorerebbe per niente.
 */
...
/**
 * Una voce che sta già dentro un archivio, con il posto che occupa.
 *
 * L'offset è quel che permette di riscrivere l'indice senza toccare i dati: una
 * voce che non cambia resta esattamente dov'è, byte per byte, e nell'indice
 * nuovo si rimette il numero che aveva.
 */
...
/**
 * I corpi delle voci, uno dopo l'altro, a partire da un offset dato.
 *
 * Torna anche dove ognuna è finita: è quel che serve all'indice, e a chi
 * riscriverà l'indice la volta dopo senza rileggere il file.
 */
...
/**
 * L'indice: la lista di tutte le voci con il loro posto, e la coda che dice
 * dove comincia la lista.
 *
 * Sono le uniche due cose che un archivio deve riscrivere quando cambia
 * qualcosa. Tutto il resto — i corpi già scritti — resta dov'è.
 */
export function indice (collocate: VoceCollocata[], da: number): Buffer {
  if (collocate.length > 0xffff) {
    throw new ErroreZip(`troppe voci per un archivio semplice: ${collocate.length}`)
  }
...
/**
 * Mette insieme un archivio intero da blocchi già pronti.
 *
 * Qui non si comprime niente: si scrivono corpi, indice e coda, e si concatena.
 * È la via della riscrittura completa — quella che si usa per un documento
 * nuovo, e per la compattazione.
 */
...
/**
 * Che cosa aggiungere in fondo a un archivio perché diventi quello nuovo.
 *
 * È il cuore della scrittura incrementale, e vale la pena dire perché funziona.
 * Uno ZIP si legge dalla fine: chi lo apre cerca la coda, la coda dice dove sta
 * l'indice, l'indice dice dove stanno le voci. Le voci che non cambiano restano
 * dove sono — non si rileggono e non si riscrivono — e in fondo al file si
 * accodano soltanto le voci nuove, l'indice completo e la coda.
 *
 * Da qui vengono due proprietà che si pagherebbero care in altro modo:
 *
 *   - la scrittura è quanto la modifica, non quanto il documento. Cambiare una
 *     collezione dentro un anno da otto megabyte scrive dieci kilobyte;
 *   - i byte di prima non si toccano mai, e chi sincronizza la cartella se ne
 *     accorge: cambia solo la coda del file.
 *
 * E una terza, la più importante: l'archivio di prima resta valido fino
 * all'ultimo byte scritto. La coda nuova è l'ultima cosa che si scrive, e
 * finché non c'è vale ancora quella vecchia — che punta a un indice vecchio,
 * che punta a voci vecchie, tutte ancora al loro posto. Un salvataggio
 * interrotto a metà lascia un documento buono, senza bisogno di scrivere una
 * copia intera per poi rinominarla.
 *
 * Il prezzo è lo spazio: la vecchia versione di una voce resta nel file, e non
 * la nomina più nessuno. Vedi `spazioMorto`, e chi la compatta.
 */
export function daAccodare (
  ...
  // I due pezzi tornano separati e non concatenati, e non è un dettaglio: la
  // garanzia descritta qui sopra vale solo se la coda arriva sul disco *dopo*
  // i corpi. Con una scrittura sola il sistema può mettere giù le pagine
  // nell'ordine che gli fa comodo, e un arresto fra l'una e l'altra
  // lascerebbe un archivio con una coda nuova che nomina corpi mai scritti —
  // cioè esattamente il documento rotto che tutto questo evita.
  return { corpiNuovi, coda: indice(tutte, inizioIndice), da: fineFile, collocate: tutte }
}

/**
 * Quanto di un archivio non serve più a nessuno: le versioni vecchie delle
 * voci, e gli indici che si sono lasciati dietro.
 *
 * È la somma di quel che c'è nel file meno quel che l'indice nomina. Quando
 * cresce troppo si riscrive il documento da capo — vedi `assembla` — e torna a
 * zero.
 */
 ...
 *
 * Normalmente sta negli ultimi 22 byte, e la si trova subito. La ricerca però
 * non si ferma ai 64 KB del commento ammesso dal formato, e la ragione è la
 * scrittura incrementale: un salvataggio interrotto lascia in fondo al file dei
 * corpi scritti a metà, senza la coda che li nomina. Sotto quei resti c'è
 * ancora la coda di prima — intatta, con il suo indice e le sue voci tutte al
 * loro posto — e trovarla vuol dire riaprire il documento di prima invece di
 * dichiararlo rotto.
 *
 * Una firma trovata per caso dentro il corpo di una voce non basta: si accetta
 * solo una coda che dichiari un indice dove un indice c'è davvero.
 */
function inizioCoda (dati: Buffer): number {
  for (let i = dati.length - CODA; i >= 0; i -= 1) {
    ...
    if (quante === 0) {
      // Un archivio vuoto: l'indice comincia dove comincia la coda, e non c'è
      // nessuna voce da verificare.
      if (dove === i) return i
      continue
    }
...
interface ZipAperto {
  voci: VoceLetta[]
  /**
   * Dove comincia l'indice: è anche il punto oltre il quale, in un archivio
   * scritto da noi, non c'è più niente che serva. Chi accoda riparte dalla fine
   * del file — non da qui — perché l'indice di prima deve restare leggibile
   * finché il nuovo non è scritto per intero.
   */
  inizioIndice: number

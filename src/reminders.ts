...
//
// Sta in mezzo fra due pezzi che non si conoscono: `domain/reminders.ts`, che
// sa *che cosa* dire di un'ora e non sa che ore sono, e `environment/notifications.ts`,
...
//
// **Una sola per ora.** Il battito passa ogni mezzo minuto — vedi `BATTITO` —
// e la finestra in cui un'ora è annunciabile dura molti battiti: senza memoria,
// la stessa lezione verrebbe annunciata a ogni giro. `annunciate` è quella
// memoria.
//
 ...
 *
 * `apri` è quel che succede premendo la notifica: lo passa `startup.ts`, perché è

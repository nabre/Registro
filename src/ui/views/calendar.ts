...
import { confineAnno, letteraSettimana } from '../../domain/years.js'
...
import { eseguiOAvvisa, statoVuotoAnno } from '../components/filters.js'
...
import { chiediEliminazione, moduloAssegnaPiano, moduloAvvio, moduloLezione } from '../forms.js'
...
/**
 * L'altezza *minima* del corpo della settimana, non la sua altezza.
 *
 * La griglia si allunga fino a riempire la finestra — i blocchi stanno in
 * percentuale della fascia oraria — e questa misura dice soltanto sotto quale
 * altezza smette di essere leggibile: lì si ferma, e a scorrere torna la
 * pagina. Serve anche a decidere quante righe scrivere dentro un blocco, che è
 * una domanda sul caso peggiore e non sull'altezza vera.
 */
 ...
 *
 * Stava sul blocco che si trascina, ed era il posto sbagliato: un ridisegno
 * durante il trascinamento — ne basta uno, e il calendario si ridisegna a ogni
 * battito dell'orologio — butta via quell'elemento insieme al suo `dragend`.
 * Nessuno azzerava più `trascinata`, e le colonne restavano accese con la
 * classe `.zona-posa`: cioè non più cliccabili, senza che si capisse perché.
 *
 * Su `document` il gesto finisce comunque, come già fa `views/pageBrowser.ts`.
 */
...
async function posa (lezione: Lezione, data: Iso, inizio: string | undefined, copia: boolean): Promise<void> {
  ...
  const proposta: Lezione = {
    ...lezione,
    // Copiando, quella che nasce è un'altra lezione, e con l'id dell'originale
    // `lezioniSovrapposte` lo scarta come se fosse se stessa: la copia lasciata
    // sopra l'originale non risultava sovrapposta a niente e spariva sotto di
    // lui, in silenzio. Spostando invece l'id giusto è il suo — una lezione non
    // si sovrappone a se stessa.
    id: copia ? '' : lezione.id,
    data,
...
function guida (
  ...
  // Dalla frazione della colonna vera, come fa `oraSotto` per decidere dove la
  // lezione cade. Prima la guida si posizionava moltiplicando i minuti per la
  // scala calcolata prima del disegno: su una finestra alta la colonna si
  // allunga per riempirla, le due misure divergono, e la riga indicava
  // un'ora — anche un'ora e mezza — diversa da quella in cui la lezione
  // sarebbe finita. La riga serve proprio a dire dove finisce.
...
/**
 * Il semestre che finisce in questo giorno, ma solo se non è l'ultimo.
 *
 * Nella striscia delle settimane il confine da vedere è uno solo, ed è questo:
 * la fine dell'ultimo semestre è la fine dell'anno, cioè il bordo della
 * striscia stessa, e segnarla vorrebbe dire disegnare una riga per dire che
 * dopo l'ultima settimana non ce n'è un'altra.
 */
function chiudeSemestreDiMezzo (data: Iso): Semestre | null {
  ...
  // l'ordine dell'elenco è quello in cui i semestri sono stati scritti.
  ...
  menuContestuale(evento, [
    {
      ...
      al: () =>
        moduloLezione({
          data,
          // Il corso del filtro, quando c'è: si sta guardando la settimana di
          // quel corso, e l'ora che si aggiunge da qui è quasi sempre sua.
...
function dettagliDellaLezione (lezione: Lezione, inizio: string, fine: string): string {
  ...
  // Tre cose diverse, e si rimediano in tre modi: il piano che non c'è si
  // sceglie, quello vuoto si apre e si riempie, quello corto si allunga. Dire
  // «senza piano» per tutti e tre costringerebbe ad aprire l'ora per sapere
  // quale dei tre è — e i minuti scoperti sono la cosa che si vuole sapere
  // guardando il calendario la sera prima.
  const piano = pianoPerId(lezione.pianoId)
  const scaletta = scalettaDiLezione(lezione)
  const scarto = piano ? minutiDiScarto(piano, lezione) : 0
  const manca = !piano
    ? 'senza piano'
    : scaletta.length === 0
 ...
 *
 * Posizione e altezza in percentuale della fascia oraria, non in pixel: la
 * griglia si allunga fino a riempire la finestra, e un blocco fissato in pixel
 * resterebbe dov'era mentre la colonna sotto di lui cresce.
 *
 * I pixel restano per decidere *che cosa scrivere dentro* — il numero
 * dell'ora, le tappe, il piede. Quella misura è calcolata sulla scala minima,
 * cioè sul caso peggiore: se una riga ci sta lì, ci sta a maggior ragione
 * quando la griglia è più alta.
 */
function bloccoLezione (
  lezione: Lezione,
    ...
    lezione.slot
      ...
      .map((pausa) =>
        h('span', {
          class: 'blocco__pausa',
          style: {
            // Dentro il blocco, e quindi in percentuale di lui: la pausa resta
            // al suo posto comunque il blocco venga stirato.
...
/*
 * I compleanni sul calendario.
 *
 * Non sono lezioni e non devono sembrarlo: non hanno un'ora, non si spostano
 * trascinandole, non si aprono come un'ora del registro. Portano una torta,
 * che è il segno che si riconosce senza leggere, e cliccandoli si va dove ha
 * senso andare — la scheda della persona, non un'ora che non esiste.
 *
 * Stanno in tutte e tre le viste perché il giorno in cui servono è uno solo
 * all'anno, e capitare proprio quel giorno sulla vista giusta sarebbe un caso.
 * Nel mese e nell'agenda per esteso, nella settimana come segno sull'
 * intestazione del giorno: nella griglia oraria non c'è una riga in cui metterli
 * — un compleanno non comincia alle 08:20 — e appenderli a un'ora a caso
 * vorrebbe dire dire una cosa falsa.
 */

/** Il suggerimento di un giorno di compleanni: chi, quanti anni, di che classe. */
function dettiCompleanni (compleanni: Compleanno[]): string {
  return compleanni.map((festa) => `${fraseCompleanno(festa)} · ${festa.classe}`).join('\n')
}

/**
 * Le classi che quel giorno si vedono davvero: quelle che hanno un'ora in
 * calendario.
 *
 * Serve a distinguere i due compleanni che non sono la stessa cosa. Quello di
 * una classe che si ha in aula quel giorno è una cosa da fare — lo si dice
 * entrando, prima dell'appello — e sta in primo piano. Quello di una classe
 * che quel giorno non si incontra è un'informazione e basta: resta scritto, ma
 * sbiadito, perché non chiede niente a chi guarda l'orario.
 */
function classiInAula (lezioni: Lezione[], data: Iso): Set<string> {
...
function qualcunoInAula (compleanni: Compleanno[], classi: Set<string>): boolean {
...
/**
 * La pastiglia di un compleanno: la torta, il nome, gli anni.
 *
 * Stessa forma delle pastiglie delle ore — nella cella del mese stanno una
 * sotto l'altra, e due forme diverse nella stessa colonna si leggono come due
 * elenchi — ma con il colore della classe sul bordo, che è quel che dice di chi
 * si sta parlando prima ancora di leggere il nome.
 */
function chipCompleanno (compleanno: Compleanno, inAula: boolean): HTMLElement {
  return h(
    'button',
    {
      ...
      attr: {
        title: [
          ...
          compleanno.classe,
          inAula ? null : 'quel giorno non si ha lezione con la sua classe',
        ]
          .filter(Boolean)
          .join(' · '),
      },
    ...
    // Gli anni in coda, dove nelle ore sta la sigla della materia: è il numero
    // che si cerca, e i diciotto sono il motivo per cui qualcuno lo cerca.
 ...
 * sono se sono più d'uno. Il resto lo dice il suggerimento: nell'intestazione
 * c'è spazio per un'icona, non per due nomi e cognomi.
 */
function segnoCompleanni (compleanni: Compleanno[], inAula: boolean): Figlio {
 ...
 * dell'anno, perché di là le medie ripartono. Si segna dove un semestre
 * *finisce*, e solo se dopo ce n'è un altro: la fine dell'ultimo è la fine
 * dell'anno, cioè il bordo della striscia, e una riga lì direbbe soltanto che
 * dopo l'ultima settimana non ce n'è un'altra.
 */
function strisciaSettimane (): Figlio {
  ...
  for (let lunedi = inizioSettimana(anno.inizio); lunedi <= ultima; lunedi = sommaGiorni(lunedi, 7)) {
    ...
    // La settimana in cui un semestre finisce porta il confine, e si segna a
    // destra: è il verso in cui la striscia si legge, e la riga cade dopo
    // l'ultima settimana che conta per quella pagella. L'inizio del semestre
    // dopo è la stessa riga detta due volte — fra l'uno e l'altro non c'è
    // niente — e l'inizio del primo è l'inizio dell'anno, che è già il bordo
    // della striscia.
    const chiude = giorni.map(chiudeSemestreDiMezzo).find(Boolean) ?? null
...
function vistaSettimana (): HTMLElement {
  ...
  return h(
    ...
    h(
      'div',
      // Qui si scorre l'ora del giorno, non la settimana: chi lavora sul
      // pomeriggio ci resta passando alla settimana dopo, e non deve riscendere
      // a ogni spunta. La chiave è ferma per questo. Il mese ha un meccanismo
      // suo — `finestraMese`, sotto — perché lì lo scorrimento allunga la
      // striscia; settimana, agenda e anno passano dal modo comune.
      { class: 'settimana__scorrevole', dataset: { scorrimento: 'calendario:settimana' } },
      h(
        'div',
        // `min-height` in pixel è il pavimento — sotto quella misura le ore non
        // si leggono più, e allora scorre il riquadro qui sopra. Da lì in su
        // comanda `height: 100%`, che sta nel foglio di stile: il corpo riempie
        // la finestra e i blocchi, che stanno in percentuale, lo seguono.
          ...
          ...orePiene.map((minuto) =>
            h(
              'div',
              {
                ...
                style: { top: `${((minuto - primaOra) / (ultimaOra - primaOra)) * 100}%` },
              },
              oraDaMinuti(minuto),
            ),
          ),
        ...
        /** L'ora su cui cade il puntatore dentro questa colonna. */
          const oraSotto = (colonna: HTMLElement, clientY: number, passo: number): number => {
            ...
            // Dalla frazione di colonna e non da una scala in pixel: la colonna
            // adesso si allunga per riempire la finestra, e una scala calcolata
            // prima del disegno direbbe un'ora diversa da quella che si vede.
...
function vistaMese (): HTMLElement {
  ...
  const cella = (data: Iso): HTMLElement => {
    ...
    const feste = compleanniDi(data)
    const inAula = classiInAula(lezioni, data)
    ...
    return h(
      'div',
      {
        ...
        attr: { title: chiusura(data) || null },
        // Il corso filtrato arriva anche da qui, come dagli altri due punti in
        // cui si crea un'ora: chi guarda il mese con un corso solo davanti sta
        // aggiungendo un'ora *di quel* corso, e farglielo riscegliere è una
        // domanda a cui aveva già risposto.
        ondblclick: () =>
          moduloLezione({
            data,
            corsoId: stato.filtroCorsoAgendaId ?? undefined,
      ...
      h(
        'div',
        { class: 'mese__lezioni' },
        // I compleanni sopra le ore: sono pochissimi, e sotto quattro
        // pastiglie di lezione finirebbero sotto il «+2» che tronca l'elenco,
        // cioè non si vedrebbero proprio il giorno in cui servono.
...
function vistaAgenda (): HTMLElement {
  ...
  const tutte = lezioniInAgenda()
    ...
    // scambiavano di posto a ogni ridisegno dell'agenda.
  ...
  // I compleanni dentro il pezzo di anno che si sta scorrendo. Un giorno senza
  // lezioni non comparirebbe nell'agenda, e chi compie gli anni di domenica —
  // o durante le vacanze — sparirebbe proprio dalla vista in cui si legge che
  // cosa viene: il giorno si aggiunge, con la sua torta e niente ore.
  const ultimo = [...perGiorno.keys()].sort().at(-1) ?? stato.data
  const feste = compleanniFra(stato.data, ultimo)
  const giorni = [...new Set([...perGiorno.keys(), ...feste.keys()])].sort()
  ...
  let semestrePrecedente = semestrePerData(giorni[0] ?? stato.data)
    ...
    ...giorni.flatMap((data) => {
      ...
      return [
        ...
        h(
          ...
          // I compleanni fra la testata e le ore: appartengono al giorno, non a
          // un'ora, e in mezzo alle righe dell'orario si leggerebbero come una
          // lezione senza orario.
          compleanni.length > 0
            ? h(
                'div',
...
function vistaAnno (): HTMLElement {
  ...
  // I compleanni dell'anno intero, presi una volta sola: le caselle sono
  // trecentosessantacinque, e ripassare l'anagrafica per ognuna vorrebbe dire
  // farlo trecentosessantacinque volte per disegnare un foglio solo.
  ...
  // In testa alla colonna dei numeri: `#`, il segno che ovunque vuol dire
  // «numero». Prima c'era «g.», che a quella misura sembrava un giorno
  // abbreviato — la riga sotto comincia con «1», e due caratteri e un punto in
  // cima a una colonna di numeri si leggevano come una data mozzata.
  ...
  for (let numero = 1; numero <= 31; numero += 1) {
    righe.push(
      h(
        'div',
        // Il modificatore non è decorazione: distingue le trentun righe dei
        // giorni dalle due d'intestazione, ed è su di loro — e non su quelle —
        // che il foglio sparge l'altezza che avanza.
    ...
    h(
      ...
      nomePausa
        ...
        : corsi.length > 0
          ...
          : '',
      // La torta in coda, dentro la stessa fila: qui la casella è alta una
      // riga e larga cinque caratteri, e il nome non ci sta — chi lo vuole
      // passa sopra, o guarda il mese. Il segno però ci sta, ed è quel che fa
      // alzare gli occhi guardando l'anno intero.
      compleanni.length > 0
        ? icona(
            'torta',
...
function legendaAnno (): HTMLElement {
  ...
  return h(
    ...
    h('span', { class: 'anno-legenda__voce' }, icona('torta'), 'compleanni'),
  )
}
...
export function vistaCalendario (): Figlio {
  ...
  const udSettimana = dellaSettimana.reduce((somma, lezione) => somma + contaUd(lezione), 0)
  const oreSettimana = dellaSettimana.reduce((somma, l) => somma + minutiEffettivi(l), 0)

  return h(
    'div',
    // Il modo finisce nella classe come appiglio per gli stili, e non decide
    // più quanto la pagina si allarga: le pagine sono larghe quanto la finestra,
    // tutte, e la regola che restringeva l'agenda se n'è andata con le quattro
    // misure di `foundations.css`. L'altezza invece la prendono tutte e quattro,
    // e quella catena sta in `calendar.css`.
    { class: ['vista', 'vista--calendario', `vista--calendario-${modo}`] },
    testataVista({
      // Compatta: sopra c'è la barra, che dice già «Calendario» e porta i suoi
      // comandi, e sotto c'è la griglia, che vuole tutta l'altezza che trova.
      compatta: true,
      titolo: 'Calendario',
      sottotitolo,
      // Niente pulsanti qui: «Oggi», le frecce e le quattro modalità sono
      // comandi di questa pagina, e stanno nella riga delle azioni insieme a
      // «Nuova ora» e «Ora da compilare». Erano due file di comandi a tre
      // centimetri l'una dall'altra, e quella che il registro chiama «le azioni
      // di questa pagina» non conteneva le uniche azioni che il calendario
      // avesse davvero.
      // Niente filtri qui: classe e corso stanno nella riga delle scelte della
      // barra, insieme ad anno e periodo — è lì che si cerca «che cosa sto
      // guardando», e averli in due posti voleva dire due tendine per la stessa
      // domanda. Nel contorno resta quel che è un numero e non una scelta.
      contorno: [
        h(
          'div',
          { class: 'sintesi' },
          datoSintetico('lezioni in settimana', String(dellaSettimana.length)),
          datoSintetico('UD in settimana', String(udSettimana)),

...
//
// Due forme, per due posti. Nel todo e nel registro della lezione una riga per
...
import { SIGLA_ASSENTE, elencoVoti, grigliaVoti, leggiCasella } from './grades.js'
...
function campoVotoRecupero (recupero: Recupero): HTMLElement {
  ...
  // Con la sigla dell'assente, come la griglia: adesso la casella la accetta,
  // e l'elenco deve offrirla invece di farla indovinare.
  ...
  const campo = h('input', {
    ...
    value: mostrato,
    placeholder: '—',
    // Come le caselle della griglia: senza un `fuoco` da riconoscere, un
    // ridisegno mentre si batte — e il registro ne fa uno a ogni battito
    // dell'orologio — sostituisce il campo, il fuoco se ne va e con lui le
    // cifre, che non avendo mai perso il fuoco non hanno mai fatto `change`.
    // Il prefisso è suo e non quello della griglia: le due caselle parlano
    // della stessa prova e della stessa persona, e con la stessa chiave il
    // fuoco tornerebbe su quella sbagliata.
    ...
    onchange: async (evento: Event) => {
      ...
      // Lo stesso lettore della griglia dei voti, e non una regola sua.
      // Qui vuoto voleva dire «assente» mentre nella griglia vuol dire
      // «nessun voto»: la stessa casella, nella stessa pagina, con due
      // significati opposti. E la sigla dell'assente — X, a — qui non si
      // poteva nemmeno scrivere. Adesso le due caselle dicono la stessa cosa.
      const letto = leggiCasella(elemento.value)
      if (!letto) {
        // Il valore battuto resta nel campo: rimettere `mostrato` voleva dire
        // sostituire la cifra sbagliata con il voto di prima, e far leggere a
        // chi trascrive un numero che crede di aver appena scritto. Il fuoco,
        // fra l'altro, e' gia' sulla riga dopo quando il lampo arriva. Stesso
        // difetto gia' corretto sulle date in `components/base.ts`.
 ...
 *
 * È la forma per il todo e per il registro della lezione, dove i recuperi di prove

// I testi della pagina «Oggi» (`today.ts`).
// Il titolo è «Oggi» di `parole()`; i nomi delle pagine a cui portano le
...
const it = {
  ...
  saluto: (ora: string) =>
    ora < '12:00' ? 'Buongiorno' : ora < '18:00' ? 'Buon pomeriggio' : 'Buonasera',
  ...
  urgenti: (n: number) => (n === 0 ? 'Nessuna urgente' : plurale(n, 'urgente', 'urgenti')),
  pagineInAttesa: (n: number) => (n === 0 ? 'Niente in attesa' : `${plurale(n, 'pagina', 'pagine')} in attesa`),
  /** Che cosa fa una tessera, detto a chi la legge a voce o si ferma sopra. */
  portaA: (pagina: string) => `Apri «${pagina}»`,

  // ------------------------------------------------------------ le ore di oggi
  leOreDiOggi: 'Le ore di oggi',
  quanteOre: (n: number, dalle: string, alle: string) =>
    `${plurale(n, 'ora', 'ore')}, dalle ${dalle} alle ${alle}`,
  nienteOggi: 'Oggi niente lezioni',
  nienteOggiTesto: 'Il calendario non ha ore per oggi: quelle dei prossimi giorni sono nel calendario.',
  adesso: 'Adesso',
  prossima: 'Prossima',
  apriLOra: (classe: string, inizio: string) => `Apri l’ora di ${classe} delle ${inizio}`,
  ...
  oreDiOggiMinuscolo: 'ore di oggi',

  // ------------------------------------------------------ prossime valutazioni
  prossimeValutazioni: 'Prossime valutazioni',
  nessunaValutazione: 'Nessuna valutazione in vista',
  nessunaValutazioneTesto: 'Quando ne metti una in calendario, compare qui con i giorni che mancano.',
  fra: (giorni: number) =>
...
export const testi = catalogo(it, {
  de: {
    saluto: (ora) => (ora < '12:00' ? 'Guten Morgen' : ora < '18:00' ? 'Guten Tag' : 'Guten Abend'),
    ...
    pagineInAttesa: (n) => (n === 0 ? 'Nichts wartet' : `${plurale(n, 'Seite wartet', 'Seiten warten')}`),
    portaA: (pagina) => `«${pagina}» öffnen`,
    leOreDiOggi: 'Die Stunden von heute',
    quanteOre: (n, dalle, alle) => `${plurale(n, 'Stunde', 'Stunden')}, von ${dalle} bis ${alle}`,
    nienteOggi: 'Heute kein Unterricht',
    nienteOggiTesto: 'Der Kalender hat für heute keine Stunden: die der nächsten Tage findest du im Kalender.',
    adesso: 'Jetzt',
    prossima: 'Als Nächstes',
    apriLOra: (classe, inizio) => `Die Stunde der ${classe} um ${inizio} öffnen`,
    fasi: {
      'in-corso': 'Läuft',
    ...
    nessunaValutazioneTesto: 'Sobald du eine in den Kalender setzt, erscheint sie hier mit den Tagen, die noch bleiben.',
    fra: (giorni) => (giorni === 0 ? 'heute' : giorni === 1 ? 'morgen' : `in ${giorni} Tagen`),
    apriValutazione: (titolo) => `Beurteilung «${titolo}» öffnen`,
    compleanni: 'Geburtstage heute',
  },
  fr: {
    saluto: (ora) => (ora < '12:00' ? 'Bonjour' : ora < '18:00' ? 'Bon après-midi' : 'Bonsoir'),
    ...
    urgenti: (n) => (n === 0 ? 'Aucune urgente' : plurale(n, 'urgente', 'urgentes')),
    pagineInAttesa: (n) => (n === 0 ? 'Rien en attente' : `${plurale(n, 'page', 'pages')} en attente`),
    portaA: (pagina) => `Ouvrir « ${pagina} »`,
    leOreDiOggi: 'Les leçons d’aujourd’hui',
    quanteOre: (n, dalle, alle) => `${plurale(n, 'leçon', 'leçons')}, de ${dalle} à ${alle}`,
    nienteOggi: 'Pas de leçons aujourd’hui',
    nienteOggiTesto: 'Le calendrier n’a aucune leçon aujourd’hui : celles des prochains jours sont au calendrier.',
    adesso: 'Maintenant',
    prossima: 'Ensuite',
    ...
    nessunaValutazioneTesto: 'Dès que tu en mets une au calendrier, elle apparaît ici avec les jours qui restent.',
    fra: (giorni) => (giorni === 0 ? 'aujourd’hui' : giorni === 1 ? 'demain' : `dans ${giorni} jours`),
    apriValutazione: (titolo) => `Ouvrir l’évaluation « ${titolo} »`,
    compleanni: 'Anniversaires du jour',
  },
  en: {
    saluto: (ora) => (ora < '12:00' ? 'Good morning' : ora < '18:00' ? 'Good afternoon' : 'Good evening'),
    ...
    pagineInAttesa: (n) => (n === 0 ? 'Nothing waiting' : `${plurale(n, 'page', 'pages')} waiting`),
    portaA: (pagina) => `Open “${pagina}”`,
    leOreDiOggi: 'Today’s lessons',
    quanteOre: (n, dalle, alle) => `${plurale(n, 'lesson', 'lessons')}, from ${dalle} to ${alle}`,
    nienteOggi: 'No lessons today',
    nienteOggiTesto: 'The calendar has no lessons for today: the coming days are in the calendar.',
    adesso: 'Now',
    prossima: 'Next',
    ...
    nessunaValutazioneTesto: 'When you put one in the calendar, it shows up here with the days left.',
    fra: (giorni) => (giorni === 0 ? 'today' : giorni === 1 ? 'tomorrow' : `in ${giorni} days`),

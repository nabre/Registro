  ...
  return h(
    ...
    h(
      ...
      pulsante({
        ...
        al: async () => {
          // Rimettere le spunte è un gesto che si disfa da sé — basta
          // rispuntare — mentre toglierle cancella in un clic il lavoro di
          // una classe intera, e nessuno si ricorda a memoria chi aveva
          // spuntato. Si chiede, come per ogni altro gesto che toglie.
          const nude = consegna.fatte.filter((f) => !f.file).length
          if (avanzamento.completa && nude > 0) {
            const vai = await conferma({
              titolo: `Togliere ${quanti(nude, { singolare: 'spunta', plurale: 'spunte', genere: 'f' })}?`,
              testo:
...
/**
 * Le consegne del registro della lezione, in una scheda loro.
 *

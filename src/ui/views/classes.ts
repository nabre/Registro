...
import { PERSONE, PIF, Uno, del, quanti } from '../../domain/lexicon.js'
...
function elencoClassi (): HTMLElement {
  ...
  return h(
    ...
    // Senza il «+» che stava qui: «Nuova classe» è nella riga delle azioni,
    // scritto per esteso e nel riquadro «Crea» insieme a «Nuovo corso». Un
    // secondo pulsante di sola icona, dieci centimetri più in basso, era lo
    // stesso gesto detto peggio — e chi lo trovava non trovava «Nuovo corso»,
    // che gli serviva subito dopo.
    h('header', { class: 'elenco-laterale__testata' }, h('h3', null, 'Classi')),
    classi.length === 0
...
function tabellaAllievi (classe: Classe): HTMLElement {
   ...
   *
   * Con `apribile` quel che c'è dentro si preme e apre il programma di posta:
   * è l'elenco da cui si scrive a una persona sola — le comunicazioni alla
   * classe partono dal fascicolo — e l'indirizzo stava lì accanto a sei
   * colonne senza che ci si potesse fare niente se non selezionarlo a mano.
   */
  const cella = (
...
function chiediDuplicaClasse (classe: Classe): void {
  ...
  apriModale({
    ...
    alSalva: async (valori, contesto) => {
      ...
      contesto.chiudi()
      // La si apre solo se è finita nell'anno che si sta guardando. L'anno
      // corrente lo decide il documento aperto e non si cambia da qui:
      // selezionare una classe di un altro anno mostrava una scheda a destra
      // mentre l'elenco e la testata restavano su quest'anno, senza nessuna
      // voce accesa — una pagina che si contraddice.
      const quiDentro = annoCorrente()?.id === annoId
      notifica(
        quiDentro ? 'Classe duplicata.' : `Classe duplicata in ${anni.find((a) => a.id === annoId)?.etichetta ?? 'un altro anno'}.`,
        'successo',
      )
      if (quiDentro && risposta.creato) {
        aggiorna({ vista: 'classi', classeId: risposta.creato.id })
      }
    },
  })
}
...
export function vistaClassi (): Figlio {
  ...
  return h(
    ...
    testataVista({
      ...
      // «Nuova classe» sta nella riga delle azioni. Qui resterebbe lo stesso
      // pulsante due volte nella stessa schermata.
    }),
    h(
      'div',

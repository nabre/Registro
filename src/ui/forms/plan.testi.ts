...
const it = {
  ...
  corsoSparito:
    'Il corso che questo piano citava non esiste più. Scegline un altro qui sopra.',
  piuOre: (ore: number) =>
    `Questa scaletta è usata in ${ore} lezioni: quel che si cambia qui cambia in tutte. ` +
    'Per cambiarne una sola, duplica il piano e adatta la copia.',
  oreConSpunte: (ore: number) =>
    `${ore === 1 ? 'Una lezione ha' : `${ore} lezioni hanno`} già le spunte ` +
  ...
  nessunPianoTesto:
    'I piani si preparano una volta e si riusano su più lezioni.',
  creaUnPiano: 'Crea un piano',
  lezioneDi: (durata: string) => `La lezione è di ${durata}.`,
  attivitaPer: (quante: number, durata: string) =>
    `${quante} attività · ${durata}`,
...
export const testi = catalogo(it, {
  de: {
    ...
    momentiDalleTappe: (momenti, voti) =>
      `Aus diesen Etappen ${
        momenti === 1
          ...
          : `sind ${momenti} Leistungsbeurteilungen`
      } entstanden` +
      (voti > 0
    ...
    attivitaPer: (quante, durata) =>
      `${plurale(quante, 'Aktivität', 'Aktivitäten')} · ${durata}`,
  ...
  fr: {
    ...
    momentiDalleTappe: (momenti, voti) =>
      `${
        momenti === 1
          ...
          : `${momenti} évaluations sont nées`
      } de ces étapes` +
      (voti > 0
        ? `, avec ${plurale(voti, 'note déjà saisie', 'notes déjà saisies')}`
        : '') +
    ...
    aiutoEtichette:
      'Séparés par une virgule : ils servent à retrouver le plan.',
    aiutoNote:
    ...
    nessunPianoTesto:
      'Un plan se prépare une fois et se réutilise sur plusieurs leçons.',
    creaUnPiano: 'Créer un plan',
    lezioneDi: (durata) => `La leçon dure ${durata}.`,
    attivitaPer: (quante, durata) =>
      `${plurale(quante, 'activité', 'activités')} · ${durata}`,
  ...
  en: {
    ...
    corsoSparito:
      'The course this plan referred to no longer exists. Choose another one above.',
    piuOre: (ore) =>
    ...
    nessunPianoTesto:
      'You prepare a plan once and reuse it across several lessons.',
    creaUnPiano: 'Create a plan',
    lezioneDi: (durata) => `The lesson lasts ${durata}.`,
    attivitaPer: (quante, durata) =>
      `${plurale(quante, 'activity', 'activities')} · ${durata}`,

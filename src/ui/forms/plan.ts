...
import { testoDiVoce, vociConValore } from '../../domain/lists.js'
import { MINUTI_UD, formattaData, formattaDurata, formattaUd } from '../../domain/dates.js'
import { creaAttivita, creaPiano } from '../../domain/factories.js'
import type {
  Attivita,
  Lezione,
  PianoLezione,
...
/**
 * Una voce del modulo che si legge e non si tocca.
 *
 * Non è un campo spento: un campo spento è un campo che adesso non si può
 * cambiare, e invita a chiedersi quando si potrà. Questa è un'informazione che
 * arriva da un'altra scelta — nella pagina dei piani, dall'ora scelta
 * nell'elenco a fianco — e che qui si dice soltanto.
 */
function voceFerma (etichetta: string, valore: string, aiuto?: string): HTMLElement {
  return h(
    'div',
    { class: 'campo campo--meta' },
    h('span', { class: 'campo__etichetta' }, etichetta),
    h('strong', { class: 'campo__dettato' }, valore),
    aiuto ? h('small', { class: 'campo__aiuto' }, aiuto) : null,
  )
}

/**
 * Un campo del dettaglio di una tappa: etichetta sopra, controllo sotto.
 *
 * L'etichetta si vede, e non è un dettaglio di stile. Prima i parametri erano
 * caselle nude in fila, con il nome soltanto nel `title`: si capiva che cosa
 * chiedessero passandoci sopra il puntatore, oppure riconoscendo il segnaposto
 * — e un campo già riempito non aveva più nemmeno quello. «CAD, banco 1–12»
 * accanto a «12» non dice a nessuno quale dei due sia la postazione.
 */
function campoTappa (etichetta: string, controllo: Figlio, aiuto?: string): HTMLElement {
  return h(
    'label',
    { class: 'campo-tappa' },
    h('span', { class: 'campo-tappa__etichetta' }, etichetta),
    controllo,
    aiuto ? h('small', { class: 'campo-tappa__aiuto' }, aiuto) : null,
  )
}
...
function spuntaTappa (
  ...
  return h(
    'label',
    { class: ['campo-tappa', 'campo-tappa--sino'], attr: { title: aiuto ?? '' } },
    h('input', {
      type: 'checkbox',
      checked: acceso,
...
/**
 * Un gruppo di campi del dettaglio, con il suo titolino.
 *
 * I campi di una tappa sono di quattro specie diverse — come si svolge, quel
 * che dipende dal tipo, la prova, il materiale — e in fila uno dopo l'altro
 * erano una parete di caselle in cui si cercava a memoria. Il titolino costa
 * una riga e dice dove si è.
 */
 ...
 *
 * Le voci delle tendine escono dalle liste di sistema — `Impostazioni → Liste` —
 * e non da un elenco scritto qui: i supporti su cui si spiega e i modi in cui si
 * formano i gruppi sono roba della scuola, non del programma.
 */
function campiParametri (voce: Attivita, alCambio: () => void): Figlio[] {
  ...
  return parametri.map((parametro) => {
    ...
    if (parametro.tipo === 'scelta') {
      // Il valore scelto resta offerto anche se la lista non lo ha più: una
      // tendina che non contiene quel che la tappa ha dentro lo cambierebbe di
...
/**
 * Come si svolge la tappa: quel che vale per ogni tipo.
 *
 * Sono tre campi che prima stavano in tre posti diversi — la descrizione era
 * una casella nuda nella riga, il raggruppamento e i materiali non si potevano
 * scrivere affatto pur comparendo nella scaletta stampata — e sono la stessa
 * domanda: che cosa succede in aula in questo quarto d'ora.
 */
  ...
  return [
    campoTappa(
      ...
      h('textarea', {
        ...
        placeholder: 'i passaggi, le consegne, quel che si dice',
        onchange: (evento: Event) => {
    ...
    campoTappa(
      'Come lavora la classe',
      h(
        'select',
        {
          class: 'campo__controllo campo__controllo--selezione',
          onchange: (evento: Event) => {
            voce.raggruppamento = (evento.target as HTMLSelectElement)
              .value as Attivita['raggruppamento']
            alCambio()
          },
        },
        ...vociConValore(
          ...
          h(
            'option',
            {
              ...
              selected: (voce.raggruppamento ?? 'plenaria') === opzione.valore,
            },
            opzione.testo,
          ),
        ),
    ...
    campoTappa(
      ...
      h('input', {
        ...
        placeholder: 'fotocopie, righello, chiavi del laboratorio',
        onchange: (evento: Event) => {
          voce.materiali = (evento.target as HTMLInputElement).value
          alCambio()
        },
      }),
      'Quel che bisogna avere in mano: si ritrova nella scaletta stampata.',
    ),
  ]
}
 ...
 *
 * Spenta, è una riga sola: la casella da spuntare. Le tre domande che seguono —
 * titolo, tipo, peso — compaiono solo quando la risposta è sì, perché a una
 * tappa che non è una prova non si chiede quanto pesa.
 */
function campiValutazione (voce: Attivita, alCambio: () => void): Figlio[] {
  const prevista = voce.valutazione ?? null

  const acceso = spuntaTappa(
    'Questa tappa è una prova',
    prevista !== null,
    (attiva) => {
  ...
  return [
    ...
    campoTappa(
      ...
      h('input', {
        ...
        placeholder: voce.titolo || 'come la tappa',
        onchange: (evento: Event) => {
    ...
    campoTappa(
      ...
      h(
        'select',
        {
          ...
          onchange: (evento: Event) => {
            if (!voce.valutazione) return
            voce.valutazione.tipo = (evento.target as HTMLSelectElement).value as TipoValutazione
            alCambio()
          },
        },
        ...vociConValore(stato.registro.impostazioni, 'tipoValutazione', prevista.tipo).map(
          (opzione) =>
            h(
              'option',
...
function editorAttivita (
  ...
  /**
   * Le tappe con il dettaglio aperto.
   *
   * Sta qui e non dentro `disegna` perché l'elenco si ridisegna tutto a ogni
   * carattere battuto: tenendolo nella riga, il pannello si richiuderebbe da
   * sé appena scritta la prima lettera di una consegna.
   */
  const aperte = new Set<string>()

  /**
   * La tappa che sta in `da` portata in `a`.
   *
   * Posizioni e non versi: è quel che chiede `riordinatore`, che è lo stesso
   * che riordina gli slot di un'ora — un gesto solo, scritto una volta sola.
   * Il fuoco torna sulla presa della tappa appena spostata, così chi riordina
   * da tastiera continua a premere la stessa freccia invece di ritrovare la
   * riga con il mouse a ogni passo. Non si può usare `fuocoSullaPresa`: là
   * l'ennesima riga è l'ennesimo figlio, qui in mezzo alle tappe ci sono le
   * testate dei gruppi e gli intervalli, e il conto non torna.
   */
  ...
  const disegna = () => {
    ...
    // L'elenco nasce prima delle righe che ci vanno dentro: il riordinatore
    // vuole il contenitore per pulire i segni della posa, e le righe vogliono
    // il riordinatore per farsi afferrare. Le righe non sono tutti i figli —
    // in mezzo ci stanno le testate dei gruppi e gli intervalli — ma questo al
    // riordinatore non serve saperlo: sa quali ha agganciato.
    const elenco = h('ol', {
    ...
    /**
     * Quel che una tappa ha dentro, detto in una riga.
     *
     * È il prezzo del dettaglio richiuso: se quel che si è scritto sparisse
     * alla chiusura, l'unico modo di sapere quali tappe hanno una consegna
     * sarebbe aprirle tutte. Qui c'è l'essenziale — come lavora la classe, i
     * parametri riempiti, la prova, quanti allegati — e il resto si apre.
     */
    const sommarioTappa = (voce: Attivita): Figlio => {
        ...
        pezzi.push(
          pastiglia(
            testoDiVoce(stato.registro.impostazioni, 'tipoValutazione', voce.valutazione.tipo) +
              (voce.valutazione.peso !== 1 ? ` · peso ${voce.valutazione.peso}` : ''),
            'attenzione',
            'valutazioni',
          ),
        )
      ...
      // La descrizione per prima e in chiaro: è quel che si rilegge davvero
      // scorrendo una scaletta preparata la settimana scorsa.
      const detto = [
     ...
     *
     * Sotto la riga c'è il dettaglio, e sta chiuso finché non lo si apre.
     * Prima era sempre aperto: ogni tappa portava con sé la descrizione, i
     * parametri del tipo, la casella della prova e l'elenco degli allegati, e
     * una scaletta di sei tappe diventava una parete di caselle in cui la
     * scaletta — che è la cosa per cui si è aperto il modulo — non si vedeva
     * più. Chiuso, di ogni tappa restano le quattro cose che si confrontano in
     * colonna, più una riga che dice che cosa c'è dentro.
     */
    const rigaAttivita = (voce: Attivita, indice: number): HTMLElement => {
      ...
      // Quanto di quel che c'è se la prende questa tappa: della capienza del
      // gruppo di UD in cui cade, o — senza un'ora sotto — della scaletta
      // intera. È il filo colorato in fondo alla riga, ed è il modo in cui la
      // striscia del tempo è tornata dentro l'elenco invece che sopra: qui i
      // tratti stanno sulla riga che si sta scrivendo, e si vede se l'ora
      // regge senza leggere un solo numero.
      ...
      // Il tipo non è più una tendina larga come mezza riga: si sceglie una
      // volta e si legge per sempre, e nove caratteri e mezzo di `select` in
      // ogni riga erano larghezza tolta al titolo, che invece si rilegge. La
      // pastiglia dice il tipo e ne porta la tinta; premuta, apre l'elenco.
      const vociDelTipo = vociTipoAttivita().filter(
        ...
        // starebbe in mezzo a quelli che si scelgono ogni volta. Se una tappa
        // ce l'ha già — il corso è cambiato sotto — resta scelta e resta
        // visibile: toglierla dall'elenco l'avrebbe cambiata di nascosto al
        // primo salvataggio.
        (v) =>
      ...
      const tipo: HTMLElement = h(
        'button',
        {
          ...
          type: 'button',
          attr: {
            title: `Tipo: ${nomeDelTipo}. Premi per cambiarlo.`,
            'aria-label': `Tipo di attività: ${nomeDelTipo}`,
            'aria-haspopup': 'menu',
          },
          onclick: () =>
            menuSotto(
      ...
      const riga = h(
        'li',
        {
          ...
          // `tipo` sulla riga e non solo nella pastiglia: la tinta del genere
          // la sceglie il foglio di stile, che è dove stanno le tinte, e non
          // una seconda tabella di colori scritta qui dentro.
          dataset: { attivitaId: voce.id, tipo: voce.tipo },
        },
        h(
          'div',
        ...
        h('input', {
          ...
          attr: { 'aria-label': 'Titolo dell’attività' },
          onchange: (evento: Event) => {
        ...
        h(
          ...
          pulsante({
            ...
            titolo: aperta ? 'Chiudi il dettaglio' : 'Dettaglio della tappa',
            al: () => {
        ...
        aperta
          ? h(
              ...
              // Il titolo dice di quale tipo sono questi campi: cambiando tipo
              // il gruppo cambia sotto le dita, e senza il nome sembrerebbe che
              // i campi siano spariti invece che cambiati.
              gruppoTappa(
                `Dettagli · ${nomeTipoAttivita(voce.tipo, stato.registro.impostazioni)}`,
                parametri,
              ),
              gruppoTappa(
                'Valutazione',
              ...
              // Il materiale della tappa sta con la tappa: la scheda che serve
              // al lavoro di gruppo appartiene a quel quarto d'ora e non al
              // piano intero, e cercarla in un elenco comune vuol dire
              // ritrovarsi in aula a chiedersi quale delle tre era.
              gestore
                ? gruppoTappa('Materiale della tappa', [
                    bloccoRisorse({
    ...
    // Senza lezione sotto non c'è scheletro da mostrare: solo l'elenco, e se è
    // vuoto lo si dice.
      ...
      h(
        ...
        pulsante({
          ...
          al: () => {
            ...
            attivita.push(nuova)
            // Una tappa appena nata è vuota, e quel che le manca sta nel
            // dettaglio: aprirlo subito evita il clic che nessuno indovina.
            aperte.add(nuova.id)
            disegna()
            allaModifica(attivita)
...
/**
 * L'editor di un piano, senza la finestra intorno.
 *
 * Stava tutto dentro `moduloPiano`, e per questo il piano si poteva modificare
 * soltanto in una modale. La pagina «Piani lezione» adesso ci mette dentro
 * questi stessi campi al posto del riepilogo che si poteva solo leggere: un
 * corpo solo, due posti in cui metterlo, e nessun rischio che la finestra e la
 * pagina chiedano due cose diverse.
 */
export function editorPiano (opzioni: {
  ...
  lezione?: Lezione | null
  /**
   * Chiamata a ogni modifica confermata — un campo lasciato, una tappa
   * spostata, un allegato.
   *
   * Serve a chi tiene l'editor dentro una pagina e salva da sé, campo per
   * campo, come fa il registro dell'ora: là non c'è un pulsante «Salva» da
   * premere, e quel che si scrive deve arrivare al registro da solo. Nella
   * modale non c'è, e allora a salvare è il suo pulsante.
   */
  allaModifica?: () => void
  /**
   * Il corso non si sceglie qui: lo detta la selezione di fuori.
   *
   * Nella pagina dei piani il piano che si ha davanti è quello dell'ora scelta
   * nell'elenco a sinistra, e il corso è già una conseguenza di quella scelta.
   * Una tendina che lo ripetesse offrirebbe di spostare il piano su un altro
   * corso proprio mentre l'elenco accanto dice di quale ora è — e chi la
   * toccasse per sbaglio si troverebbe il piano sparito dall'elenco da cui lo
   * aveva appena aperto. Resta una riga che lo dice.
   *
   * Un piano rimasto senza corso la tendina ce l'ha lo stesso: il corso è
   * stato eliminato sotto, e questo è l'unico posto da cui lo si riaggancia.
   */
  ...
  // La tendina del corso resta dov'è un corso da scegliere: su un piano che il
  // suo l'ha perso, e in ogni modulo che non ha un elenco accanto a dettarlo.
  ...
  const corpoModulo: HTMLElement = h(
    ...
    //
    // Le tre sezioni sono le tre domande che ci si fa preparando, nell'ordine
    // in cui ci si arriva: di che cosa parla l'ora, come la si spende, che cosa
    // bisogna avere sottomano. Prima le etichette e le note stavano in fondo,
    // dopo i materiali: sono la stessa cosa degli obiettivi — quel che del
    // piano si ricorda cercandolo un anno dopo — e chiederle in coda voleva
    // dire scrivere le due metà della stessa risposta ai due capi del modulo.
    sezioneModulo(
      'Di che cosa parla',
      riga(
        corsoFermo
          ? voceFerma(
              'Corso',
              nomeCorso(base.corsoId),
              'Lo dice l’ora scelta qui accanto. Per usare il piano altrove lo si duplica.',
            )
          : campoCorso({
              valore: base.corsoId ?? '',
              richiesto: true,
      ...
      riga(
        ...
        campo({
          ...
          aiuto: 'Che cosa devono già sapere per starci dietro.',
          larghezza: 'meta',
        }),
      ),
      riga(
        ...
        campo({
          ...
          righe: 2,
          valore: base.note ?? '',
          aiuto: 'Per me: com’è andata l’altra volta, che cosa cambiare.',
          larghezza: 'meta',
        }),
      ),
    ),
    sezioneModulo(
      ...
      editorAttivita(
        ...
        (nuove) => {
          attivita = nuove
          // Una tappa spostata, aggiunta o tolta non passa da un `change` dei
          // campi: senza questa riga, chi salva da solo se la perdeva.
  ...
  // Un `change` qualunque dei campi è una modifica confermata: gli `input` si
  // lasciano stare apposta, perché chi salva da solo non deve salvare a ogni
  // lettera battuta. Le tappe hanno già il loro `allaModifica` qui sopra.
...
export function moduloPiano (
  piano?: PianoLezione,
  /**
   * Che cosa fare col piano appena creato. Può essere asincrona: chi apre questo
   * modulo da una lezione ci assegna il piano subito dopo, e quella è un'altra
   * azione mandata al registro — `salva` la aspetta, e gli errori tornano a casa.
   */

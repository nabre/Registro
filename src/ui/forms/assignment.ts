...
import { campo, riga, sezioneModulo, valoriModulo } from '../components/base.js'
import { apriModale } from '../components/modal.js'
import { notifica } from '../components/notifications.js'
import { h, rimpiazza } from '../dom.js'
import { classeDelCorsoId, materiaDelCorsoId, nomeCorso, stato } from '../state.js'
...
export function moduloConsegna (opzioni: OpzioniModuloConsegna = {}): void {
  ...
  /**
   * Quel che era scritto nei campi prima dell'ultimo ricambio del corpo.
   *
   * Cambiare corso rifà il modulo da capo — cambia la classe, quindi cambiano
   * le persone e le ore — e fin qui rifaceva anche tutto il resto, riportando
   * ai valori di partenza il testo, il tipo, i destinatari e le note: chi
   * scopriva di aver scelto il corso sbagliato dopo aver compilato ricominciava
   * da zero senza che niente glielo dicesse.
   *
   * Si conserva solo quel che dal corso non dipende. Le spunte delle persone e
   * la scadenza legata a una lezione no: parlano di una classe e di ore che
   * dopo il cambio non ci sono più.
   */
  ...
  const corpoConCorso = (
    ...
    riga(
      ...
      h(
        ...
        // Come arriva a destinazione, quando è il docente a consegnare.
        //
        // Il registro sapeva già farlo — il modello lo prevede, la convalida lo
        // accetta e `actions/assignments.ts` prepara le mail — ma nessun controllo
        // scriveva questo campo: restava sul valore predefinito «a mano», e
        // quella parte del programma non era raggiungibile da nessun gesto.
        campo({
          nome: 'modoConsegna',
          etichetta: 'Come lo consegno',
          tipo: 'select',
          valore: val('modoConsegna', base.modoConsegna ?? 'mano'),
          opzioni: [
            { valore: 'mano', testo: 'A mano, in classe' },
            { valore: 'email', testo: 'Per e-mail, uno a uno' },
          ],
          aiuto:
  ...
  apriModale({
    ...
    alSalva: async (valori, contesto) => {
      ...
      const aggiornata: Consegna = {
        ...
        // Solo quando si consegna: ricevendo un documento non c'è niente da
        // mandare, e un «per e-mail» appeso lì direbbe una cosa che non succede.
        modoConsegna:

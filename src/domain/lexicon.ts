 ...
 *
 * Erano sei elenchi in sei file — i tipi di attività in `domain/activities.ts`,
 ...
 */
/**
 * Il dizionario, che è la forma con la garanzia: `Record<StatoPresenza, ...>`
 * il compilatore lo vuole completo, e uno stato aggiunto al tipo non si può
 * dimenticare qui. L'elenco che segue ne esce, e non gli sta accanto.
 */
...
/**
 * Gli stessi stati in fila, nell'ordine in cui si mostrano.
 *
 * L'ordine è quello in cui il dizionario li scrive: `Object.keys` lo conserva,
 * e le tendine dell'appello si leggono da «non impostato» a «esonerato» come
 * prima.
 */
...
/**
 * A chi risponde un numero, detto con le parole del lessico.
 *
 * Non un elenco di nomi scritti a mano: sono le stesse tre persone che il
 * registro nomina dappertutto, e cambiando «persona in formazione» in
 * «allieva» cambia anche il titolino sopra i suoi numeri.
 */
...
/**
 * Come si chiamano i numeri: minuscoli, perché si leggono dentro una frase
 * («il cellulare della mamma») e non come titoli di colonna.
 */
...
/**
 * Che numero si sta scrivendo, quando se ne apre uno nuovo.
 *
 * Il predefinito non è lo stesso per tutti e tre: di una persona in formazione
 * si scrive il cellulare, di un'azienda il centralino. Indovinarlo giusto è un
 * menu che non si apre nove volte su dieci.
 */
...
/**
 * Come lavora la classe durante una tappa.
 *
 * «Plenaria» è il caso normale — si parla a tutti — e sta per primo perché è
 * quello che una tappa nuova ha già addosso: gli altri si scelgono quando
 * cambia davvero qualcosa nella stanza.
 */
 ...
 */
/**
 * Le chiavi di uno di questi dizionari, come elenco.
 *
 * I `Record<Tipo, string>` qui sotto sono **esaustivi per il compilatore**: se
 * si aggiunge un tipo al modello e non lo si nomina, il file non compila. Gli
 * elenchi di stringhe no — e `domain/validation.ts` ne teneva quattro,
 * scritti a mano, gemelli di questi: due liste della stessa cosa che nessuno
 * confrontava, e quella senza garanzia era proprio quella che decide che cosa
 * sopravvive a un giro di apertura del documento. Adesso si deriva.
 */

...
/**
 * Un file che sta nell'archivio della classe, con il perché ci sta.
 *
 * È la stessa cosa che la matrice mostra come casella piena, letta come elenco:
 * la casella risponde a «chi manca», l'elenco a «che cosa ho in mano». Servono
 * tutte e due, e l'elenco serve soprattutto a scorrere i fogli uno dopo
 * l'altro guardandoli — controllare venticinque scansioni tornando ogni volta
 * alla casella giusta di una griglia è il modo per saltarne una.
 */
interface FileRaccolto {
   ...
   * - `tutti`    un foglio solo, uguale per tutti: la circolare da distribuire;
   * - `firme`    il foglio con cui si dimostra di aver consegnato;
   * - `docente`  quel che il docente raccoglie per sé, non dagli allievi.
   */
...
/**
 * I file dell'archivio di una classe, nell'ordine in cui si guardano.
 *
 * L'ordine è quello della pagina — le richieste come le elenca la matrice, e
 * dentro ognuna le persone come stanno in elenco — perché è l'ordine in cui li
 * si scorre: le frecce dell'anteprima vanno avanti di un foglio, e «il
 * prossimo» deve essere quello che si aspetta, non il prossimo in ordine di
 * arrivo.
 *
 * I fogli di colonna vengono prima dei singoli: il foglio firme e la circolare
 * uguale per tutti riguardano la richiesta intera, e leggerli in mezzo ai nomi
 * li farebbe sembrare di qualcuno.
 */

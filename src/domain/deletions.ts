...
export interface FileDaTogliere {
   ...
   *
   * Separati dai documenti raccolti perché sono un'altra cosa. Quelli li ha
   * portati qualcuno e non si rifanno; questi sono copie di quel che il
   * registro sapeva, e quando l'originale se ne va restano a raccontare una
   * lezione che non c'è più. Chi li vuole ancora li rifà con un pulsante.
   */
  stampati: string[]
}
...
/**
 * I fogli che il registro aveva stampato di una cosa che se ne va.
 *
 * Senza doppioni: la stessa scheda si scrive in tre periodi e in più cartelle,
 * e due volte nell'elenco vorrebbe dire due passaggi nel cestino per lo stesso
 * file — il secondo su un file che non c'è più.
 */
...
export function eliminazione (registro: Registro, bersaglio: Bersaglio): Eliminazione | null {
  ...
  const file: FileDaTogliere = { risorse: [], allegati: [], documenti: [], stampati: [] }
  ...
  if (bersaglio.genere === 'allievo') {
    ...
    if (recuperi > 0) perdite.push(plurale(recuperi, 'recupero', 'recuperi'))
    ...
    if (caselle > 0) {
      perdite.push(plurale(caselle, 'casella del comportamento', 'caselle del comportamento'))
    }
    ...
    // Le sue schede già stampate, in tutte le materie e in tutti i periodi: è
    // il foglio che parla di lui, e di lui non resta niente nel registro.
    ...
    return {
      ...
      applica: (r) => {
        ...
        for (const lezione of r.lezioni) {
          ...
          // E le caselle segnate sulla matrice: restavano intestate a un
          // identificatore che non è più di nessuno, e la griglia le mostrava
          // come «persona non più in elenco».
        ...
        for (const momento of r.valutazioni) {
          ...
          // I recuperi sono righe sue quanto i voti, e restavano: intestate a
          // un identificatore che non è più di nessuno, invisibili a
          // `orphans.ts` e a `repairs.ts`, e contate da `recuperiUrgenti`.
  ...
  //
  // Un'ora cancellata lasciava il suo verbale nella cartella: un PDF con
  // l'appello di quel giorno e i nomi di chi c'era, che racconta una lezione
  // che il registro non ha più. Non è un file del docente — è una copia che il
  // registro ha scritto da sé, e quando l'originale sparisce la copia non ha
  // più niente da copiare. I documenti raccolti, quelli sì del docente,
  // seguono le regole di sempre: stanno qui sopra, uno per uno.
  ...
  for (const corsoId of corsi) {
    ...
    // Le schede stampate dentro la cartella di quella materia: quelle delle
    // altre materie restano, perché le altre materie restano.
  ...
  // Una classe che se ne va porta via anche le schede fuori dalle materie —
  // quelle chieste dall'elenco della classe, che stanno nella sua cartella.
  for (const classeId of classi) {
  ...
  // Si dice, perché sono file che spariscono dalla cartella: si rifanno con un
  // pulsante, ma chi li aveva consegnati deve saperlo prima e non dopo.
  const fogli = file.stampati.length
  if (fogli > 0) {
    perdite.push(`${plurale(fogli, 'foglio già stampato', 'fogli già stampati')} nella cartella`)
  }
  ...
  if (bersaglio.genere === 'piano') {
    collezioni.add('piani')
    // Togliere un piano stacca le ore che lo seguivano e i momenti che ne
    // nascevano — `applica` scrive in tutte e due le raccolte. Ma per un
    // bersaglio «piano» gli insiemi `lezioni` e `valutazioni` restano vuoti,
    // perché si riempiono dai corsi, e le due righe qui sopra non scattavano:
    // le collezioni non dichiarate non vengono riscritte (`archivio.modifica`
    // sporca solo quel che le si annuncia), quindi il distacco viveva solo in
    // memoria e riaprendo l'anno i riferimenti morti tornavano tutti.

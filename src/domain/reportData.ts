...
// disegnarli è `data/reportsPdf.ts`. Il giorno in cui un rapporto servisse in
...
import { recuperiDelMomento, rigaDelRecupero, type StatoRecupero } from './retakes.js'
...
import type { DatiRapporto, Tabella } from './reports.js'
...
function legenda (dati: DatiRapporto): void {
  ...
  for (const voce of vive) {
    ...
    dati.valori[`nome${Maiuscola(nome)}`] = voce.nome.toLowerCase()
  }
...
export function datiLezione (
  ...
  // Quel che si è segnato sulla matrice del comportamento, in chiaro.
  //
  // Si compila durante l'ora, una casella alla volta, ed è l'unica cosa del
  // registro che dal verbale restava fuori: il foglio diceva «osservazioni:
  // nessuna» di un'ora in cui erano state segnate sei caselle. Le caselle vuote
  // non ci sono — non si salvano nemmeno — e le colonne sono quelle della
  // griglia: chi, che cosa si guardava, com'è andata, e la riga scritta accanto.
  dati.tabelle.comportamento = {
    intestazione: [corto(PIF), 'Aspetto', 'Com’è andata', 'Annotazione'],
    pesi: [4, 3, 3, 6],
...
/**
 * Le caselle di un'ora nell'ordine in cui si leggono: per persona, e dentro la
 * persona nell'ordine della lista degli aspetti.
 *
 * L'ordine della lista e non quello alfabetico: è quello delle colonne della
 * griglia a schermo, e un verbale che le rimescolasse costringerebbe a cercare
 * due volte la stessa cosa in due posti diversi.
 */
function celleOrdinate (
  ...
  const quanto = (aspetto: string) => {
    ...
    // Gli aspetti tolti dalle impostazioni dopo essere stati usati vanno in
    // fondo: quel che è stato segnato resta segnato.
...
export function datiMomento (registro: Registro, momento: MomentoValutazione): DatiRapporto {
  ...
  dati.valori = {
    ...
    sufficienti:
      conti.conteggio === 0
        ? '—'
        : `${conti.sufficienti} su ${conti.conteggio} (${percento(conti.quotaSufficienti)})`,
 ...
 *
 * La regola non è qui: sta in `alerts.ts`, che è anche quello che la
 * trasforma in una pendenza. La scheda del singolo, il foglio della classe e
 * l'elenco delle cose da fare devono segnalare le stesse persone — tre conti
 * scritti in tre posti sono tre occasioni di segnalarne di diverse, e il giorno
 * in cui non tornano chi legge non sa a quale credere.
 */
...
export function datiAllievo (
  ...
  // Con più corsi il monte ore è la somma dei loro: la scheda parla di quelli,
  // e il cento per cento è quel che insieme prevedono.
  //
  // Il ripiego per un corso senza orario fisso si fa **corso per corso**, e
  // non sulla somma. `matriceCorso` ripiega quando il monte ore che riceve è
  // zero, ma con due corsi — uno con l'orario e uno senza — la somma non è
  // zero, e quello senza finiva per mettere le sue ore al numeratore e niente
  // al denominatore. Una classe con un laboratorio non pianificato leggeva
  // «assenza del 91%» dove il conto vero diceva 48, e sopra al venti per cento
  // la scheda ci scrive un avviso che nessuno ha chiesto.
  const previste = corsi.reduce((somma, suo) => {
    ...
    return somma + sue.reduce((totale, l) => totale + contaUd(l), 0)
  }, 0)
  ...
  dati.valori = {
    ...
    // Chi non frequenta più lo dice il foglio: la scheda di un ritirato si
    // stampa ancora — la si chiede proprio quando qualcuno se n'è andato — e
    // senza questa parola si legge come quella di chi c'è ancora, con dei conti
    // che si fermano senza spiegare perché.
    stato: allievo.attivo ? '' : 'ritirato',
    ...
    appello: percento(riga?.presenza, 'appello mai fatto'),
    ...
    // I pezzi, per chi intesta una busta invece di stampare una riga: il
    // NAP su una riga sua è quel che distingue un indirizzo da una frase.
    ...
    periodo: semestre?.etichetta ?? 'anno intero',
    // Il primo numero di ciascuno, per i modelli che hanno una casella sola:
    // qual è il primo lo decide l'ordine dell'anagrafica, non questo foglio.
    telefono: primoTelefono(allievo, 'pif'),
    telefonoDatore: primoTelefono(allievo, 'datore'),
    // E tutti quanti, per chi li vuole tutti: chi ha un cellulare e un fisso
    // su un foglio da controfirmare ne ha due da provare, e una casella sola
    // ne nascondeva uno.
  ...
  const quanteUd = ore.reduce((massimo, l) => Math.max(massimo, contaUd(l)), 0)
  ...
  // La matrice del comportamento, ora per ora.
  //
  // Sono le caselle segnate durante le lezioni: stavano solo dentro l'ora in
  // cui erano state messe, e una scheda che le taceva diceva meno di quel che
  // il registro sa — «tre volte da migliorare in laboratorio» è esattamente il
  // genere di cosa per cui una scheda si stampa. Una riga per casella, in
  // ordine di giorno, con l'annotazione dove c'è.
  dati.tabelle.comportamento = {
    intestazione: ['Data', 'Corso', 'Aspetto', 'Com’è andata', 'Annotazione'],
    pesi: [2, 4, 3, 3, 6],

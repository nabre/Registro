...
/**
 * Il giorno di cui parla una modifica, quando ne parla di uno.
 *
 * Serve a scegliere il periodo dei documenti da rifare. I fogli di un corso
 * sono divisi per semestre — il nome del file lo dice, e la cartella li tiene
 * separati — e finché questo giorno non si guardava si rifacevano sempre
 * quelli del semestre di oggi: correggendo a marzo un voto di novembre si
 * riscriveva la griglia del secondo semestre, che quel voto non lo contiene, e
 * quella del primo — la sola che fosse cambiata — restava com'era. Un foglio
 * vecchio che nessuno rifà è il modo in cui questa automazione si rompe senza
 * dirlo.
 *
 * Una lezione e una valutazione hanno la loro data. Un piano non ha periodo,
 * una classe e un allievo attraversano l'anno intero: per loro non c'è un
 * giorno, e chi chiama ripiega su oggi, che è il periodo in cui si sta
 * lavorando.
 */

...
// memoria — nessuna lettura da disco, nessun `apparato`. È il motivo per cui
...
import { PIF, STATI_PRESENZA } from './lexicon.js'
...
export function slotInConflitto (slot: Slot[]): Array<[Slot, Slot]> {
  ...
  for (let i = 0; i < ordinati.length - 1; i += 1) {
    // Non basta guardare il vicino: uno slot lungo ne può contenere due, e il
    // secondo non e' adiacente al primo. Si scorre in avanti finche' il
    // prossimo comincia dopo la fine di questo — da li' in poi, ordinati per
    // inizio, nessuno lo puo' piu' toccare.
...
/**
 * Se una casella dell'appello vale come un'ora persa.
 *
 * **Solo `assente`.** Il ritardo è una presenza, e l'esonero pure: sono tre
 * fatti diversi e uno solo toglie un'ora.
 *
 * Il ritardo non è mezz'ora persa perché il registro conta per **unità
 * didattica**, non per lezione: chi entra alla terza UD di un blocco di quattro
 * ha le prime due segnate `assente` — quelle sono le ore perse, e si contano da
 * sé — e la terza, quella in cui è arrivato, è un'ora in cui c'era. Contare
 * anche quella come assenza vorrebbe dire toglierla due volte, e la percentuale
 * che si consegna direbbe più di quel che è successo. I minuti di ritardo
 * restano scritti sulla presenza, e i ritardi si contano a parte: sono una cosa
 * che si guarda, non ore da giustificare.
 *
 * L'esonero è fuori per un'altra ragione: quell'ora non conta per nessuno —
 * c'è un'autorizzazione dietro — e abbassare la frequenza di chi è esonerato
 * vorrebbe dire penalizzarlo per un permesso che gli è stato dato.
 *
 * Una funzione sola per i tre conti che la usano — il quadro dell'allievo, il
 * riepilogo dell'ora, la matrice del corso — e quindi anche per tutto quel che
 * ne discende: le percentuali di fine semestre, i rapporti stampati e la soglia
 * che genera le segnalazioni. Scritta tre volte, era tre occasioni di cambiarne
 * una sola.
 */
 ...
 *
 * Vince il caso peggiore che non sia una scusa: assente batte ritardo, e il
 * ritardo batte tutto il resto.
 *
 * L'esonero no, e questa è l'eccezione: esce solo se **tutte** le UD decise
 * sono esonerate. Un'ora in cui si è esonerati da una sola unità è un'ora in
 * cui si è stati in aula, e riassumerla con «esonerato» direbbe che quell'ora
 * non conta per niente — mentre conta per la parte in cui si era lì. Una UD
 * esonerata accanto a una presente si legge quindi «presente». Qui sopra c'era
 * scritto che l'esonero batte la presenza: non è mai stato vero, e
 * `tests/domain/calculations.test.mjs` fissa il contrario.
 *
 * Un'ora tutta presente resta presente, e un'ora su cui non si è detto niente
 * resta non impostata: le caselle mute non fanno numero né in un senso né
 * nell'altro.
 */
...
// I conti per persona — UD perse, ritardi, assenze intere — stavano qui, in una
// `statisticheAllievo` che li rifaceva per conto suo. Erano gli stessi conti di
// `matriceCorso`, scritti una seconda volta e con un denominatore diverso: la
// quota stava sulle UD con l'appello fatto invece che su quelle previste
// dall'orario, e il foglio di calcolo delle presenze diceva una percentuale che
// il PDF della stessa classe non confermava.
//
// Adesso il conteggio è uno solo e sta in `matriceCorso`: la matrice a schermo,
// il rapporto stampato, il CSV e le segnalazioni leggono tutti quella riga.
 ...
 *
 * Stava scritta a mano in una dozzina di punti — la geometria della mappa, il
 * posto di una finestra ancorata, l'altezza di un dialogo, la scala del testo
 * di sistema, quante righe torna una lettura — e ogni copia era un
 * `Math.min(Math.max(…))` con gli argomenti in un ordine suo. Nessuna
 * sbagliava; ma dodici modi di scrivere la stessa cosa sono dodici cose da
 * rileggere per accorgersi che erano la stessa.
 */
 ...
 *
 * Esce dal dominio perché anche l'API deve contare gli stessi: `valutazioni.voti`
 * si era rifatta il filtro per conto suo e teneva dentro chi era segnato assente,
 * così rispondeva una media diversa da quella che l'applicazione mostra.
 *
 * Il tipo stretto serve a chi legge il valore subito dopo: senza, ogni chiamante
 * si riscrive un `as number` o un `?? 0` che il filtro ha già reso impossibile.
 */
...
interface ContestoNomePiano {
   ...
   * sotto mano — `numeroDellaLezione` in `courses.ts` — perché il conto riparte a ogni
 ...
 * scoperto, positivo se sfora, zero quando le due misure coincidono.
 *
 * In minuti e non in unità perché è un numero che si legge, non uno su cui si
 * conta: «venti minuti scoperti» dice subito che cosa resta da pensare, mentre
 * «zero virgola quattro UD» chiede una moltiplicazione prima di voler dire
 * qualcosa. Il cambio è quello dell'ora vera, non i quarantacinque di listino.
 *
 * Con il segno, e non due funzioni: sono la stessa misura guardata dai due
 * lati, e chi la mostra vuole distinguerli — un'ora mezza vuota e una scaletta
 * che non ci sta sono due lavori diversi.
 */
export function minutiDiScarto (piano: PianoLezione, lezione: Lezione): number {

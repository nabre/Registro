...
import { annoAllineato, intervalloAnno, ordinaSettimane } from './years.js'
import {
  MINUTI_UD,
  durataMinuti,
  formattaData,
...
import { emailValida, nomeDelFile, normalizzaTesto, percorsoRelativo, plurale } from './text.js'
import { chiaveIndirizzo } from './map.js'
import { allieviAttivi, contaUd, nomePiano, slotInConflitto } from './calculations.js'
import { QUANDO_RIFARE_PDF } from './automation.js'
import { normalizzaListe } from './lists.js'
...
import {
  FASCIA, PERSONE, PIF,
  STATI_LEZIONE as LESSICO_STATI_LEZIONE,
  TIPI_ATTIVITA as LESSICO_TIPI_ATTIVITA,
  TIPI_OSSERVAZIONE as LESSICO_TIPI_OSSERVAZIONE,
  TIPI_VALUTAZIONE as LESSICO_TIPI_VALUTAZIONE,
  VOCI_PRESENZA,
  chiaviDi, del, quanti, un,
} from './lexicon.js'
import { CHI_INSEGNA, VERSIONE_DATI } from './models.js'
...
/** Sta in `text.ts`, dove non importa nulla: da qui si riesporta per chi la cercava qui. */
export { emailValida }
...
// Dal dizionario del lessico, per la stessa ragione dei quattro qui sotto: era
// l'unico elenco d'appello rimasto scritto a mano, e un sesto stato aggiunto al
// tipo sarebbe stato riletto da disco come «non impostato» senza che niente lo
// segnalasse — cioè una casella dell'appello svuotata in silenzio a ogni
// riapertura del documento.
const STATI_PRESENZA = chiaviDi(VOCI_PRESENZA)
...
// Derivati dai dizionari del lessico invece che riscritti: quelli sono
// `Record<Tipo, string>`, cioe' esaustivi per il compilatore, e questi erano
// quattro elenchi paralleli che nessuno confrontava con loro. Un tipo aggiunto
// al modello e dimenticato qui usciva da `unaVoce` come il primo valore
// dell'elenco — cioe' una scelta dell'utente sostituita in silenzio a ogni
// riapertura del documento, che e' esattamente quel che e' successo con
// `'a-mano'` fra i motivi di quarantena.
const TIPI_OSSERVAZIONE = chiaviDi(LESSICO_TIPI_OSSERVAZIONE)
const STATI_LEZIONE = chiaviDi(LESSICO_STATI_LEZIONE)
const TIPI_ATTIVITA = chiaviDi(LESSICO_TIPI_ATTIVITA)
const TIPI_VALUTAZIONE = chiaviDi(LESSICO_TIPI_VALUTAZIONE)
...
/**
 * Un indirizzo collocato, se quel che c'è scritto è davvero un punto.
 *
 * Coordinate fuori scala o un indirizzo vuoto valgono come «non l'abbiamo
 * ancora cercato»: un segnaposto a latitudine 400 non si disegna da nessuna
 * parte, e uno senza l'indirizzo a cui appartiene non si può nemmeno
 * riconoscere.
 */
function normalizzaCoordinata (grezzo: unknown): Coordinata | null {
  ...
  return {
    // La chiave si ricalcola sempre dall'indirizzo invece di fidarsi di quella
    // scritta: è una funzione dell'indirizzo, e una chiave scritta a mano che
    // non corrisponde produrrebbe una voce che nessuno ritrova più.
...
/**
 * Le coordinate dell'anno: quelle scritte nel loro file, più quelle che stavano
 * dentro le anagrafiche prima che diventassero una raccolta a sé.
 *
 * La migrazione è muta e definitiva: `normalizzaAllievo` non rilegge più
 * `geo` e `geoDatore`, quindi al primo salvataggio quei campi spariscono dalle
 * classi e restano qui, una volta sola per indirizzo. Due fratelli che avevano
 * due copie dello stesso punto ne lasciano una.
 */
function coordinateDellAnno (dati: Record<string, unknown>, classiGrezze: unknown[]): Coordinata[] {
  ...
  const accogli = (grezzo: unknown) => {
    ...
    // A parità di indirizzo vince la risposta più recente: è quella che
    // qualcuno ha chiesto sapendo che la precedente non andava bene.
  ...
  for (const grezzaClasse of classiGrezze) {
    for (const grezzoAllievo of elenco(oggetto(grezzaClasse).allievi)) {
      ...
      // Nei file di prima il punto stava accanto all'indirizzo della persona, e
      // portava dentro l'indirizzo per cui valeva: è tutto quel che serve per
      // farne una voce della raccolta.
      accogli(allievo.geo)
...
/**
 * I numeri di telefono di un allievo, letti da un file di qualunque età.
 *
 * Prima erano due campi soli — `telefono` e `telefonoDatore` — e i file già
 * scritti li portano ancora. Qui diventano due voci dell'elenco, con il
 * contatto giusto e l'etichetta che quel contatto si porta dietro: il numero di
 * una persona in formazione era il suo cellulare, quello del datore era il
 * centralino. Ripassando di qui una volta sola il file si riscrive nella forma
 * nuova, e i due campi vecchi smettono di comparire come ogni altro campo che
 * il registro ha smesso di prevedere.
 *
 * I vecchi si aggiungono anche quando l'elenco nuovo c'è già, ma solo se quel
 * numero non c'è ancora: un file toccato da due versioni diverse del programma
 * non deve perdere un numero né raddoppiarlo.
 */
/**
 * Un indirizzo letto da un file di qualunque età.
 *
 * Nei file scritti prima è una riga sola: la si spezza una volta, e da lì in poi
 * sta nelle sue caselle. `leggiIndirizzo` è fatto per non perdere niente —
 * quel che non riconosce lo lascia intero nella via — perché la riga rimessa
 * insieme è la chiave con cui le coordinate già trovate si ritrovano.
 */
...
function normalizzaTelefoni (dati: Record<string, unknown>): Telefono[] {
  ...
  /**
   * Aggiunge quel che c'era scritto in una casella: uno o più numeri.
   *
   * L'etichetta si legge da quel che c'è quando c'è — chi l'ha scelta a mano
   * ha ragione lui — e si propone dal numero quando non c'è, che è il caso di
   * ogni file scritto prima che le etichette esistessero. Solo il primo pezzo
   * tiene l'id e l'etichetta scritta: gli altri sono numeri che fino a ieri
   * nessuno sapeva di avere, e un'etichetta copiata dal primo direbbe che il
   * fisso di casa è un cellulare.
   */
  const aggiungi = (
    ...
    separaNumeri(grezzo).forEach((pezzo, indice) => {
      ...
      if (numero === '') return
      // Lo stesso numero due volte per lo stesso contatto è una riga in più
      // da cancellare a mano, non un secondo recapito.
  ...
  // I due campi di prima, quando ci sono ancora: un file toccato da due
  // versioni del programma non deve perdere un numero né raddoppiarlo.
...
function normalizzaAllievo (grezzo: unknown): Allievo {
  ...
  return {
    ...
    // Dove cadono questi due indirizzi non si legge più di qui: sta in
    // `registro.coordinate`, con la chiave sull'indirizzo. Un `geo` rimasto nei
    // file vecchi lo raccoglie `coordinateDellAnno`, e poi smette di comparire.
 ...
 *
 * `anno` è il calendario in cui il periodo deve stare, quando lo si sa: un
 * rapporto di assenze parla di ore di scuola, e ore di scuola fuori dall'anno
 * scolastico non ce ne sono. Un periodo che sborda è quasi sempre un anno
 * sbagliato nella data — 2025 per 2026 — e scoprirlo dopo vuol dire un
 * periodo salvato su un anno in cui nessuno lo andrà a cercare.
 */
export function validaBloccoAssenze (
  ...
  const errori: string[] = []
  // Il nome non si chiede e non si convalida: lo ricava chi salva dal semestre
  // in cui il periodo cade, e fuori dai semestri un periodo è le sue due date.
  ...
  if (
    ...
    errori.push(
      `Il periodo esce dall’anno ${anno.etichetta}: va da ` +
        `${formattaData(anno.inizio)} a ${formattaData(anno.fine)}.`,
    )
  }
...
function normalizzaCorso (grezzo: unknown): Corso {
  const dati = oggetto(grezzo)
  const ora = istanteAdesso()
  return {
...
/**
 * Una casella della matrice del comportamento, riletta dal file.
 *
 * `null` quando non è una casella: senza persona o senza aspetto non si sa di
 * chi né di che cosa parli, e una casella senza né segno né annotazione non
 * dice niente — è la stessa regola con cui `osservazione.cella` la toglie
 * invece di lasciarla lì a occupare una riga.
 */
...
function normalizzaLezione (grezzo: unknown, corsoId = ''): Lezione {
  const dati = oggetto(grezzo)
  const ora = istanteAdesso()
  ...
  return {
    ...
    // La matrice del comportamento. Restava fuori di qui, e siccome il registro
    // passa da questa funzione ogni volta che si apre, le caselle segnate
    // duravano quanto la finestra: si segnavano, si vedevano, e alla
    // riapertura non c'erano più. Un dato che sparisce in silenzio è il modo
    // peggiore di perderlo — nessuno se ne accorge il giorno in cui succede.
    //
    // Si scrive solo quando c'è qualcosa: le ore senza niente segnato non si
    // portano dietro un elenco vuoto, e i registri di prima che la matrice
    // esistesse restano com'erano.
...
function normalizzaAttivita (grezzo: unknown): Attivita {
  ...
  return {
    ...
    // che c'era diviso la lunghezza di una UD, e basta. Un piano da «venti
    // minuti» diventa mezza unità, che è quel che voleva dire — e adesso resta
    // mezza unità anche dove le UD sono da cinquanta.
    //
    // Non si arrotonda a niente, e in particolare **non al quarto**: dentro il
    // piano la durata si scrive in minuti, e un'attività da venti minuti
    // riletta come mezza unità e riarrotondata tornerebbe a schermo da
    // ventitré. Si tiene il numero che viene fuori dalla divisione, purché sia
    // positivo. Qui sopra c'era scritto «portato al quarto più vicino», e due
    // righe dopo il contrario: l'arrotondamento non c'è mai stato.
    durataUd: Math.max(
      0.01,
...
export function normalizzaImpostazioni (grezzo: unknown): Impostazioni {
  ...
  return {
    ...
    // Solo le liste riconosciute, e solo le voci che una tendina può offrire:
    // il resto non lo leggerebbe nessuno. Vedi `domain/lists.ts`.
...
const MOTIVI_QUARANTENA = [
  // `'a-mano'` c'era nel tipo e non qui: `sorting.ts` lo produce quando si
  // divide a passo fisso o a mano, e alla rilettura `unaVoce` lo degradava a
  // `'senza-nome'` — cioè il registro ripresentava come «nome non riconosciuto»
  // un blocco che nessuno aveva mai chiesto di riconoscere.
  'a-mano',
  'senza-nome',
  'senza-testo',
  'ambiguo',
...
/**
 * Come il PDF è stato diviso, riletto dal disco.
 *
 * Il campo c'era nel tipo e lo scriveva `actions/sorting.ts`, ma
 * `normalizzaSmistamento` non lo nominava: a ogni apertura dell'anno la scelta
 * spariva, e il ripiego di `domain/sorting.ts` — scritto «per gli
 * smistamenti fatti prima che la domanda esistesse» — tornava a dire `nomi`.
 * Chi aveva scelto «un documento ogni quattro pagine» o «divido a mano» se lo
 * ritrovava disfatto al riavvio.
 */
...
/**
 * Il riquadro in cui è stato letto il nome, riportato dentro il foglio.
 *
 * Sono frazioni, e un file scritto a mano o da una versione con un difetto può
 * portarne di assurde: un riquadro che comincia a metà e largo il doppio della
 * pagina disegnerebbe un rettangolo che esce dall'immagine. Qui si stringe a
 * quel che esiste, e se non resta niente non c'è nessun riquadro.
 */
...
/**
 * La casella di assenze in cui sono finite delle pagine, riletta dal file.
 *
 * Senza periodo o senza classe non è una casella: torna `undefined`, e quella
 * fetta vale come archiviata in una consegna — che è come si comportava il
 * registro prima che le pagine potessero cadere sulla matrice delle assenze.
 */
  ...
  return {
    ...
    assegnate: elenco(dati.assegnate).map((voce) => {
      ...
      return {
        ...
        // colonna, e l'allievo resta vuoto apposta.

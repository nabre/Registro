...
import { confrontaNomi, plurale } from './text.js'
...
export function riparazioni (registro: Registro): Riparazione[] {
  ...
  // I numeri sono arrivati da file in cui l'etichetta non esisteva, e sono
  // entrati tutti con lo stesso nome: il primo numero di una persona in
  // formazione «cellulare», quello di un'azienda «centralino». Per quasi tutti
  // è giusto; per i fissi di casa finiti nella prima casella no, e la riga si
  // legge al contrario di com'è.
  //
  // Si propone e non si fa da sé: «cellulare» su un fisso può anche essere una
  // scelta di chi l'ha scritto, e riscriverla in silenzio a ogni apertura
  // vorrebbe dire non lasciargliela cambiare mai.
  ...
  if (storti.length > 0) {
    esito.push({
      descrizione:
        `Rinomina ${plurale(storti.length, 'numero', 'numeri')} secondo quel che ` +

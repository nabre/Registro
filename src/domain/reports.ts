...
/**
 * Le direttive che il corpo di un modello riconosce. Ogni altra riga viene
 * saltata — vedi `templateCheck.ts`, che sulla stessa lista dice a chi
 * scrive un modello quel che il lettore salterà.
 */
 ...
 *
 * «Di nuovo in chi apre il file» è la parte che conta, e vale la pena dire dove
 * sta, perché questo controllo da solo **non basterebbe**: qui si guardano i
 * pezzi separati da `/`, e una barra rovescia resta dentro un pezzo solo. Le
 * due porte vere sono `immagineModello` in `data/templates.ts` — che riammette il
 * solo nome secco, con una regola che la barra rovescia non passa — e
 * `fileAllegato` in `data/paths.ts`, che taglia su tutti e due i separatori
 * e butta via ogni `..` prima di comporre l'indirizzo. Chi tocca questa riga
 * non rilassi quelle due pensando che qui ci sia già tutto.
 */
 ...
 *
 * Il file passa da `immagineAmmessa`, che ammette tanto il nome secco di
 * `templates/` quanto un percorso dentro la cartella dell'anno — è così che una
 * foto d'anagrafica arriva su un foglio, e qui sopra c'era scritto «solo un
 * nome di file, senza cartelle», che di questa funzione non è mai stato vero.
 * Dove si va a prendere il file lo decide `immagineDelRapporto` in
 * `actions/reports.ts`, guardando se c'è una barra.
 *
 * Null se non si capisce, e la riga sparisce come ogni altra riga storta — un
 * rapporto senza logo esce, uno che non esce non serve a niente.
 */
 ...
 *
 * Stavano dentro `reportData.ts`, mescolate ai conti che le producono. Ma una
...
/**
 * Una riga «chiave: valore» di un blocco, spezzata in due. `null` se i due
 * punti non ci sono, cioè se la riga non dichiara niente.
 *
 * La chiave si abbassa di caso — è un'etichetta da confrontare — il valore no:
 * è testo che finisce stampato nel rapporto così com'è scritto.
 */

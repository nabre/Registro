...
// fa `environment/tray.ts`, che di lezioni non sa niente.
//
// ## I segni
//
...
const NOME_FASE: Readonly<Record<FaseOra, string>> = {
  'in-corso': 'in corso',
...
function motivoDellOra (
  ...
  switch (fase) {
    ...
    case 'svolta':
      ...
      break
    // Due modi di non essere pronti, e si rimediano in due posti: il piano che
    // manca si sceglie, la scaletta corta si allunga dove già sta.
    case 'da-preparare':
      pezzi.push(lezione.pianoId ? 'scaletta corta' : 'senza piano')
      break
    case 'futura':
      pezzi.push('piano completo')
      break
    case 'annullata':
...
function nomeDelCorso (registro: Registro, corso: Corso): string {
  ...
  // `titoloCorso` e' il generatore del **titolo memorizzato** — lo chiamano
  // `actions/register.ts` e `domain/repairs.ts` per scrivere
  // `corso.titolo` — e i suoi ripieghi sono segnaposto da riparazione:
  // «classe», «Materia», in minuscolo. Passandoli a un menu si leggeva
  // letteralmente «classe — Matematica» per un corso la cui classe era stata
  // cancellata, mentre l'intestazione del pannello — che ripiega su
  // `corso.titolo` — mostrava il nome vero. Qui si mostra, non si scrive: il
  // ripiego giusto e' il titolo che il corso si porta dietro.
...
function faseDelMucchio (
  ...
  switch (chiave) {
    ...
    // Preparata vuol dire con la scaletta che arriva in fondo all'ora, non con
    // un piano appeso: vedi `oraCoperta`.
    default:
      return oraCoperta(registro, lezione) ? 'futura' : 'da-preparare'
  }
}
...
function faseDelCorso (registro: Registro, mucchi: OreRaggruppate): FaseOra {
  ...
  for (const lezione of mucchi.prossime) {
    presenti.add(oraCoperta(registro, lezione) ? 'futura' : 'da-preparare')
  }
...
function corsoDelVassoio (
  ...
  const fase = faseDelCorso(registro, raggruppate)
  ...
  const mucchi = MUCCHI.map((mucchio) => {
    ...
    return {
      ...
      ore: ore
        ...
        .map((lezione) =>
          voceOra(registro, lezione, faseDelMucchio(registro, mucchio.chiave, lezione), oggi, ora),
        ),

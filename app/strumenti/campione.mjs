// Rigenera il documento campione: `npm run campione`.
//
// In `prove/campioni/2026-2027.registro` c'è un anno finto — una classe, tre
...
//   - alla prova `prove/dati/campione.test.mjs`, che lo riapre a ogni giro. È
...
const DESTINAZIONE = percorso.join(APP, 'prove', 'campioni', '2026-2027.registro')

/**
 * L'archivio gira sui bundle di prova — quelli con `electron` finto — perché è
 * l'unico modo di farlo lavorare fuori da una finestra. `npm run campione` li
 * ricostruisce prima di arrivare qui.
 */
const bundle = (nome) => `file:///${percorso.join(APP, 'dist-prove', nome).replace(/\\/g, '/')}`

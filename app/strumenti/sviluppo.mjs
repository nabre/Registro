// Il modo sviluppo: `npm run sviluppo`.
//
...
//      `src/ambiente/sviluppo.ts`, dentro l'applicazione, perché è di lì che si
...
function avviaElectron () {
  ...
  processo = spawn(elettrone, [AVVIO], {
    ...
    env: {
      ...process.env,
      // La accende `src/ambiente/sviluppo.ts`, che da lì fa ricaricare le
 ...
 * che si lancia `npm run sviluppo`.
 *
...
function segnala (configurazione, esito) {
  ...
  if (esito.errors.length > 0) {
    ...
    finitoIlPrimoGiro(nome)
    return
  }

  if (!primoGiro.has(nome)) {
    finitoIlPrimoGiro(nome)
    return
  }
  dice(`${nome} ricostruito`)

  // Le pagine si ricaricano da sole: `sviluppo.ts` guarda `dist/`.
...
function finitoIlPrimoGiro (nome) {
  if (primoGiro.has(nome)) return
  primoGiro.add(nome)

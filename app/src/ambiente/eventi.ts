...
/** Qualunque cosa si possa chiudere: è la forma minima che `Disposable.from` accetta. */
 ...
 * vscode.Disposable` — `Archivio`, `Smistatore` — e con il campo privato
...
export class Disposable {
  ...
  static from (...smaltibili: Smaltibile[]): Disposable {
...
export class EventEmitter<T> {
  ...
  readonly event: Event<T> = (ascoltatore, questo, smaltibili) => {
    ...
    const smaltibile = new Disposable(() => this.#ascoltatori.delete(legato))
...
class Gettone implements CancellationToken {
  #annullato = false
...
export class CancellationTokenSource {
  ...
  readonly token: CancellationToken = this.#proprio

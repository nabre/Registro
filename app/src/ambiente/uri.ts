...
// contro l'eco delle proprie scritture in `archivio.ts` tiene una mappa
...
export class Uri {
   ...
   * ed è così che `anni.ts` e `archiviazione.ts` prendono la cartella che
 ...
 * JSON dell'anno in `archivio.ts`, i PDF in arrivo in `smistatore.ts` — e non

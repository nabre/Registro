...
import { archiviaCopia, percorsoRisorsaPiano, rinominaArchivio } from '../dati/archiviazione.js'
import { fileAllegato } from '../dati/percorsi.js'
import { formattaData } from '../dominio/date.js'
import { creaPiano, creaRisorsa, duplicaPiano } from '../dominio/fabbriche.js'
import { classeDelCorso, corsoPerId } from '../dominio/corsi.js'
import type { Attivita, PianoLezione, Risorsa } from '../dominio/modelli.js'
import { validaRisorsa, validaPiano } from '../dominio/validazione.js'
...
export const piani = {
  ...
  'piano.duplica': async (contesto, azione) => {
    ...
    for (const { attivita, risorsa } of appese(copia)) {
      ...
      const partenza = fileAllegato(risorsa.file)
  ...
  'risorsa.apri': async (contesto, azione) => {
    ...
    if (risorsa.tipo === 'collegamento') {
      ...
      await vscode.env.openExternal(vscode.Uri.parse(risorsa.url))
      return fatto
    }

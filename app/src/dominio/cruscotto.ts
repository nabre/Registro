...
//
// Il registro sa già tutto quel che serve — se una lezione ha un piano, se
...
import { confrontaLezioni, inizioLezione, momentoLezione, riepilogaPresenze } from './calcoli.js'
import { consegneDaGuardare, consegneDellaLezione } from './consegne.js'
import { recuperiDaFare, recuperiUrgenti } from './recuperi.js'
import { riconsegneAgliAllievi, riconsegneAperte, riconsegneDaFare } from './riconsegne.js'
import { nelPeriodo } from './date.js'
import type { AnnoScolastico, Corso, Iso, Lezione, Ora, Registro, Semestre } from './modelli.js'
 ...
 */
export type Urgenza = 'apposto' | 'da-preparare' | 'manca'

export interface DiagnosiLezione {
  lezione: Lezione
...
export function diagnosiLezione (
  ...
  // Il piano: un'ora futura senza scaletta è lavoro da fare, non un errore.
  // Un'ora passata senza scaletta non è più niente — si è svolta lo stesso.
  if (lezione.pianoId) segni.push('con-piano')
  else if (!passata && !annullata) segni.push('senza-piano')
...
export interface ColonnaCruscotto {
  corso: Corso
  ore: DiagnosiLezione[]
  ...
  buchi: number
  /** Quante ore future non hanno ancora una scaletta. */
  daPreparare: number
...
export interface SezioneCruscotto {
...
export interface RiepilogoCruscotto {
...
/** L'ora di inizio di una lezione, o stringa vuota: serve alle etichette. */
export function oraDiLezione (lezione: Lezione): string {
  return inizioLezione(lezione) ?? ''
}

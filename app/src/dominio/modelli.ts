...
export interface Allievo {
  ...
  dataNascita?: Iso
  /**
   * Dove abita: via, numero, NAP e località, in una riga sola.
   *
   * Una riga e non quattro campi perché il registro non ci fa niente — non
   * ordina per NAP, non stampa etichette — e lo mostra e basta. Spezzarlo
   * vorrebbe dire quattro caselle da riempire per ottenere la stessa riga, e
   * un indirizzo estero che in quelle caselle non ci sta.
   */
  indirizzo?: string
  email?: string
  ...
  azienda?: string
  /**
   * Dove sta l'azienda: via, NAP e località, in una riga sola come quella di
   * casa.
   *
   ...
   */
  indirizzoDatore?: string
  emailDatore?: string
  /**
   * Il numero a cui si chiama in azienda, accanto alla casella a cui si scrive.
   *
   * Un foglio da controfirmare che non torna si sollecita al telefono, non con
   * una seconda mail alla stessa casella che non ha risposto alla prima: chi
   * risponde è il datore di lavoro, e cercarne il numero fuori dal registro
   * vuol dire aprire un altro elenco mentre si ha la cornetta in mano.
   */
  telefonoDatore?: string
  /** Il numero della persona in formazione: il suo, non quello dell'azienda. */
  telefono?: string
  /**
...
export interface Attivita {
   ...
   * dice `dominio/attivita.ts`, tipo per tipo; quel che resta scritto sotto una
...
export type ModoConsegna = 'mano' | 'email'
 ...
 */
export interface DocumentoAllievo {
  allievoId: string
...
export type DestinatarioConsegna = 'classe' | 'docente' | 'allievi'
...
export interface Smistamento {
  ...
  /** Le pagine già finite nella consegna di qualcuno: si racconta e basta. */
  assegnate: Array<{ allievoId: string, da: number, a: number }>

...
import { annoAllineato, semestriFra } from './anni.js'
import {
  MINUTI_UD,
  udArrotondate,
  daIso,
...
import { VERSIONE_DATI } from './modelli.js'
...
export function coloreLibero (usati: string[]): string {
...
export function creaBloccoAssenze (
  ...
  return {
    ...
    etichetta: etichetta.trim() || 'Periodo',
    dal,
    al,
    ...
    // legge, e deve dire che cosa c'è dentro davvero.
    oggetto: `{tipi} — {allievo} — ${etichetta.trim() || 'periodo'}`,
    corpo: CORPO_ASSENZE,
    aAllievo: false,
    aTutore: false,
...
export function creaAllievo (cognome: string, nome: string): Allievo {
  return { id: nuovoIdAllievo(), cognome: cognome.trim(), nome: nome.trim(), attivo: true }
}

...
// niente. Copia dei PDF e invio stanno nell'extension host — `azioni.ts`,
// `dati/posta.ts` — e ricevono da qui che cosa allegare e a chi scrivere.

import { nomeCompleto } from './calcoli.js'
import { formattaData } from './date.js'
import type {
  Allievo,
  BloccoAssenze,
  Classe,
...
export { TIPI_RAPPORTO } from './validazione.js'
...
export interface AvanzamentoAssenze {
...
export function richiesteFirma (
  ...
  for (const mucchio of Object.values(esito)) mucchio.sort(ordinaRichieste)
  return esito
}
...
export interface DestinatariAssenze {
...
export function testoAssenze (
  ...
  const valori: Record<string, string> = {
    ...
    indirizzoAzienda: allievo.indirizzoDatore?.trim() || '',
...
export function documentoFoglio (
  ...
  return `${cosa.charAt(0).toUpperCase()}${cosa.slice(1)} ${periodo}`
    .replace(/[\\/:*?"<>|]+/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
}

...
import type { AnnoScolastico, Iso, Istante, Ora, Semestre } from './modelli.js'
...
/** L'anno scolastico che contiene la data; se più d'uno, il primo per data d'inizio. */
export function annoDi (anni: AnnoScolastico[], iso: Iso): AnnoScolastico | null {
  return (
    anni
      .filter((a) => nelPeriodo(iso, a.inizio, a.fine))
      .sort((a, b) => a.inizio.localeCompare(b.inizio))[0] ?? null
  )
}

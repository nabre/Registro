...
import { inizioLezione, lezioniSovrapposte, unitaDidattiche } from './calcoli.js'
import { aIso, daIso, giornoSettimana, nelPeriodo, sommaMinuti, udDaMinuti } from './date.js'
import { creaLezione } from './fabbriche.js'
import type {
  AnnoScolastico,
  Corso,
  Iso,
...
/** Vero se la fascia eredita l'inizio da quella prima di lei, nello stesso giorno. */
export function ricorrenzaAttaccata (orario: Ricorrenza[], indice: number): boolean {
  const prima = orario[indice - 1]
  return Boolean(prima && prima.giorno === orario[indice]?.giorno)
}

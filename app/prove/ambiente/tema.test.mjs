...
//
// La prova conta anche perché il colore di fondo delle finestre è l'unica cosa
// del tema scritta due volte — una in CSS e una in `tema.ts` — e qui si vede
// almeno che la seconda cambia quando deve.
...
const { bancoElectron, nativeTheme, BrowserWindow } = await import('../aiuti/finto-electron.mjs')
const { applicaTema, coloreSfondo, osservaTema, ricaricaImpostazioni, scuro, workspace } =
  await import('../../dist-prove/ambiente.mjs')
...
describe('il colore di fondo segue il tema', () => {
  ...
  it('cambiare l’impostazione basta: non serve riavviare', () => {
    ...
    // Da `update`, che è la via della finestra delle impostazioni: è lei a far
    // scattare `onDidChangeConfiguration`, ed è lì che `osservaTema` ascolta.
    void workspace.getConfiguration().update(CHIAVE, 'scuro')

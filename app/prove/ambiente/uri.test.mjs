// L'`Uri` dello shim, e in particolare la sua canonicità.
//
// La regola provata qui vale l'intera migrazione: lo stesso file, raggiunto
// per vie diverse — costruito da un percorso nativo, riletto da una stringa,
// composto un pezzo alla volta con `joinPath` — deve dare la stessa identica
// `toString()`. Il registro ci chiava sopra due difese: quella contro l'eco
// delle proprie scritture in `archivio.ts` e la mappa delle risorse del
// webview. Se due vie danno due stringhe, il registro si ricarica a ogni
// salvataggio e la causa non si vede.

import assert from 'node:assert/strict'
import { describe, it } from 'node:test'

import { RelativePattern, Uri } from '../../dist-prove/ambiente.mjs'
...
describe('il percorso è sempre POSIX, il percorso nativo no', () => {
  ...
  it('dà il nome del file come lo prende percorsi.ts', () => {
...
describe('with cambia una parte e ricanonicalizza', () => {
  it('è il modo in cui archivio.ts si fa il nome del temporaneo', () => {
...
describe('RelativePattern tiene insieme una base e un glob', () => {
  it('accetta un uri o un percorso, e dà sempre i due campi', () => {
    const daUri = new RelativePattern(Uri.file('C:\\dati\\registro'), '{registro.json,*/dati/*.json}')
    const daStringa = new RelativePattern('C:\\dati\\registro', '{registro.json,*/dati/*.json}')

// Validazione, normalizzazione, fabbriche e lettura degli elenchi.
//
// La normalizzazione è la rete di sicurezza del registro: i file stanno in una
// cartella sincronizzata e si possono aprire a mano, quindi deve reggere un
// JSON monco, un campo di tipo sbagliato e un file di una versione precedente
// senza far cadere l'estensione.
...
describe('quel che si legge da un file scritto a mano', () => {
  it('un voto che non è un numero resta vuoto, non diventa zero', () => {
    // «4,5» con la virgola italiana diventava NaN e poi 0: un voto fuori scala
    // che tirava giù la media senza che nessuno lo avesse messo.
...
describe('materie e corsi', () => {
  it('trasforma la materia scritta sulla classe in una materia vera e in un corso', () => {
    ...
    // Due grafie, una materia sola: e' il motivo per cui non e' piu' testo libero.
    assert.equal(registro.materie.length, 1)
    assert.equal(registro.materie[0].nome, 'Matematica')
    // La classe non porta piu' la materia: la porta il corso, uno per classe.
  ...
  it('migra il vecchio programma in un corso e ci riaggancia i piani', () => {
    ...
    // Il piano perde anno e classe e si aggancia al corso che il programma è
    // diventato: da lì si duplica per gli altri corsi.
    assert.equal(registro.piani[0].corsoId, registro.corsi[0].id)
    assert.equal(registro.piani[0].annoId, undefined)
    // La lezione si aggancia al corso che il programma e' diventato.
...
describe('validazione', () => {
  it('una lezione vuole un corso, una data e almeno uno slot', () => {
    assert.equal(validaLezione({ data: '2025-09-15', corsoId: '', slot: [] }).valido, false)
    const buona = creaLezione('cor1', '2025-09-15', '08:00', 45)
    assert.equal(validaLezione(buona).valido, true)
  })

  it('rifiuta slot sovrapposti e slot al contrario', () => {
    assert.equal(validaSlot([creaSlot('08:00', 60), creaSlot('08:30', 45)]).valido, false)
    assert.equal(validaSlot([{ id: 's', inizio: '09:00', fine: '08:00', tipo: 'lezione' }]).valido, false)
  })

  it('non accetta una lezione fatta di sole pause', () => {
    assert.equal(validaSlot([creaSlot('08:00', 15, 'pausa')]).valido, false)
  })
...
describe('appello di un file di prima', () => {
  /** Una lezione con `n` UD di lezione, come la scriveva il registro di prima. */
...
describe('id di classe scambiato per id di corso', () => {
  // Il caso vero: chi crea un momento passa la classe dove ci vuole il corso.
  // Il momento esiste, i suoi dati sono buoni, ma non appartiene a niente e
  // nessuna vista lo mostra. In lettura si riaggancia.
  const registro = () =>
    normalizzaRegistro({
      anni: [
        {
...
describe('orario e generazione delle lezioni', () => {
  ...
  it('non tocca le fasce di partenza', () => {
    ...
    ricorrenzeIncatenate(orario)
    assert.equal(orario[1].inizio, '14:00')
  })

  it('descrive una fascia come la si legge sul foglio della sede', () => {
    assert.equal(
      descriviRicorrenza({ id: 'r', giorno: 2, inizio: '08:20', durataMin: 90, aula: 'A12' }),
      'mar 08:20–09:50 · 2 UD · A12',
    )
  })

  it('una fascia dura unità didattiche intere, comunque la si crei', () => {
    assert.equal(MINUTI_UD, 45)
    assert.equal(creaRicorrenza(2, '08:20', 100).durataMin, 90)
    assert.equal(creaRicorrenza(2, '08:20', 45).durataMin, 45)
  })

  it('rifiuta una fascia che non è un multiplo dell’unità didattica', () => {
    const buona = validaRicorrenza({ id: 'r', giorno: 2, inizio: '08:20', durataMin: 90 })
    assert.equal(buona.valido, true)
    const storta = validaRicorrenza({ id: 'r', giorno: 2, inizio: '08:20', durataMin: 50 })
...
describe('la ponderazione di un momento di valutazione', () => {
  // Il peso è decimale davvero — «questa conta una volta e mezza», «il test
  // d'ingresso vale un quarto» — e va da zero a dieci. Zero è ammesso e vuol
  // dire «non fa media»: prima il minimo era 0,1 e per dire che una prova non
  // contava bisognava non metterci i voti.
  ...
  it('un peso di zero non entra nella media', () => {
    // Prima lo zero veniva letto come uno — una guardia contro i dati storti —
    // e una prova dichiarata senza peso finiva per contare come tutte le altre.
...
describe('la nota di fine semestre', () => {
  ...
  // prova. Questo è la regola con cui una media diventa la nota della pagella,
  // e nelle scuole non è la stessa — quarti durante l'anno, mezzi a fine
  // semestre.

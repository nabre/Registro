// Il menu dell'icona accanto all'orologio.
//
// È l'unica vista del registro che si legge senza aprirlo, e quel che vi si
// chiede è una distinzione sola: che cosa è a posto, che cosa è rimasto aperto,
// che cosa sta succedendo adesso e che cosa viene. Sbagliarla non si vede — il
// menu si apre lo stesso — e si paga un mese dopo, con un'ora senza appello che
// nessuno ha più segnalato.
//
// Le prove guardano le stringhe, che qui sono l'interfaccia: il segno in prima
// colonna e il motivo in coda sono quel che chi insegna legge davvero.
...
import { scuolaMinima } from '../aiuti/registro.mjs'
...
describe('il menu del vassoio', () => {
  ...
  it('conta in testa le ore da chiudere di tutti i corsi', () => {
    ...
    assert.equal(albero.intestazione, 'Registro docenti — 3 ore da chiudere')
  })
  ...
  it('niente da chiudere si dice, invece di lasciare la riga muta', () => {
    ...
    assert.equal(albero.intestazione, 'Registro docenti — niente da chiudere')
  })
  ...
  it('i corsi restano in ordine di nome, non di urgenza', () => {
    ...
    // La matematica ha il buco, la chimica no: se il menu si riordinasse per
    // urgenza, la matematica passerebbe davanti e domani sarebbe altrove.
    ora(registro, corso, '2026-10-12')
  ...
  it('un registro senza corsi non inventa niente', () => {
    ...
    assert.equal(albero.suggerimento, 'Registro docenti')
  })
})

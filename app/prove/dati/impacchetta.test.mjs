// Il trasloco dalle cartelle ai documenti.
//
// È codice che gira una volta sola, sui dati veri di un docente, e che nessuno
// rivedrà mai in azione: o funziona la prima volta o si è perso un anno di
// lezioni. Per questo la prova mette in piedi una cartella `dati/` com'era
// prima — con lo storico dentro, e con i file di scarto che ci si depositano —
// e guarda che cosa resta dopo.
//
// Le tre promesse, in ordine di importanza: quel che c'era si ritrova dentro
// il documento; la cartella di prima non viene cancellata sul serio ma va nel
// cestino; e un anno che ha già il suo documento non si tocca.
...
before(async () => {
  ...
  ;({ impacchettaAnni } = await import('../../dist-prove/anni.mjs'))
...
function vociDelDocumento (nome) {
  return leggiZip(readFileSync(percorso.join(dati, `${nome}.registro`))).map((v) => v.nome)
}
...
describe('dalle cartelle ai documenti', () => {
  it('impacchetta un anno e manda via la cartella di prima', async () => {
    ...
    assert.deepEqual(await impacchettaAnni(), ['2026-2027'])
    ...
    // La cartella dell'anno resta: dentro ci sono allegati ed esportazioni, che
    // nel documento non entrano.
    assert.equal(existsSync(percorso.join(dati, '2026-2027')), true)
  })

  it('non ripassa su un anno già impacchettato', async () => {
    // La cartella `dati/` ricompare: è la sincronizzazione che porta indietro
    // quel che una macchina non ancora aggiornata continua a scrivere.
    const tornata = annoAllaVecchiaManiera('2026-2027')

    assert.deepEqual(await impacchettaAnni(), [])
    // E soprattutto: il documento di oggi non è stato coperto da quei file.
    assert.equal(existsSync(tornata), true)
    const dentro = leggiZip(readFileSync(percorso.join(dati, '2026-2027.registro')))
  ...
  it('lascia fuori quel che non sono dati', async () => {
    ...
    assert.deepEqual(await impacchettaAnni(), ['2027-2028'])
    ...
    // La copia di un file rotto invece entra: è un JSON messo da parte dal
    // registro, e buttarlo via qui vorrebbe dire buttare via l'unica copia di
    // quel che non si era saputo leggere.
  ...
  it('non tocca una cartella che non è un anno', async () => {
    ...
    assert.deepEqual(await impacchettaAnni(), [])
    assert.equal(existsSync(percorso.join(dati, 'appunti.registro')), false)
  })
})

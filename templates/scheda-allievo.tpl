...
# che non scriverlo.
campi: {{frase.stato}}={{stato}}; {{frase.data-di-nascita}}={{nascita}}; {{frase.indirizzo}}={{indirizzo}}; {{frase.email}}={{email}}; {{frase.telefono}}={{telefoni}}; {{frase.rappresentante}}={{tutore}}; {{frase.azienda}}={{azienda}}; {{frase.indirizzo-azienda}}={{indirizzoDatore}}; {{frase.datore}}={{datore}}; {{frase.telefono-rappresentante}}={{telefoniRappresentante}}; {{frase.telefono-datore}}={{telefoniDatore}} | colonne 1
...
campi: {{frase.ud-previste}}={{udPreviste}}; {{frase.ud-seguite}}={{udSeguite}}; {{frase.ud-di-assenza}}={{udAssenza}}; {{frase.ritardi}}={{ritardi}}; {{frase.presenza}}={{presenza}}; {{frase.assenza}}={{assenza}}; {{frase.appello-fatto-su}}={{appello}}
...
# Quel che si è segnato sulla matrice del comportamento, un'ora per riga: sta
# dopo le annotazioni perché si legge insieme a quelle — le une dicono che cosa
# è stato scritto, l'altra come è andata giorno per giorno. Senza caselle

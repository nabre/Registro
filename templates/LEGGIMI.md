...
cartella. Sono file di testo con righe `direttiva: contenuto`, interpretati
mentre si stampa.
...
modello non scrive nessuna parola — `sezione: {{frase.presenze}}`, non
`sezione: Presenze` — e le parole stanno in un file per lingua: `_testi.tpl` è
l'italiano, `_testi-de.tpl` il tedesco, `_testi-fr.tpl` il francese,
`_testi-en.tpl` l'inglese. La struttura è una sola, e un modello cambiato vale
in tutte le lingue insieme; vedi più sotto, «Tradurre un modello».
...
**La firma delle e-mail di serie sta in `_firma.html`.** Non è un modello di
rapporto: è quel che il registro mette in fondo a ogni messaggio che spedisce,
riempito con `{{docente}}` e `{{sede}}` dell'intestazione del documento. Chi ne
vuole una sua la scrive nell'intestazione, e da lì vale quella; senza nome né
sede la mail parte senza firma. È HTML, e con una firma in HTML tutta l'e-mail
parte in HTML.

Sono quattro strati e non uno perché si toccano per motivi diversi: in `_base`
si cambia *che cosa* c'è scritto in testata, in `_stile` *quanto è grande*, in
...
[piede]
riga: {{titolo}} | | {{frase.pagina-di-pagine}}
...
`{{titolo}}` è il nome del rapporto — «Verbale della lezione» — scritto dal
registro nella lingua in cui stampa; in testa al file è il titolo che il PDF
porta nelle sue proprietà.
...
```
campi: {{frase.indirizzo}}={{indirizzo}}; {{frase.email}}={{email}}
campi: {{frase.indirizzo}}={{indirizzo}}; {{frase.email}}={{email}} | colonne 1
```

A sinistra dell'uguale l'etichetta, che è una frase di `_testi.tpl`; a destra
il valore.
...
```
riquadro: {{frase.nota-semestre}}={{notaSemestre}}
```
...
sarebbe una trappola. Il nome di serie è quello italiano in tutte le lingue: il
foglio tedesco scrive «LP» in cima alla prima colonna, e `tabella: presenze |
PiF` la trova lo stesso. Un `*` sta per «tutte le altre, nell'ordine loro»: è come
...
`logo.png` è un nome riservato: vuol dire «il logo dell'intestazione del
documento», qualunque nome abbia là dentro, e l'altezza scritta qui vale solo
finché il documento non ne sceglie un'altra. Un percorso con delle barre —
`{{foto}}` diventa `archivio/docente-di-classe/DIC4a/foto/Rossi Mario.jpg` — è
un file della cartella dell'anno, ed è così che il ritratto di una persona in
formazione arriva sulla sua scheda. Le risalite `..` non passano. Altri nomi
secchi non trovano niente: le immagini non stanno più in questa cartella.
Si dichiara solo l'altezza, in millimetri: la larghezza viene dalle proporzioni del
...
`_base.tpl` porta `immagine: logo.png | altezza 14 | destra`: è il logo della
scuola in alto a destra, scelto nell'intestazione del documento. Senza logo la
riga sparisce, e la banda resta bassa.
...
quel che permette a `_base.tpl` di prevedere una riga per il nome della scuola
(`{{sede}}`) senza obbligare chi non ce l'ha a cancellarla.
...
Frase è tutto quel che si legge: un titolo di sezione di una parola sola è una
frase come la nota di cinque righe sulle percentuali. Un'etichetta di `campi:` o
di `riquadro:` non può contenere `=`, `;` né `|`, che sono i segni con cui la
riga si divide.
...
`[colonne]` è di una lingua sola, come le frasi: un nome cambiato in
`_testi.tpl` vale per i fogli italiani, e in `_testi-de.tpl` per quelli
tedeschi.

**Che frasi ci sono, e dove.** Tutte stanno sotto `[frasi]` in `_testi.tpl`,
divise per specie: la testata e il piede, i sottotitoli, le etichette dei campi,
i titoli delle sezioni, e le frasi con dentro un numero —
`riepilogo-presenze`, `appello-incompleto`, `legenda-presenze`,
`nota-presenze`, `esecuzione-riconsegna`, `oltre-soglia`. I numeri che ci vanno
dentro li mette il registro; le parole attorno sono lì e si cambiano. Della
legenda dell'appello le **lettere** restano del codice — sono le stesse che si
premono nella griglia a schermo, e riscriverle qui vorrebbe dire poter dire una
cosa diversa da quella che si compila — mentre le parole si scrivono in
`_testi.tpl`.
...
Un modello non si traduce: si traducono le sue parole. Le direttive —
`titolo:`, `sezione:`, `campi:`, `se:`, `usa:`, `[corpo]`, `verticale`,
`colonne 1` — sono il linguaggio dei modelli, e restano scritte così in tutte
le lingue; i nomi dei segnaposto, delle tabelle e dei blocchi anche.

- **Una parola nuova in un modello** si scrive come frase: `sezione:
  {{frase.mia-sezione}}` nel modello, e `mia-sezione: …` sotto `[frasi]` in
  **tutti e quattro** i `_testi`. Una frase che manca in una lingua non ripiega
  sull'italiano: esce vuota, e la riga con lei.
- **Una frase cambiata** si cambia in ogni lingua in cui dice un'altra cosa.
...
traduzione abbia gli stessi segnaposto, che nessuna sia rimasta in italiano, e
che nei modelli non sia rimasta una parola scritta fuori dai `_testi`
(`tests/data/templateLanguages.test.mjs`).
...
o «ganzes Jahr». I dati del documento — nomi, titoli delle prove, note —
